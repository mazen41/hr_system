...
SalaryPeriods AS (
    SELECT 
        UserID,
        GREATEST(conform_date, '$date_range[0]') AS period_start,
        LEAST(COALESCE(new_e_date, '$date_range[1]'), '$date_range[1]') AS period_end,
        Salary,
        DATEDIFF(
            LEAST(COALESCE(new_e_date, '$date_range[1]'), '$date_range[1]'),
            GREATEST(conform_date, '$date_range[0]')
        ) + 1 AS days_in_period,
        BranchID,
        Id AS renewal_id,
        ROW_NUMBER() OVER (
...
AllEmployees AS (
    ...
    FROM tblusers e
    JOIN tblremewal eu ON eu.Id = e.lastversion
    JOIN attendancet a ON e.UserID = a.EmpID
    JOIN tbshift p ON FIND_IN_SET(p.ShiftID, eu.shiftID) > 0
    JOIN shift_setting s ON p.ShiftID = s.shift_id
    ...
    GROUP BY e.UserID, a.Date, p.ShiftID
),
...
EmployeeWorkHours AS (
    SELECT 
        ...
        ath.totalhour,
        SUM(CASE
            WHEN es.termination_type IS NOT NULL AND es.termination_status IS NOT NULL AND d.Date >= es.termination_date THEN '00:00'
            WHEN es.new_e_date IS NOT NULL AND d.Date > es.new_e_date THEN '00:00'
            WHEN es.IsDisabled IS NOT NULL THEN '00:00'
            WHEN h.Date IS NOT NULL and h.Date not between es.new_s_date and es.new_e_date  THEN '00:00'
            -- edit
            -- WHEN d.Date IS NOT NULL and d.Date not between es.new_s_date and es.new_e_date  THEN '00:00'
        ...
        COUNT(DISTINCT CASE
            WHEN h.Date IS NULL AND el.UserID IS NULL AND 
                 (ea.valid_checkin IS NOT NULL OR ea.first_punch IS NOT NULL) 
            THEN d.Date
...
SELECT DISTINCT
    ...
    (SELECT GROUP_CONCAT(CONCAT(period_start, ' إلى ', period_end, ': ', Salary, ' ريال') SEPARATOR '; ')
     FROM SalaryPeriods sp WHERE sp.UserID = u.UserID) AS salary_periods_details
