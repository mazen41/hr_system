 <?php
if(!isset($_GET['id'])){
	echo'<script> location.replace("report-leaveRequest"); </script>';
	die(); 
} 
$screen = 'إجازة';
$page_title = 'تفاصيل  الاجازه';
$report_name = 'تفاصيل الاجازة';
...
$query = " SELECT a.Id ,a.UserID,a.leavetype,a.path ,a.leave_start_date,a.leave_end_date, a.day_leave,a.status,a.Draft,
f.FirstName as f_name, f.LastName as l_name,b.branch_name as bran,a.description as descp,w.Name
,w.Description,w.isaccept,w.type,w.state,w.chose,w.AmountType,w.Amount,w.RequiresAttachment,cc.branch_name

FROM   tblleaverequest AS a 
LEFT JOIN  leaveclassification AS w ON w.Id = a.leavetype 
LEFT JOIN branches AS b ON a.BranchID = b.branch_id
LEFT JOIN branches AS cc ON w.BranchID = cc.branch_id
      ...
      <div class="container-fluid "> 
        <div class="row ">
         <div class="col-md-9 col-sm-7 col-xs-12">
            <span class="page-title"><?= $row['f_name'].' '.$row['l_name']?> </span>
...
/* 
	...
	<?php endif;?>
			...
			<div class="card-body p-2">
              ...
              <li class="nav-item">
                <a class="nav-link active" id="main-info-tab" data-toggle="pill" href="#main-info" role="tab" aria-controls="main-info" aria-selected="true"><strong>عن الاجازة</strong></a>
              </li>            
			</ul>
			...
			<!-- start -->
                               ...
                               <div class="section">
            <div class="section-title">معلومات الاجازة</div>
            <table>
                <tbody><tr>
                    <td style="width: 25%; font-weight: bold;">اسم الاجازة</td>
                    <td style="width: 25%;"><?= !empty($row['Name'])?$row['Name']:'غير محدد' ?></td>
                    <td style="width: 25%; font-weight: bold;">الفرع</td>
                    <td style="width: 25%;"><?= !empty($row['branch_name'])?$row['branch_name']:'غير محدد' ?></td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">حالة الاجازة</td>
                    <td><?= !empty($row['state'])?'موقفة':'نشطة' ?></td>
                    <td style="font-weight: bold;">نوع الاجازة</td>
                    <td><?= ($row['type']==1)?'مدفوعه كليا':(($row['type']==2)?'مدفوعه جزئياً':'غير مدفوعه')?></td>
                </tr>


                 <tr>
                    <td style="font-weight: bold;">تحتاج موافقه</td>
                    <td><?= ($row['state']==1)?'نعم':'لا' ?></td>
                    <td style="font-weight: bold;">تتطلب ملافق</td>
                    <td><?= ($row['state']==1)?'نعم':'لا' ?></td>
                </tr>

<tr>
<td style="font-weight: bold;">بواسطة</td>
<td><?= !empty($row['FirstName'])?$row['FirstName'].' '.$row['LastName'] :'غير معتمد' ?></td>
                
<?php if (!empty($row['type']) && $row['type']==2 ) { ?>
                
                    <td style="font-weight: bold;">المنسبة بالنسبة لليوم الواحد</td>
                    <td colspan="3"><?= !empty($row['Amount'])?$row['Amount'].' '.$row['AmountType']:'غير محدد' ?></td>
                </tr>
<?php  } ?>
                               ...
                               <div class="section">
            <div class="section-title">معلومات الموظف مع الاجازة</div>
            <table>
                <tbody><tr>
                    <td style="width: 25%; font-weight: bold;">اسم الموظف</td>
                    <td style="width: 25%;"><?= !empty($row['f_name'])?$row['f_name'].' '.$row['l_name']:'غير محدد' ?></td>
                    <td style="width: 25%; font-weight: bold;">الفرع</td>
                    <td style="width: 25%;"><?= !empty($row['bran'])?$row['bran']:'غير محدد' ?></td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">تاريخ البدء</td>
                    <td><?= !empty($row['"leave_start_date"'])?$row['"leave_start_date"']:'نشطة' ?></td>
                    <td style="font-weight: bold;">نوع الاجازة</td>
                    <td><?= !empty($row['"leave_end_date"'])?$row['"leave_end_date"']:'نشطة' ?></td>
                </tr>


                 <tr>
                    <td style="font-weight: bold;">تم الرفع</td>
                    <td><?= !empty($row["Draft"]) ? 'مسودة' : 'تم الرفع' ?></td>
                    <td style="font-weight: bold;">الاعتماد</td>
                    <td><?= !empty($row["status"]) ? 'معتمد' : 'غير معتمد' ?></td>
                </tr>

                <tr>
                    <td style="font-weight: bold;">عدد ايام الاجازة</td>
                    <td><?= !empty($row["day_leave"]) ? $row["day_leave"] : 'غير محدد' ?></td>
                    <td style="font-weight: bold;">الملف</td>
                    <td>
...
<tr>
        ...
        <div class="section">
            <div class="section-title">الوصف العام للاجازة</div>
            <table>
                <thead>
                </thead>
                <tbody>
        ...
        <?php endif;?>
                ...
                <?php if(!empty($row["descp"])) :?>
        <div class="section">
            <div class="section-title">الوصف الموظف لسبب الاجازة</div>
            <table>
                <thead>
                </thead>
                <tbody>
