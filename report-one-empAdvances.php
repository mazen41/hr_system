   <?php
if(!isset($_GET['id'])){
	echo'<script> location.replace("report-empAdvances"); </script>';
	die(); 
} 
$screen = 'السلف';
$page_title = 'تفاصيل السلف';
$report_name = 'تفاصيل السلف';
...
$query = " SELECT a.Id ,a.UserID,a.Amount,a.currency ,a.Status,a.LastUpdateDate, a.Draft,a.DueDate,
f.FirstName as f_name, f.LastName as l_name,b.branch_name,a.type,a.description
...
/* 
	...
	<?php endif;?>
        ...
        <div class="section">
            <div class="section-title">تقرير سلفة الموظف</div>
            <table>
                <thead>
                    <tr>
                        <th >#</th>
...
foreach ($row as $index => $ins) { 
    ...
    <tr onclick="toggleReason('<?= $rowId ?>')" style="cursor:pointer;">
        ...
        <td><?= $ins["Amount"] . ' ' . $ins["currency"] ?></td>
        <td><?= $ins["DueDate"] ?></td>
        <td><?= !empty($ins["Status"]) ? 'معتمد' : 'غير معتمد' ?></td>
        <td><?= !empty($ins["Draft"]) ? 'مسودة' : 'تم الرفع' ?></td>
        <td><?= ($ins["type"] == 1) ? 'سلفة على الراتب' : 'سلفة خارج الراتب' ?></td>
    </tr>
    <tr id="<?= $rowId ?>" style="display:none; background:#f9f9f9;">
        <td colspan="7">
            <strong>السبب:</strong> <?= !empty($ins["description"]) ? $ins["description"] : 'لا يوجد سبب مسجل' ?>
        </td>
    </tr>
<?php } ?>
                </tbody>
