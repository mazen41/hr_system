<?php
if(!isset($_GET['id'])){
	echo'<script> location.replace("order-emp-list-admin"); </script>';
	die(); 
}  
...
$page_title = 'إدارة الطلبات ';
 ...
 if (isset($_GET['id'])) {
    ...
    $query = "SELECT a.Id , a.UserID,a.Status,a.Draft,a.description,a.title,a.isread,
    ...
    if ($st->rowCount() > 0) {
		...
		$isread=isread($connect_pdo, $get_id);
...
function confirm_check (id) {
	if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
				  ...
				  $('#modal_default .modal-dialog').removeClass('modal-lg');
				$('#modal_default').modal({show:true});
			  $('#modal_default .modal-body').load('./hr-app/order-emp-conform-admin?id='+id+'',function(){
			});
		}
	}    
