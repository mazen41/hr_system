...
class BiometricSync
{
    ...
    public function syncDevice(int $deviceId): array
    {
        ...
        if (!$device) {
            return ['success' => false, 'device_id' => $deviceId, 'error' => 'Device not found', 'records_imported' => 0];
        }
    ...
    private function importLogs(array $logs, int $branchId, int $deviceId): int
    {
        ...
        $insertStm = $this->pdo->prepare(
            "INSERT INTO tblattendance (EmpID, BranchID, Date, Type, Time, source, who_add, CreatedDate)
             VALUES (:emp, :branch, :date, :type, :time, 'device', NULL, NOW())"
        );

        $checkStm = $this->pdo->prepare(
            "SELECT AttendanceID FROM tblattendance
             WHERE EmpID = :emp AND Date = :date AND Time = :time AND source = 'device'
             LIMIT 1"
        );
            ...
            try {
                $this->pdo->prepare(
                    "INSERT INTO audit_log (user_id, action, table_name, notes, ip_address, created_at)
                     VALUES (NULL, 'device_sync', 'tblattendance', :notes, :ip, NOW())"
                )->execute([
    ...
    public function testConnection(int $deviceId): array
    {
        ...
        if (!$device || empty($device['ip'])) {
            return ['connected' => false, 'error' => 'Device not found or no IP'];
        }
