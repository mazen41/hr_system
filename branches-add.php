...
function allApps($connect){
    $data=[];
    $q = "SELECT AppID,AppName
	FROM apps 
    where Disabled is  null AND IsRrequred is  null 
	order by apps.Sort ";
    $stm = $connect->prepare($q);
    $stm->execute();  
    if($stm->rowCount() > 0){
...
.no-brder-input  tbody td,.no-brder-input  tfoot td {
	padding: 0.1rem !important;
}
.no-brder-input  .select2-selection--single,.no-brder-input  input[type="text"]{
    ...
    <!-- Content Header (Page header) -->
                ...
                <div class="row">
                    <div class="col-md-2">
                        <div class="form-group">
                            <label for="br_no" class="col-form-label required">رقم الفرع</label>
                                <input type="number" value="<?=!empty($branch_ref) ? $branch_ref:''?>" class="form-control number-format "   title="ادخل رقم مميز للفرع" id="br_no" name="br_no" placeholder="" autocomplete="off" style="direction:rtl" <?=!empty($branch_name) ? 'readonly':'' ?> required>
                        </div>
                    </div>
                    ...
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="br_name" class="col-form-label required">اسم الفرع</label>
                                <input type="text" value="<?=!empty($branch_name) ? $branch_name :''?>" class="form-control "   title="إكتب اسم الفرع" id="br_name" name="br_name" placeholder="" autocomplete="off" required>
                        </div>
                    </div>
                    ...
                    <div class="col-md-12">
                        ...
                        <div class="form-group">
                            ...
                            <label for="br_style" class="col-form-label ">ستايل خاص : <ul class="fc-color-picker branch_style" id="color-chooser">
                      <li data-color="#f06292"><span style="color:#f06292" ><i class="fas fa-square"></i></span></li>
                      <li data-color="#009688"><span style="color:#009688" ><i class="fas fa-square"></i></span></li>
                      <li data-color="#9c27b0"><span style="color:#9c27b0" ><i class="fas fa-square"></i></span></li>
                      <li data-color="#00bcd4"><span style="color:#00bcd4" ><i class="fas fa-square"></i></span></li>
                      <li data-color="#007bff"><span class="text-primary" ><i class="fas fa-square"></i></span></li>
                      <li data-color="#ffc107"><span class="text-warning"><i class="fas fa-square"></i></span></li>
                      <li data-color="#28a745"><span class="text-success"><i class="fas fa-square"></i></span></li>
                      <li data-color="#dc3545"><span class="text-danger"><i class="fas fa-square"></i></span></li>
                      <li data-color="#6c757d"><span class="text-muted"><i class="fas fa-square"></i></span></li>
                      <li><span style="color:#247dbd" ><i class="fas fa-square"></i></span></li>
                    </ul></label>
                    <div style="margin-right: 5px; height: 25px;  width: 305px;background:<?=!empty($branch_style) ? $branch_style :'#247dbd'?>"id="privew_style"></div>
                                <input type="hidden" value="<?=!empty($branch_style) ? $branch_style :''?>" id="br_style" name="br_style" >
                        </div>
                    </div>
                    <?php
                    if(!empty($branch_name) && empty($branch_data['isdefault'])){ ?>
                    <div class="col-sm-12">
            ...
            <div class="col-md-6">
					  ...
					  <div class="row">		  
						<div class="col-md-6">
						  ...
						  <div class="form-group">
							...
							<input type="text" class="form-control req" data-toggle="tooltip" title="ادخل العنوان" id="street" name="street" placeholder="" value="<?=!empty($address['Street'])? $address['Street']:''?>" required>
						  </div>
						</div>
						...
						<div class="col-md-6">
						  <div class="form-group">
							...
							<input type="text" class="form-control req" id="block" name="block" autocomplete="off" data-toggle="tooltip" title="ادخل اسم الحي" value="<?=!empty($address['Block'])? $address['Block']:''?>" required>
						  </div>
						</div>
						...
						<div class="col-md-6">
						  <div class="form-group">
							...
							<input type="text" class="form-control req" id="city" name="city"  title="ادخل اسم المدينة / المحافظة" data-toggle="tooltip" value="<?=!empty($address['City'])? $address['City']:''?>" required> 
						  </div>
						</div>
						...
						<div class="col-md-6">
						  <div class="form-group">
							...
							<input type="text" class="form-control number-format" data-toggle="tooltip" title="ادخل رقم المبنى" id="building" name="building" placeholder="" autocomplete="off" value="<?=!empty($address['Building'])? $address['Building']:''?>">
						  </div>
						</div>
						...
						<div class="col-md-6">
						  <div class="form-group">
							...
							<input type="text" class="form-control number-format" data-toggle="tooltip" title="ادخل رقم الهاتف" id="phone" name="phone" placeholder="" autocomplete="off" value="<?=!empty($address['Phone'])? $address['Phone']:''?>">
						  </div>
						</div>
						...
						<div class="col-md-6">
						  <div class="form-group">
							...
							<input type="text" class="form-control number-format" data-toggle="tooltip" title="ادخل رقم الجوال" id="mobile" name="mobile" placeholder="" autocomplete="off"  value="<?=!empty($address['Mobile'])? $address['Mobile']:''?>">
						  </div>
						</div>
						...
						<div class="col-md-6">
						  <div class="form-group">
							...
							<input type="text" class="form-control number-format" id="postal_code" name="zip" autocomplete="off" data-toggle="zip" title="ادخل الرمز البريدي" value="<?=!empty($address['ZipCode'])? $address['ZipCode']:''?>">
						  </div>
						</div>
						...
						<div class="col-md-6">
						  <div class="form-group">
							...
							<input type="text" class="form-control " id="email" name="email" autocomplete="off" data-toggle="zip" title="ادخل عنوان البريد الالكتروني"  autocomplete="off" value="<?=!empty($address['Email'])? $address['Email']:''?>">
						  </div>
						</div>
						...
						<div class="col-md-6">
						  <div class="form-group">
							...
							<input type="text" class="form-control number-format" id="vat" name="vat" autocomplete="off" data-toggle="tooltip" title="ادخل الرقم الضريبي" value="<?=!empty($address['VatNumber'])? $address['VatNumber']:''?>">
						  </div>
						</div>
                        <div class="col-md-6">
						  <div class="form-group">
							<label class="col-form-label " for="vat_g">رقم المجموعة الضريبية</label>
							<input type="text" class="form-control number-format" id="vat_g" name="vat_g" autocomplete="off" data-toggle="tooltip" title="إذا كان الفرع ضمن مجموعة ضريبية"  value="<?=!empty($address['VatGNumber'])? $address['VatGNumber']:''?>">
						  </div>
						</div>
						...
						<div class="col-md-8">
						  <div class="form-group">
							...
							<select id="idtype" name="idtype" class="form-control selectpicker" data-container="body" data-size="12" data-width="100%" data-toggle="tooltip" title="حدد نوع الهوية" <?= (!empty($site['IdentityDetail']) ? 'required' :'')?>>
                ...
                echo '<option value="'.$id['IDType'].'" data-subtext=" ('.$id['IDType'].') " '.(!empty($address['IdentityType']) && $address['IdentityType'] == $id['IDType'] ? 'selected' :'').'> '.$id['TypeName'].'</option>';
						...
						<div class="col-md-4" id="idnfield" style="display:<?=(!empty($address['IdentityDetail']) ? '':'none')?>">
						  <div class="form-group">
							<label class="col-form-label " for="idno">معرف الهوية</label>
							<input type="text" class="form-control" id="idno" name="idno" autocomplete="off" data-toggle="tooltip" title="ادخل رقم الهوية" value="<?=(!empty($address['IdentityDetail']) ? $address['IdentityDetail']:'')?>"  <?= (!empty($address['IdentityType']) ? 'required' :'')?>>
						  </div>
						</div>
					...
					<div class="card-footer text-left" style="display: none;">
						  ...
						  <div class="form-group">
                              ...
                              if(!empty($all_apps)){
								...
								<div class="custom-control custom-checkbox">
								  ...
								  <label for="customCheckbox1" class="custom-control-label "> '.$v.'</label>
								</div>';
								}
                              }
							  ?>
...
$.ajax({
		...
		success:function(data){
			...
			//$('.default_currency').text(currencies[1].short_name);
				//alert(currencies[1].short_name);
			$.each(data.data, function(k, v) {
				if(data.data[k].id != $('#br_currency').val()){
...
/* $('#use_main_address').change(function(){

    if($(this).is(':checked')){
        $("#address_container").collapse('toggle');
 ...
 */
/* $('#all_apps').change(function(){

    if($(this).is(':checked')){
        $("#apps_container").collapse('toggle');
    }
    else
    {
        $("#apps_container").collapse('toggle');
