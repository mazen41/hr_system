...
$appid  = 'HR';
$page_perm=['عرض تعويض','تعديل تعويض','حذف تعويض','اعتماد تعويض','إضافة تعويض'];
// $screen = 'التعويضات';
...
$page_title = 'إدارة التعويضات والمزايا ';
 ...
 if (isset($_GET['id'])) {
    ...
    $query = "SELECT a.Id, a.BranchID, a.UserID, a.name, a.Amount,a.AmountType ,a.Currency, a.Reason, a.for_what, a.extionsion,a.Status ,
                     a.DueDate, a.CreatedDate, a.LastUpdateDate, a.CreatedBy, u.FirstName, u.LastName, d.branch_name,a.beneft_type
...
.bdanger{ border-right:0.3rem solid red; display:block;}
	...
	<?php endif;?>
					...
					<div class="card">
						<div class="card-header p-2 d-print-none with-nav-btn">
                            ...
                            <?php  if ($User->isAllowedPerm(['تعديل تعويض'], $appid)) {  
                                 if($flage){ 
                                ?>
								<a href="Benefits-add?id=<?=$row['Id']?>" class="btn btn-default btn-white  btn-sm quick-action-btn">
								<span class="fa fa-edit" aria-hidden="true"></span>
								تعديل</a>
								<?php  }
                            }
                                if ($User->isAllowedPerm(['حذف تعويض'], $appid)) { ?>
                                ...
                                <a href="Benefits-add?copy=1&id=<?=$row['Id']?>" class="btn btn-default btn-white  btn-sm quick-action-btn">
								<span class="fa fa-copy" aria-hidden="true"></span>
								نسخ</a>
                                <?php  } if ($User->isAllowedPerm(['اعتماد تعويض'], $appid)) { ?>
			...
			<div class="card-body p-2">
              ...
              <li class="nav-item">
                <a class="nav-link active" id="main-info-tab" data-toggle="pill" href="#main-info" role="tab" aria-controls="main-info" aria-selected="true"><strong>عن التعويض والمزياء</strong></a>
              </li>            
			</ul>
...
function confirm_remove (id) {
	if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  $('#modal_default .modal-body').load('./hr-app/Benefits-remove?id='+id+'',function(){
			});
		}
	}
...
function confirm_check (id) {
	if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
				  ...
				  $('#modal_default .modal-dialog').removeClass('modal-lg');
				$('#modal_default').modal({show:true});
			  $('#modal_default .modal-body').load('./hr-app/Benefits-conform?id='+id+'',function(){
			});
		}
	}    
