...
entriesData('no');
function entriesData(is_date_search,filter_type='',branchs=[],date_range='',filter_status=''){
	//var account = 0;
	var groupColumn = 0;
  ...
  var dataTable = $('#entries_tb').DataTable({
   ...
   "ajax" : {
	url:"hr-app/index.php?action=report-empAdvances",
	type:"POST",
	data:{
     ...
     buttons: [
         ...
         drawCallback: function (settings) {
           ...
           // $('#selected_client').html(settings.json.results_note.selected_client);
            $('#selected_client').html('<h5>سلف '+$('#select2-client-container').text()+'</h5>');
             }else{
