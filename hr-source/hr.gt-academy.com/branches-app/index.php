            ...
            if ($id > 0) {
                ...
                if ($addressId) {
                    $stmt = $connect_pdo->prepare("UPDATE tbladdress SET 
                        ...
                        WHERE AddressID = ?");
                    $stmt->execute([$street, $block, $city, $building, $phone, $mobile, 
                        $zip, $email, $vat, $vatG, $idType ?: null, $idNo, $addressId]);
                } else {
                    // Create new address
                    $stmt = $connect_pdo->prepare("INSERT INTO tbladdress 
                        (AddressType, Street, Block, City, Building, Phone, Mobile, ZipCode, Email, VatNumber, VatGNumber, IdentityType, IdentityDetail)
                        VALUES ('BRANCH', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->execute([$street, $block, $city, $building, $phone, $mobile, $zip, $email, $vat, $vatG, $idType ?: null, $idNo]);
