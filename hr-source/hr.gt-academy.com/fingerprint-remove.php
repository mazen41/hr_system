...
if(!empty($_GET['confirme']) && !empty($_POST['finger_id']) ){
	
	 $result = false;
	 $reload = false;
	 $msg ='خطاء..!';
	 $id= (int) $_POST['finger_id'];

    function GetlastID($connect, $id) {
        // Check if device is assigned to any employees in the new fingerprint_device_users table
        ...
        // Also check the old table for backward compatibility
	...
	function removeRow($connect, $id) {
		...
		try {
                ...
                $parma2 = ["id" => (int)$id ];
                // First try to delete from the new table
                $q2 = "delete from fingerprint_devices WHERE id = :id LIMIT 1";
                ...
                if($stm2->rowCount() > 0){
                    ...
                    // If not found in new table, try the old table for backward compatibility
                    $q2 = "delete from tbfingerprint WHERE FingerprintID = :id LIMIT 1";
                    $stm2 = $connect->prepare($q2);
                    $stm2->execute($parma2);
		...
		return false;
				...
				{
					$msg = 'هذه الجهاز مرتبطه بمستخدمين ولايمكن حذفها';
				}
...
<form class="form-horizontal" role="form" action="#" method="post" id="remove_confirmed_fm">
	<p class="text-danger"><b>هل ترغب بالفعل بحذف جهاز البصمة</b>
	<input type="hidden" name="finger_id" id="finger_id" class="form-control" value="<?=$_GET['id']?>" >
...
$(document).ready(function(){
    ...
    $("#remove_confirmed_fm").on('submit', function(e){
        ...
        var form_data = $(this).serialize();
            $.ajax({
                url: 'hr-app/index.php?action=fingerprint-remove?confirme=1',
                type: 'POST',
                data:form_data,
                dataType:"json",
                beforeSend:function(){
