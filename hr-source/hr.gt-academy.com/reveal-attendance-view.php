<?php
$rawAttendanceParam = trim((string)($_GET['date'] ?? $_GET['id'] ?? ''));
$employeeId = isset($_GET['emp']) ? (int)$_GET['emp'] : (ctype_digit((string)($_GET['id'] ?? '')) ? (int)$_GET['id'] : 0);
$selectedAttendanceDate = preg_match('/^\d{4}-\d{2}-\d{2}$/', $rawAttendanceParam) ? $rawAttendanceParam : '';
$employee = null;
$stopped_status = 'غير معروف';
$stopped_color = 'secondary';

if ($employeeId > 0) {
	$_GET['id'] = (string)$employeeId;
}
...
$parma = array( ':id'  => $id, );
    ...
    if($stm->rowCount() > 0)
    {
		...
		echo'<script> location.replace("reveal-attendance"); </script>';
		die();
    }	
}
?>
      ...
      <div class="container-fluid ">
        <div class="row ">
         ...
         <span class="badge badge-<?= $stopped_color?>"> <?= $stopped_status?> </span>
            <span class="page-title"><?= htmlspecialchars(trim(($employee['FirstName'] ?? '') . ' ' . ($employee['LastName'] ?? '')), ENT_QUOTES, 'UTF-8') ?> </span>
                               ...
                               <div class="card-body card-body pt-0 pb-0">
                                       <div class="row" style="align-items: flex-end;">
                                        ...
                                        <div class="col-md-8">
                                          ...
                                          <input type="text" name="date_range" class="form-control input-date-range"  placeholder="من - الى" id="date_range" autocomplete="off" value="<?= htmlspecialchars($selectedAttendanceDate ? ($selectedAttendanceDate . ' - ' . $selectedAttendanceDate) : '', ENT_QUOTES, 'UTF-8') ?>">
                                        </div>
...
const employeeId = urlParams.get('emp') || (/^\d+$/.test(rawId) ? rawId : '');
const selectedDate = (/^\d{4}-\d{2}-\d{2}$/.test(urlParams.get('date') || '') ? urlParams.get('date') : (/^\d{4}-\d{2}-\d{2}$/.test(rawId) ? rawId : ''));
const initialDateRange = selectedDate ? (selectedDate + ' - ' + selectedDate) : ($('#date_range').val() || '');

if (!employeeId) {
    window.location.replace('reveal-attendance');
    return;
}
...
function apply_filters()
{
    ...
    clients_data(date_range ? 'yes' : 'no', employeeId, date_range);
}
...
if (initialDateRange) {
    $('#date_range').val(initialDateRange);
}

clients_data(initialDateRange ? 'yes' : 'no', employeeId, initialDateRange);
