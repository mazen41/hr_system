...
if(isset($_GET['add']) || isset($_GET['edit'])){
    ...
    k.Salary,k.Currency,k.new_s_date,k.new_e_date,k.state,
	...
	if($stm->rowCount() > 0){
		...
		echo'<script> location.replace("contractRenewal-emp-list"); </script>';	
	}
    ...
    <div class="row">
			<div class="col-md-12">
                ...
                <div class="card-header"    style="background: aliceblue; cursor: pointer">
					<h4 class="card-title">تفاضيل تجديد الموظف <?= $employee['FirstName'].' '.$employee['LastName']  ?></h4>
					...
					<div class="card-body p-3">
					 ...
					 <div class="row">
                             ...
                             <div class="col-md-4">
                                <div class="form-group">
                                    <label for="user_section" class="col-form-label  logindata required">القسم الجديد</label>
                        ...
                        <div class="col-md-6">
                          ...
                          <div class="form-group col-md-6">
							...
							<?php if(!empty($employee['Currency'])){ ?>
								<!-- <option value="<?= $employee['Currency'] ?>" ><?= $employee['Currency'] ?></option> -->
                   ...
                   <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                          ...
                          <input type="text" name="emp_contract_S" class="form-control input-date"  placeholder="تاريخ بداية العقد" id="emp_contract_S" autocomplete="off" value="<?=(!empty($employee['new_s_date'])? $employee['new_s_date'] : '' )?>" required >
                        </div>
                      </div>
                      <div class="col-md-4">
                        <div class="form-group">
                          <label class="col-form-label required " for="emp_contract_F">تاريخ انتهاء العقد الجديد</label>
                          <input type="text" name="emp_contract_F" class="form-control input-date"  placeholder="تاريخ انتهاء العقد" id="emp_contract_F" autocomplete="off" value="<?=(!empty($employee['new_e_date'])? $employee['new_e_date'] : '' )?>" required>
                          <input type="hidden" name="emp_contract_F_hiden" class="form-control input-date"  placeholder="تاريخ انتهاء العقد" id="emp_contract_F_hiden" autocomplete="off" value="<?=(!empty($employee['new_e_date'])? $employee['new_e_date'] : '' )?>" required>
                      ...
                      <div class="col-md-4">
                        <div class="form-group">
                          ...
                          <input type="text" name="day_before_go" class="form-control "  placeholder="الايام قبل المغادرة" id="day_before_go" autocomplete="off" value="<?=(!empty($employee['day'])? $employee['day'] : '' )?>" >
                        </div>
                      </div>
...
//    var form_data = $(this).serialize() + '&add=' + param_id+ '&edit=' + param_idd+ '&user_shiftt=' + user_shiftt+ '&user_fingerr=' + user_fingerr;
	...
	if($(this).valid()){
	$.ajax({
        url:"hr-app/index.php?action=contractRenewal-add",
        method:"POST", 
		data:form_data,
		dataType:"json", 
		beforeSend:function(){
		...
		success:function(data){
			if(data.result){
				...
				if(data.id > 0){  
                   window.location.href = 'contractRenewal-list';
				}
                if(data.edit){  
                   window.location.href = 'contractRenewal-list';
				}
...
function get_info(branch) {
        ...
        if (selectedValue) {
            $.ajax({
                url: 'hr-app/index.php?action=allUserInfo',
                type: 'POST',
