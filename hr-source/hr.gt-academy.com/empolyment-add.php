...
<?php
$appid  = 'HR';
$page_perm=['اضافة نمط عمل'];

// $screen = 'أنماط التوظيف';
// if(isset($_GET['id'])){
// 	$page_title = 'إدارة انماط التوظيف';
// }else{
// $page_title = 'إضافة نمط جديد';
// }
...
if(isset($_GET['id'])){
	...
	if($st->rowCount() > 0){
		...
		$sql = "SELECT 	TypeID FROM tblremewal WHERE FIND_IN_SET(:id, TypeID) LIMIT 1";
    ...
    <div class="row">
			<div class="col-md-12">
                ...
                <div class="card-header"    style="background: aliceblue; cursor: pointer">
					<h4 class="card-title">تفاضيل النمط</h4>
					...
					<div class="card-body p-3">
					  <div class="row">
                          ...
                          <div class="form-group col-md-6">
							<label class="col-form-label required"  for="empolymentname">اسم النمط </label>
							<input type="text"  value="<?=!empty($row['Name']) ? $row['Name'] : ''?>" class="form-control"  data-toggle="tooltip"  id="empolymentname" name="empolymentname"  autocomplete="off" required>
						 </div>
                         <div class="form-group col-md-6">
                          ...
                          <label class="col-form-label required" for="branchs_list">هذا النمط تم ربطه بموظفين لايمكن تغير الفرع</label>
...
$('#AddShift').on('submit', function(e){  
	...
	if($(this).valid()){
	$.ajax({
        url:"hr-app/index.php?action=empolyment-add",
        method:"POST", 
		data:form_data,
		dataType:"json", 
		beforeSend:function(){
		...
		success:function(data){
			if(data.result){
				...
				if(data.id > 0){  
                   window.location.href = 'empolyment-view?id='+data.id+'';
				}
