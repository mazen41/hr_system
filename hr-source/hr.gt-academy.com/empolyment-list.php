<?php
$appid  = 'HR';
$page_perm=['اضافة نمط عمل','عرض نمط عمل','تعديل نمط عمل','حذف نمط عمل'];

// $screen = 'النمط الوظيفي';
// $page_title = 'إدارة النمط الوظيفي';
      ...
      <div class="container-fluid ">
        <div class="row align-items-center">
          <div class="col-12 col-sm-7 mb-2 mb-sm-0">
            <span class="page-title">النمط</span>
          </div><!-- /.col -->
          <div class="col-12 col-sm-5 text-left text-sm-left">	
          <?php if($User->isAllowedPerm(['اضافة نمط عمل'],$appid)){ ?>
            <div class="d-flex flex-wrap gap-2 justify-content-start justify-content-sm-end">
              <button type="button" class="btn btn-primary btn-sm"  id="add-client-bt"><i class="fas fa-plus"></i><span class="d-none d-sm-inline"> إضافة</span></button>	
              <button type="button" class="btn btn-primary btn-sm"  id="add-client-bt-setting"><i class="fas fa-cog"></i><span class="d-none d-sm-inline"> قائمة الاعدادات</span></button>
            </div>
            <?php
         } 
         ?> 
...
$('#add-client-bt').on('click', function(event){
   window.open("empolyment-add","_self");
});
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='',branchs=[], states=[])
 {
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=empolyment-list",
    type:"POST",
    data:{
    ...
    aoColumns: [
          ...
          var options='<div class="btn-group"> <button type="button" class="btn btn-default " data-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-left" role="menu">';
                <?php if ($User->isAllowedPerm(['عرض نمط عمل'], $appid)) { ?>
                    options += '<a href="empolyment-view?id=' + data.id + '" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['تعديل نمط عمل'], $appid)) { ?>
                    options += '<a href="empolyment-add?id=' + data.id + '" type="button" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['حذف نمط عمل'], $appid)) { ?>
...
function confirm_remove (id) {
		console.log('confirm_remove called with id:', id);
		if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  console.log('Loading modal from: /hr-app/empolyment-remove?id='+id);
			  $('#modal_default .modal-body').load('/hr-app/empolyment-remove?id='+id+'',function(response, status, xhr){
					console.log('Modal load response status:', status);
					console.log('Modal load response:', response);
					if (status === "error") {
...
$(document).on('click', '.remove_client', function(){
	var id = $(this).val();
	console.log('Remove client button clicked with id:', id);
	confirm_remove(id);
