...
$page_title = 'إدارة الطلبات';
 ...
 if (isset($_GET['id'])) {
    ...
    $query = "SELECT a.Id , a.UserID,a.Status,a.Draft,a.description,a.created_by,a.isread,a.title,
    ...
    if ($st->rowCount() > 0) {
        $row = $st->fetch();
            ...
            if(!empty($row['isread'])){
                    $isread_status = 'مقرؤه';
                    $isread_color = 'success';
                }
            else{
                    $isread_status = 'غير مقرؤة';
                    $isread_color = 'danger';
                    }
      ...
      <div class="container-fluid ">
        <div class="row ">
         <div class="col-md-9 col-sm-7 col-xs-12">
            ...
            <span class="badge badge-<?= $isread_color?>"> <?= $isread_status?> </span>
...
.bdanger{ border-right:0.3rem solid red; display:block;}
	...
	<?php endif;?>
			...
			<div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-md-12">
                         <h4 class="headline mt-4">بينات الطلب</h4>
                             ...
                             <?php if(!empty($row['CreatedDate']) ):?>
                              ...
                              <?php if(!empty($row['Status']) ):?>
                               ...
                               <?php endif;?>
                                ...
                                <?php if(!empty($row['isread']) ):?>
                                <tr>
									<td>هل تمت القراءة</td>
									<td><?= (!empty($row['Draft']))?'مقروءه':'غير مقروءه' ?></td>
								</tr>
                                <?php endif;?>
...
function confirm_check (id) {
	if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
				  ...
				  $('#modal_default .modal-dialog').removeClass('modal-lg');
				$('#modal_default').modal({show:true});
			  $('#modal_default .modal-body').load('./hr-app/order-emp-upload?id='+id+'',function(){
			});
		}
	}    
