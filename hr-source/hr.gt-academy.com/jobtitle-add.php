...
<?php
$appid  = 'HR';
$page_perm=['اضافة مسمى وظيفي'];

// $screen = 'المنصب الوظيفي';
// if(isset($_GET['id'])){
// 	$page_title = 'إدارة المنصب';
// }else{
// $page_title = 'إضافة منصب وظيفي';
// }
...
$queryjobtitle = "SELECT *
FROM  tbljobtitle where BranchID in ($allowed_branch) ";
$stjobtitle = $connect_pdo->prepare($queryjobtitle);
$stjobtitle->execute();
if($stjobtitle->rowCount() > 0){
    $allowed_jobtitle = $stjobtitle->fetchAll();
    if (!empty($allowed_jobtitle))
{                                 
    foreach($allowed_jobtitle as $row_){
        $jobtitle [$row_['Id']]= $row_['Name'];
    }
}
} 
...
if(isset($_GET['id'])){
	...
	if($st->rowCount() > 0){
		$row = $st->fetch();
		$client_id = $row['Id'];
		$form_title = 'تعديل مسمى ' .$row['Name'].'';
    ...
    <div class="row">
			<div class="col-md-12">
                ...
                <div class="card-header"    style="background: aliceblue; cursor: pointer">
					<h4 class="card-title">تفاضيل المنصب</h4>
					...
					<div class="card-body p-3">
					  <div class="row">
                          ...
                          <div class="form-group col-md-12">
							<label class="col-form-label required"  for="jobtitlename">اسم المنصب</label>
							<input type="text"  value="<?=!empty($row['Name']) ? $row['Name'] : ''?>" class="form-control"  data-toggle="tooltip"  id="jobtitlename" name="jobtitlename"  autocomplete="off" required>
						 </div>
					 ...
					 <div class="row">
                          ...
                          <label class="col-form-label required" for="branchs_list">هذا المنصب تم ربطه بموظفين لايمكن تغير الفرع</label>
                          ...
                          <label class="col-form-label required" for="branchs_list">هذا المنصب تم ربطه بمناصب اخرى لايمكن تغير الفرع</label>
                              ...
                              <div class="form-group col-md-6">
                                <label class="col-form-label" for="select_jobtitle">الاب</label>
                                <select class=" selectpicker select_jobtitle"  data-live-search="true"  data-container="body" data-size="5" data-width="100%" title="أدخل الاب" id="select_jobtitle" name="select_jobtitle" >
                                    <?php
									if(!empty($row['ParentID'])) {
										echo "<option value='{$row['ParentID']}' selected>{$jobtitle[$row['ParentID']]}</option>";
									}
                                    // else
...
$('#AddShift').on('submit', function(e){  
	...
	$.ajax({
		...
		success:function(data){
			if(data.result){
				...
				if(data.id > 0){  
                   window.location.href = 'jobtitle-view?id='+data.id+'';
				}
	...
	$('#branchs_list').change(function() {
        ...
        if (selectedValue) {
            $.ajax({
                url: 'hr-app/index.php?action=info-of-section-and-job-title',
                type: 'POST',
                ...
                success: function(response) { 
                    // user_manager
					populateSelect('#select_jobtitle', response.jobtitle);
					$('#preloading').hide();
                },
                error: function() {
