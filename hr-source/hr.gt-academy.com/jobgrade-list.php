<?php
$appid  = 'HR';
$page_perm=['اضافة درجة وظيفية','عرض درجة وظيفية','تعديل درجة وظيفية','حذف درجة وظيفية'];
// $screen = 'الدرجات الوظيفية';
// $page_title = 'إدارة الردرجات الوظيفية';
...
.table.dataTable  td, .table.dataTable  th {
    ...
    <div class="content-header page-nav" >
      <div class="container-fluid ">
        <div class="row ">
          <div class="col-7">
            <span class="page-title">الدرجات</span>
          </div><!-- /.col -->
          <div class="col-5 text-left">	
          <?php if($User->isAllowedPerm(['اضافة درجة وظيفية'],$appid)){ ?>
...
$('#add-client-bt').on('click', function(event){
   window.open("jobgrade-add","_self");
});
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='',branchs=[], states=[])
 {
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=jobgrade-list",
    type:"POST",
    data:{
    ...
    aoColumns: [
          ...
          var options='<div class="btn-group"> <button type="button" class="btn btn-default " data-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-left" role="menu">';
                <?php if ($User->isAllowedPerm(['عرض درجة وظيفية'], $appid)) { ?>
                    options += '<a href="jobgrade-view?id=' + data.id + '" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['تعديل درجة وظيفية'], $appid)) { ?>
                    options += '<a href="jobgrade-add?id=' + data.id + '" type="button" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['حذف درجة وظيفية'], $appid)) { ?>
