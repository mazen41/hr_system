...
$query = " SELECT a.Id  ,b.branch_name,a.name,a.Status,a.LastUpdateDate,a.DueDate,
 a.Amount,a.Currency,u.FirstName , u.LastName
...
foreach($result as $row)
{
 ...
 $sub_array[] = $row['Amount'] . ' ' .$row['Currency'];
 $sub_array[] = $row['DueDate'];
 $sub_array[] = $row['FirstName'].' '.$row['LastName'];
 $sub_array[] = ($row['Status']==1)?'معتمد':'غير معتمد';
 $sub_array[] = '<a href="report-one-deductions?id='.$row['Id'].'" class="btn btn-info btn-sm show-details" data-id="'.$row['Id'].'" title="تفاصيل"><i class="fa fa-eye"></i></button>';
