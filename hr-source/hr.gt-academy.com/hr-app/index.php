...
case 'save-mail-settings':
    if (!($User->userIsAdmin() || $User->userIsEmployer())) { $result=false; $msg='ØºÙŠØ± Ù…ØµØ±Ø­'; break; }
    ...
    if ($type === 'smtp') {
        ...
        $result = false;
        $msg = 'Ù†ÙˆØ¹ Ø§Ù„Ø¥Ø¹Ø¯Ø§Ø¯ ØºÙŠØ± Ù…Ø¹Ø±ÙˆÙ';
    }
    break;
/*
        $current['smtp'] = [
...
case 'allUserInfo': // This is the new case to handle the frontend request
    ...
    if ($branchId <= 0) {
        $result = false;
        $msg = 'Ù…Ø¹Ø±Ù Ø§Ù„ÙØ±Ø¹ ØºÙŠØ± ØµØ§Ù„Ø­';
        break;
    }
    ...
    $result = true;
    $msg = 'ØªÙ… Ø¬Ù„Ø¨ Ø¨ÙŠØ§Ù†Ø§Øª Ø§Ù„ÙØ±Ø¹ Ø¨Ù†Ø¬Ø§Ø­';
    break;
...
function handleReportAllEmployer($pdo, $User, $employee_status_map) {
    ...
    if (empty($allowed_user_branch_ids)) {
        echo json_encode([
            ...
            "results_note" => [
                ...
                "selected_period" => "Ø¬Ù…ÙŠØ¹ Ø§Ù„ÙØªØ±Ø§Øª",
                "filter_note" => "Ù„Ø§ ØªÙ…Ù„Ùƒ ØµÙ„Ø§Ø­ÙŠØ© Ø§Ù„ÙˆØµÙˆÙ„ Ù„Ø£ÙŠ ÙØ±Ø¹.",
                "selected_branch" => ""
            ]
        ], JSON_UNESCAPED_UNICODE);
        exit();
    ...
    if (!empty($emp_name)) {
        ...
        $params[] = '%' . $emp_name . '%';
        $filter_notes[] = "Ø§Ø³Ù… Ø§Ù„Ù…ÙˆØ¸Ù: " . htmlspecialchars($emp_name);
    }
    ...
    if ($status_filter_value > 0) { // If any status (including "Ù…Ø³ØªÙ…Ø±") is selected
        if ($status_filter_value === 1) { // "Ù…Ø³ØªÙ…Ø±" (Active)
            ...
            $filter_notes[] = "Ø­Ø§Ù„Ø© Ø§Ù„Ù…ÙˆØ¸Ù: Ù…Ø³ØªÙ…Ø±";
            ...
            $filter_notes[] = "Ø­Ø§Ù„Ø© Ø§Ù„Ù…ÙˆØ¸Ù: " . $employee_status_map[$db_status_for_filter];
        }
    }
    ...
    if (!empty($sections)) {
        ...
        $filter_notes[] = "Ø§Ù„Ù‚Ø³Ù…: (ØºÙŠØ± ÙØ¹Ø§Ù„ØŒ Ø§Ù„Ø¹Ù…ÙˆØ¯ ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯ Ø£Ùˆ ØºÙŠØ± Ù…Ø±ØªØ¨Ø· Ø¨Ù€ tblusers)";
    }
    ...
    if (!empty($job_titles)) {
        ...
        $filter_notes[] = "Ø§Ù„Ù…Ø³Ù…Ù‰ Ø§Ù„ÙˆØ¸ÙŠÙÙŠ: (ØºÙŠØ± ÙØ¹Ø§Ù„ØŒ Ø§Ù„Ø¹Ù…ÙˆØ¯ ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯ Ø£Ùˆ ØºÙŠØ± Ù…Ø±ØªØ¨Ø· Ø¨Ù€ tblusers)";
    }
    ...
    if (!empty($grades)) {
        ...
        $filter_notes[] = "Ø§Ù„Ø¯Ø±Ø¬Ø© Ø§Ù„ÙˆØ¸ÙŠÙÙŠØ©: (ØºÙŠØ± ÙØ¹Ø§Ù„ØŒ Ø§Ù„Ø¹Ù…ÙˆØ¯ ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯ Ø£Ùˆ ØºÙŠØ± Ù…Ø±ØªØ¨Ø· Ø¨Ù€ tblusers)";
    }
    ...
    if (!empty($shifts)) {
        ...
        $filter_notes[] = "ÙØªØ±Ø§Øª Ø§Ù„Ø¹Ù…Ù„: (ØºÙŠØ± ÙØ¹Ø§Ù„ØŒ Ø§Ù„Ø¹Ù…ÙˆØ¯ ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯ Ø£Ùˆ ØºÙŠØ± Ù…Ø±ØªØ¨Ø· Ø¨Ù€ tblusers)";
    }
    ...
    if (!empty($groups)) {
        ...
        $filter_notes[] = "Ø§Ù„Ù…Ø¬Ù…ÙˆØ¹Ø© Ø§Ù„ÙˆØ¸ÙŠÙÙŠØ©: " . implode(', ', getNamesByIds($pdo, 'tblgroup', 'Id', 'Name', $groups));
    }
    ...
    if (!empty($branches_filter)) {
        ...
        if (!empty($intersected_branches)) {
            ...
            $filter_notes[] = "Ø§Ù„ÙØ±Ø¹: " . implode(', ', getNamesByIds($pdo, 'branches', 'branch_id', 'branch_name', $intersected_branches));
        } else {
            // If user selected branches they don't have access to, return empty result
             echo json_encode([
                ...
                "results_note" => [
                    "report_time" => date('Y-m-d H:i:s'),
                    "selected_period" => "Ø¬Ù…ÙŠØ¹ Ø§Ù„ÙØªØ±Ø§Øª",
                    "filter_note" => "Ø§Ù„ÙØ±ÙˆØ¹ Ø§Ù„Ù…Ø®ØªØ§Ø±Ø© Ù„Ø§ ØªØªÙˆØ§ÙÙ‚ Ù…Ø¹ ØµÙ„Ø§Ø­ÙŠØ§ØªÙƒ.",
                    "selected_branch" => ""
                ]
            ], JSON_UNESCAPED_UNICODE);
            exit();
        ...
        $filter_notes[] = "Ø§Ù„ÙØ±Ø¹: " . implode(', ', $selected_branches_names_for_note);
    }
    ...
    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
        $data[] = [
            $row['employee_name'],
            $row['branch_name'],
            $row['job_title_name'] ?? 'ØºÙŠØ± Ù…Ø­Ø¯Ø¯',
            $row['section_name'] ?? 'ØºÙŠØ± Ù…Ø­Ø¯Ø¯',
            $row['contract_start_date'] ?? 'ØºÙŠØ± Ù…Ø­Ø¯Ø¯',
            $row['contract_end_date'] ?? 'ØºÙŠØ± Ù…Ø­Ø¯Ø¯',
            number_format($row['salary'] ?? 0, 2)
        ];
    }

    $filter_note_text = !empty($filter_notes) ? "Ø§Ù„ÙÙ„Ø§ØªØ± Ø§Ù„Ù…Ø·Ø¨Ù‚Ø©: " . implode(" | ", $filter_notes) : "Ù„Ø§ ØªÙˆØ¬Ø¯ ÙÙ„Ø§ØªØ± Ù…Ø·Ø¨Ù‚Ø©.";
    ...
    $response = [
        ...
        "results_note" => [
            "report_time" => date('Y-m-d H:i:s'),
            "selected_period" => "Ø¬Ù…ÙŠØ¹ Ø§Ù„ÙØªØ±Ø§Øª", // This can be dynamic if you add date filters
...
case 'dashboard-emp':
    try {
    $uid = (int)($user ?: 0);
    if (!$uid) { echo json_encode(['result' => false, 'msg' => 'ØºÙŠØ± Ù…ØµØ±Ø­'], JSON_UNESCAPED_UNICODE); exit; }
    ...
    $currency = $currencyCode !== '' ? $currencyCode : ($User->currency ?? 'Ø±.Ø³');
...
case 'dashboard-emp-legacy':
    ...
    $currency = $sal ? ($sal['Currency'] ?? 'Ø±.Ø³') : 'Ø±.Ø³';

    // Parse filter to year/month range
    $filterYr = null; $filterMn = null; $filterYrOnly = false;
    if ($filter == 'this_month') {
        $filterYr = (int)date('Y'); $filterMn = (int)date('n');
        ...
        $filterYr = (int)date('Y'); $filterYrOnly = true;
    ...
    echo json_encode([
        ...
        echo json_encode(['result'=>false,'error'=>$e->getMessage(),'salary'=>'0.00','incentive'=>'0.00','benefit'=>'0.00','dections'=>'0.00','total_hour'=>0,'end_salary'=>'0.00','advance'=>'0.00','currency'=>'Ø±.Ø³'], JSON_UNESCAPED_UNICODE);
    }
    exit;
...
case 'emp-chart-legacy':
    ...
    $arabicMonths = ['','ÙŠÙ†Ø§ÙŠØ±','ÙØ¨Ø±Ø§ÙŠØ±','Ù…Ø§Ø±Ø³','Ø£Ø¨Ø±ÙŠÙ„','Ù…Ø§ÙŠÙˆ','ÙŠÙˆÙ†ÙŠÙˆ','ÙŠÙˆÙ„ÙŠÙˆ','Ø£ØºØ³Ø·Ø³','Ø³Ø¨ØªÙ…Ø¨Ø±','Ø£ÙƒØªÙˆØ¨Ø±','Ù†ÙˆÙÙ…Ø¨Ø±','Ø¯ÙŠØ³Ù…Ø¨Ø±'];
...
case 'branches-list':
    ...
    try {
        $sql = "SELECT b.*, CONCAT(u.FirstName, ' ', u.LastName) as CreatorName
                FROM branches b
                LEFT JOIN tblusers u ON b.created_by = u.UserID
    ...
    try {
       ...
       if (!empty($dateRange)) {
        ...
        foreach ($rows as $row) {
            $data[] = [
                'id' => $row['Id'],
                'employee_name' => $row['employee_name'] ?? 'ØºÙŠØ± Ù…Ø¹Ø±ÙˆÙ', // Use the alias from SQL
                'branch_name' => $row['branch_name'] ?? 'ØºÙŠØ± Ù…Ø¹Ø±ÙˆÙ',     // Use the alias
               'from_date' => $row['new_s_date'] ?? '',
            'to_date' => $row['new_e_date'] ?? '',
                'renewal_status' => $row['state'] ?? 0, // Use the alias, default to 0 for unknown
                'creator_info' => ($row['creator_name'] ?? 'ØºÙŠØ± Ù…Ø¹Ø±ÙˆÙ') . '<br>' . ($row['LastUpdate'] ?? '') // Combined for display
            ];
        }
        ...
        echo json_encode([
            ...
            'recordsFiltered' => 0,
            'data' => [],
            'error' => "Ø­Ø¯Ø« Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: " . $e->getMessage() // More user-friendly error
        ], JSON_UNESCAPED_UNICODE);
    }
    exit;
        ...
        */
        ...
        foreach ($rows as $row) {
            $data[] = [
                ...
                'employee_name' => $row['employee_name'] ?? 'ØºÙŠØ± Ù…Ø¹Ø±ÙˆÙ', 
                ...
                'creator_info' => ($row['creator_name'] ?? 'ØºÙŠØ± Ù…Ø¹Ø±ÙˆÙ') . '<br>' . ($row['updated_at'] ?? '') 
            ];
        }
        ...
        echo json_encode([
            ...
            'recordsFiltered' => 0,
            'data' => [],
            'error' => "Ø­Ø¯Ø« Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: " . $e->getMessage()
        ], JSON_UNESCAPED_UNICODE);
    }
    exit;
case 'get-promotion-details':
        ...
        if ($promotionId <= 0) {
            $result = false;
            $msg = 'Ù…Ø¹Ø±Ù Ø§Ù„ØªØ±Ù‚ÙŠØ© ØºÙŠØ± ØµØ§Ù„Ø­';
            break;
        }
        ...
        if ($promotionDetails) {
            ...
            $result = true;
            $msg = 'ØªÙ… Ø¬Ù„Ø¨ ØªÙØ§ØµÙŠÙ„ Ø§Ù„ØªØ±Ù‚ÙŠØ© Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            $result = false;
            $msg = 'Ø·Ù„Ø¨ Ø§Ù„ØªØ±Ù‚ÙŠØ© ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯';
        }
        break;
// ============================================================
...
case 'leaveClassficate-list':
    ...
    try {
        ...
        foreach ($rows as $row) {
            ...
            $typeText = '';
            switch ($row['type'] ?? '') {
                case '1': $typeText = 'Ø§Ø¬Ø§Ø²Ø© Ù…Ø±Ø¶ÙŠØ©'; break;
                case '2': $typeText = 'Ù†ØµÙ ÙØªØ±Ø©'; break;
                case '3': $typeText = 'ÙŠÙˆÙ… ÙƒØ§Ù…Ù„'; break;
                default: $typeText = $row['type'] ?? '';
            }
            // Map state values
            $stateText = ($row['isaccept'] ?? 0) == 1 ? 'Ù†Ø´Ø·' : 'Ù…ÙˆÙ‚Ù';
    ...
    try {
        ...
        foreach ($rows as $row) {
            ...
            if ($updatedDate && $updatedDate !== $createdDate) {
                $creatorParts[] = 'Ø¢Ø®Ø± ØªØ­Ø¯ÙŠØ«: ' . substr($updatedDate, 0, 10);
            }
            ...
            if ($startTime && $endTime) {
                ...
                if ($diff < 0) { $diff += 24; }
                $totalHours = round($diff, 2) . ' Ø³';
            }
            
            $stateBadge = ($row['ShiftState'] ?? 0) == 1
                ? '<span class="badge badge-danger">Ù…ÙˆÙ‚ÙˆÙØ©</span>'
...
case 'insurance-list':
    ...
    try {
        ...
        foreach ($rows as $row) {
            $stateText = '';
            switch ($row['Cstate'] ?? '') {
                case '1': $stateText = 'Ù†Ø´Ø·'; break;
                case '2': $stateText = 'Ù…ÙˆÙ‚Ù'; break;
                default: $stateText = '-';
            }
    ...
    try {
        ...
        foreach ($rows as $row) {
            ...
            switch ((int)($row['FingerprintState'] ?? 0)) {
                case 1: $stateText = 'Ø´ØºØ§Ù„'; break;
                case 2: $stateText = 'Ù…ÙˆÙ‚Ù'; break;
                case 3: $stateText = 'Ø¬Ø§Ø±ÙŠ Ø§Ù„ØµÙŠØ§Ù†Ø©'; break;
                default: $stateText = '-';
            }
...
case 'get-benefit':
    $id = (int)($_GET['id'] ?? 0);
    if ($id <= 0) { $result = false; $msg = 'Ø§Ù„Ù…Ø¹Ø±Ù ØºÙŠØ± ØµØ­ÙŠØ­'; break; }
    ...
    if (!$data) { $result = false; $msg = 'Ø§Ù„ØªØ¹ÙˆÙŠØ¶ ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯'; }
    break;
...
// ============================================================
case 'Benefits-add':
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') { $result = false; $msg = 'Ø·Ø±ÙŠÙ‚Ø© ØºÙŠØ± ØµØ­ÙŠØ­Ø©'; break; }
    ...
    if (empty($name)) { $result = false; $msg = 'Ø§Ø³Ù… Ø§Ù„ØªØ¹ÙˆÙŠØ¶ Ù…Ø·Ù„ÙˆØ¨'; break; }
    if (empty($branchId)) { $result = false; $msg = 'Ø§Ù„ÙØ±Ø¹ Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            ...
            $stmt = $connect_pdo->prepare("UPDATE tblbenefit SET 
                ...
                WHERE Id = ?");
            $stmt->execute([$branchId, $employer, $name, $benefitType, $amountType, $amount, 
                $currency, $reason, $forWhat, $extinsion, $dueDate, $monthly, ($isDraft ? null : 1), $id]);
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø§Ù„ØªØ¹ÙˆÙŠØ¶ Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            // Insert new
            $stmt = $connect_pdo->prepare("INSERT INTO tblbenefit 
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURDATE())");
            $stmt->execute([$branchId, $employer, $name, $benefitType, $amountType, $amount, 
                $currency, $reason, $forWhat, $extinsion, $dueDate, $monthly, ($isDraft ? null : 1), $userId]);
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„ØªØ¹ÙˆÙŠØ¶ Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
    }
    break;
...
case 'Benefits-conform':
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') { $result = false; $msg = 'Ø·Ø±ÙŠÙ‚Ø© ØºÙŠØ± ØµØ­ÙŠØ­Ø©'; break; }
    $id = intval($_POST['id'] ?? 0);
    if ($id <= 0) { $result = false; $msg = 'Ù…Ø¹Ø±Ù ØºÙŠØ± ØµØ§Ù„Ø­'; break; }
    
    try {
        $stmt = $connect_pdo->prepare("UPDATE tblbenefit SET Status = 1 WHERE Id = ?");
        $stmt->execute([$id]);
        $msg = 'ØªÙ… Ø§Ø¹ØªÙ…Ø§Ø¯ Ø§Ù„ØªØ¹ÙˆÙŠØ¶ Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª';
    }
    break;
...
case 'Benefits-remove':
    ...
    try {
        ...
        $stmt->execute([$id]);
        $msg = 'ØªÙ… Ø­Ø°Ù Ø§Ù„ØªØ¹ÙˆÙŠØ¶ Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª';
    }
...
case 'deductions-add':
    ...
    if (empty($name)) { $result = false; $msg = 'Ø§Ø³Ù… Ø§Ù„Ø®ØµÙ… Ù…Ø·Ù„ÙˆØ¨'; break; }
    if (empty($branchId)) { $result = false; $msg = 'Ø§Ù„ÙØ±Ø¹ Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            $stmt = $connect_pdo->prepare("UPDATE tbldeductions SET 
                ...
                WHERE Id = ?");
            $stmt->execute([$branchId, $employer, $name, $amount, $currency, $reason, 
                $forWhat, $extinsion, $dueDate, ($isDraft ? null : 1), $id]);
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø§Ù„Ø®ØµÙ… Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            $stmt = $connect_pdo->prepare("INSERT INTO tbldeductions 
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURDATE())");
            $stmt->execute([$branchId, $employer, $name, $amount, $currency, $reason, 
                $forWhat, $extinsion, $dueDate, ($isDraft ? null : 1), $userId]);
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ø®ØµÙ… Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'deductions-conform':
    ...
    try {
        ...
        $stmt->execute([$id]);
        $msg = 'ØªÙ… Ø§Ø¹ØªÙ…Ø§Ø¯ Ø§Ù„Ø®ØµÙ… Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª';
    }
...
case 'deductions-remove':
    ...
    try {
        ...
        $stmt->execute([$id]);
        $msg = 'ØªÙ… Ø­Ø°Ù Ø§Ù„Ø®ØµÙ… Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª';
    }
...
case 'incentive-add':
    ...
    if (empty($name)) { $result = false; $msg = 'Ø§Ø³Ù… Ø§Ù„Ø­Ø§ÙØ² Ù…Ø·Ù„ÙˆØ¨'; break; }
    if (empty($branchId)) { $result = false; $msg = 'Ø§Ù„ÙØ±Ø¹ Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            $stmt = $connect_pdo->prepare("UPDATE tblincentives SET 
                ...
                Currency = ?, Reason = ?, for_what = ?, extionsion = ?, DueDate = ?, monthly = ?, Status = ?
                WHERE Id = ?");
            $stmt->execute([$branchId, $employer, $name, $incentiveType, $amountType, $amount, 
                $currency, $reason, $forWhat, $extinsion, $dueDate, $monthly, ($isDraft ? null : 1), $id]);
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø§Ù„Ø­Ø§ÙØ² Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            ...
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ø­Ø§ÙØ² Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'incentive-conform':
    ...
    try {
        ...
        $stmt->execute([$id]);
        $msg = 'ØªÙ… Ø§Ø¹ØªÙ…Ø§Ø¯ Ø§Ù„Ø­Ø§ÙØ² Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª';
    }
...
case 'incentive-remove':
    ...
    try {
        ...
        $stmt->execute([$id]);
        $msg = 'ØªÙ… Ø­Ø°Ù Ø§Ù„Ø­Ø§ÙØ² Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª';
    }
...
case 'EmpAdvances-add-add':
    ...
    if (empty($empId)) { $result = false; $msg = 'Ø§Ù„Ù…ÙˆØ¸Ù Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            ...
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø§Ù„Ø³Ù„ÙØ© Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            ...
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ø³Ù„ÙØ© Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'EmpAdvances-conform-admin':
    ...
    try {
        ...
        $stmt->execute([$id]);
        $msg = 'ØªÙ… Ø§Ø¹ØªÙ…Ø§Ø¯ Ø§Ù„Ø³Ù„ÙØ© Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª';
    }
...
case 'EmpAdvances-remove-admin':
    ...
    try {
        ...
        $stmt->execute([$id]);
        $msg = 'ØªÙ… Ø­Ø°Ù Ø§Ù„Ø³Ù„ÙØ© Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª';
    }
    ...
    if (!empty($_FILES['file']['name'])) {
        ...
        if (move_uploaded_file($_FILES['file']['tmp_name'], $targetPath)) {
            try {
                ...
                $msg = 'ØªÙ… Ø±ÙØ¹ Ø§Ù„Ù…Ù„Ù Ø¨Ù†Ø¬Ø§Ø­';
            } catch (PDOException $e) {
                $result = false;
                $msg = 'Ø®Ø·Ø£ ÙÙŠ Ø­ÙØ¸ Ø§Ù„Ù…Ù„Ù';
            }
        } else {
            $result = false;
            $msg = 'ÙØ´Ù„ Ø±ÙØ¹ Ø§Ù„Ù…Ù„Ù';
        }
    } else {
        $result = false;
        $msg = 'Ù„Ù… ÙŠØªÙ… Ø§Ø®ØªÙŠØ§Ø± Ù…Ù„Ù';
    }
    break;
...
case 'section-add':
    ...
    if (empty($name)) { $result = false; $msg = 'Ø§Ø³Ù… Ø§Ù„Ù‚Ø³Ù… Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            ...
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø§Ù„Ù‚Ø³Ù… Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            $stmt = $connect_pdo->prepare("INSERT INTO tblsection (Name, BranchID, ParentID) VALUES (?, ?, ?)");
            $stmt->execute([$name, $branchId, $parentId]);
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ù‚Ø³Ù… Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'jobtitle-add':
    ...
    if (empty($name)) { $result = false; $msg = 'Ø§Ø³Ù… Ø§Ù„Ù…Ø³Ù…Ù‰ Ø§Ù„ÙˆØ¸ÙŠÙÙŠ Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            ...
            $stmt->execute([$name, $branchId, $parentId, $id]);
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø§Ù„Ù…Ø³Ù…Ù‰ Ø§Ù„ÙˆØ¸ÙŠÙÙŠ Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            $stmt = $connect_pdo->prepare("INSERT INTO tbljobtitle (Name, BranchID, ParentID) VALUES (?, ?, ?)");
            $stmt->execute([$name, $branchId, $parentId]);
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ù…Ø³Ù…Ù‰ Ø§Ù„ÙˆØ¸ÙŠÙÙŠ Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'jobgrade-add':
    ...
    if (empty($name)) { $result = false; $msg = 'Ø§Ø³Ù… Ø§Ù„Ø¯Ø±Ø¬Ø© Ø§Ù„ÙˆØ¸ÙŠÙÙŠØ© Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            ...
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø§Ù„Ø¯Ø±Ø¬Ø© Ø§Ù„ÙˆØ¸ÙŠÙÙŠØ© Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            ...
            $data = ['id' => $lastId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ø¯Ø±Ø¬Ø© Ø§Ù„ÙˆØ¸ÙŠÙÙŠØ© Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'groub-add':
    ...
    if (empty($name)) { $result = false; $msg = 'Ø§Ø³Ù… Ø§Ù„Ù…Ø¬Ù…ÙˆØ¹Ø© Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            ...
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø§Ù„Ù…Ø¬Ù…ÙˆØ¹Ø© Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            // Handle multiple branches
            $branches = is_array($branchIds) ? $branchIds : [$branchIds];
            $lastId = 0;
            ...
            $data = ['id' => $lastId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ù…Ø¬Ù…ÙˆØ¹Ø© Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'insurance-add':
    ...
    if (empty($name)) { $result = false; $msg = 'Ø§Ø³Ù… Ø´Ø±ÙƒØ© Ø§Ù„ØªØ£Ù…ÙŠÙ† Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            ...
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø´Ø±ÙƒØ© Ø§Ù„ØªØ£Ù…ÙŠÙ† Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            // Handle multiple branches
            $branches = is_array($branchIds) ? $branchIds : [$branchIds];
            $lastId = 0;
            foreach ($branches as $bid) {
                $stmt = $connect_pdo->prepare("INSERT INTO tbinsurance 
                    (Name, BranchID, NameOfRepresentative, type, Phone, Email, Address, state, Note) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$name, intval($bid), $representative, $type, $phone, $email, $address, $state, $note]);
                $lastId = $connect_pdo->lastInsertId();
            }
            $data = ['id' => $lastId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø´Ø±ÙƒØ© Ø§Ù„ØªØ£Ù…ÙŠÙ† Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'empolyment-add':
    ...
    if (empty($name)) { $result = false; $msg = 'Ø§Ø³Ù… Ù†ÙˆØ¹ Ø§Ù„ØªÙˆØ¸ÙŠÙ Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            ...
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ù†ÙˆØ¹ Ø§Ù„ØªÙˆØ¸ÙŠÙ Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            // Handle multiple branches
            $branches = is_array($branchIds) ? $branchIds : [$branchIds];
            $lastId = 0;
            ...
            $data = ['id' => $lastId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ù†ÙˆØ¹ Ø§Ù„ØªÙˆØ¸ÙŠÙ Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'empolyment-remove':
    ...
    try {
        ...
        $stmt->execute([$id]);
        $msg = 'ØªÙ… Ø­Ø°Ù Ù†ÙˆØ¹ Ø§Ù„ØªÙˆØ¸ÙŠÙ Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª';
    }
...
case 'leaveRequest-add-add':
    ...
    if (empty($empId)) { $result = false; $msg = 'Ø§Ù„Ù…ÙˆØ¸Ù Ù…Ø·Ù„ÙˆØ¨'; break; }
    if (empty($leaveType)) { $result = false; $msg = 'Ù†ÙˆØ¹ Ø§Ù„Ø¥Ø¬Ø§Ø²Ø© Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            $stmt = $connect_pdo->prepare("UPDATE tblleaverequest SET 
                ...
                WHERE Id = ?");
            $stmt->execute([$empId, $branchId, $leaveType, $startDate, $endDate, 
                $dayLeave, $description, $isDraft, ($isDraft ? null : 0), $id]);
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø·Ù„Ø¨ Ø§Ù„Ø¥Ø¬Ø§Ø²Ø© Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            $stmt = $connect_pdo->prepare("INSERT INTO tblleaverequest 
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURDATE())");
            $stmt->execute([$empId, $branchId, $leaveType, $startDate, $endDate, 
                $dayLeave, $description, $isDraft, ($isDraft ? null : 0), $userId]);
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø·Ù„Ø¨ Ø§Ù„Ø¥Ø¬Ø§Ø²Ø© Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'leaveClassficate-add':
    ...
    if (empty($name)) { $result = false; $msg = 'Ø§Ø³Ù… ØªØµÙ†ÙŠÙ Ø§Ù„Ø¥Ø¬Ø§Ø²Ø© Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            ...
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« ØªØµÙ†ÙŠÙ Ø§Ù„Ø¥Ø¬Ø§Ø²Ø© Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            // Handle multiple branches
            $branches = is_array($branchIds) ? $branchIds : [$branchIds];
            $lastId = 0;
            ...
            $data = ['id' => $lastId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© ØªØµÙ†ÙŠÙ Ø§Ù„Ø¥Ø¬Ø§Ø²Ø© Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
    ...
    try {
        if ($id > 0) {
            $stmt = $connect_pdo->prepare("UPDATE tblremewal SET 
                ...
                WHERE Id = ?");
            $stmt->execute([$empId, $branchId, $sectionId, $groupId, $gradeId, $jobtitleId, 
                $typeId, $shiftId, $fingerId, $salary, $currency, $day, $reason, 
                $newStartDate, $newEndDate, $id]);
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« ØªØ¬Ø¯ÙŠØ¯ Ø§Ù„Ø¹Ù‚Ø¯ Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            ...
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© ØªØ¬Ø¯ÙŠØ¯ Ø§Ù„Ø¹Ù‚Ø¯ Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'contractRenewal-conform':
    ...
    try {
        ...
        $stmt->execute([$id]);
        $msg = 'ØªÙ… Ø§Ø¹ØªÙ…Ø§Ø¯ ØªØ¬Ø¯ÙŠØ¯ Ø§Ù„Ø¹Ù‚Ø¯ Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª';
    }
...
case 'contractRenewal-remove':
    ...
    try {
        ...
        $stmt->execute([$id]);
        $msg = 'ØªÙ… Ø­Ø°Ù ØªØ¬Ø¯ÙŠØ¯ Ø§Ù„Ø¹Ù‚Ø¯ Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª';
    }
...
case 'shift-add':
    ...
    if (empty($name)) { $result = false; $msg = 'Ø§Ø³Ù… Ø§Ù„ÙˆØ±Ø¯ÙŠØ© Ù…Ø·Ù„ÙˆØ¨'; break; }
        ...
        if ($id > 0) {
            ...
            if ($checkStmt->rowCount() > 0) {
                $stmt = $connect_pdo->prepare("UPDATE shift_setting SET 
                    ...
                    WHERE shift_id = ?");
                $stmt->execute([$allowedLateMinutes, $enableHalfDay, $halfDayMinutes, $absentMinutes,
                    $allowedEarlyLeave, $enableEarlyHalfDay, $earlyHalfDayMinutes, $earlyAbsentMinutes,
                    $missingCheckoutAction, $lateCheckoutPolicy, $id]);
            } else {
                ...
                $stmt->execute([$id, $allowedLateMinutes, $enableHalfDay, $halfDayMinutes, $absentMinutes,
                    $allowedEarlyLeave, $enableEarlyHalfDay, $earlyHalfDayMinutes, $earlyAbsentMinutes,
                    $missingCheckoutAction, $lateCheckoutPolicy]);
            }
            
            // Delete old schedules and insert new ones
            $connect_pdo->prepare("DELETE FROM shifts_schedule WHERE shift_id = ?")->execute([$id]);
            
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø§Ù„ÙˆØ±Ø¯ÙŠØ© Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            ...
            foreach ($branches as $bid) {
                ...
                $stmt = $connect_pdo->prepare("INSERT INTO shift_setting 
                    ...
                    allowed_early_leave, enable_early_half_day, early_half_day_minutes, early_absent_minutes,
                    missing_checkout_action, late_checkout_policy) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
                $stmt->execute([$shiftId, $allowedLateMinutes, $enableHalfDay, $halfDayMinutes, $absentMinutes,
                    $allowedEarlyLeave, $enableEarlyHalfDay, $earlyHalfDayMinutes, $earlyAbsentMinutes,
                    $missingCheckoutAction, $lateCheckoutPolicy]);
            }
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„ÙˆØ±Ø¯ÙŠØ© Ø¨Ù†Ø¬Ø§Ø­';
        }
...
case 'resignation-add-add':
    ...
    try {
        if ($id > 0) {
            ...
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø·Ù„Ø¨ Ø§Ù„Ø§Ø³ØªÙ‚Ø§Ù„Ø© Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            ...
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø·Ù„Ø¨ Ø§Ù„Ø§Ø³ØªÙ‚Ø§Ù„Ø© Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'dismissal-add':
    ...
    try {
        if ($id > 0) {
            ...
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ù‚Ø±Ø§Ø± Ø§Ù„ÙØµÙ„ Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            $stmt = $connect_pdo->prepare("INSERT INTO tblresignation 
                (UserID, BranchID, DueDate, Reason, type, Draft, Status, CreatedBy, CreatedDate)
                VALUES (?, ?, ?, ?, 2, ?, ?, ?, CURDATE())");
            ...
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ù‚Ø±Ø§Ø± Ø§Ù„ÙØµÙ„ Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'finger-forget-add':
    ...
    try {
        if ($id > 0) {
            ...
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø·Ù„Ø¨ Ø§Ù„Ø¨ØµÙ…Ø© Ø§Ù„Ù…Ù†Ø³ÙŠØ© Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            ...
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø·Ù„Ø¨ Ø§Ù„Ø¨ØµÙ…Ø© Ø§Ù„Ù…Ù†Ø³ÙŠØ© Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'change-manager-emp':
    ...
    try {
        ...
        $msg = 'ØªÙ… ØªØºÙŠÙŠØ± Ø§Ù„Ù…Ø¯ÙŠØ± Ø§Ù„Ù…Ø¨Ø§Ø´Ø± Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
    }
...
case 'employer-add':
    ...
    if (empty($firstName)) { $result = false; $msg = 'Ø§Ø³Ù… Ø§Ù„Ù…ÙˆØ¸Ù Ù…Ø·Ù„ÙˆØ¨'; break; }
    if (empty($branchId)) { $result = false; $msg = 'Ø§Ù„ÙØ±Ø¹ Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        ...
        if ($id > 0) {
            ...
            $stmt = $connect_pdo->prepare("UPDATE tblusers SET 
                ...
                Phone = ?, ohter_phone = ?, Note = ?, IsDisabled = ?, UserGroupID = ?,
                ...
                WHERE UserID = ?");
            $stmt->execute([
                $branchId, $firstName, $secondName, $lastName, $email,
                $phone, $otherPhone, $note, $isDisabled, $userGroupId ?: null,
                $insuranceId, $bankName, $bankAccount, $managerId ?: null, $relatedTo ?: null,
                $sex, $maritalStatus, $address, $healthCondition,
                $idH, $idHStart, $idHEnd,
                $idLicense, $idLicenseStart, $idLicenseEnd,
                $idPassport, $idPassportStart, $idPassportEnd,
                $idHealth, $idHealthStart, $idHealthEnd,
                $createdBy, $id
            ]);
            $empId = $id;
            ...
            $stmt = $connect_pdo->prepare("INSERT INTO tblusers 
                ...
                IsDisabled, UserGroupID, isemp, user_insurance, user_bank_name, user_account_bank, manager, related_to,
                Sex, marital_status, user_address, HealthCondition,
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, CURDATE(), ?)");
            $stmt->execute([
                $branchId, $firstName, $secondName, $lastName, $email, $hashedPass, $phone, $otherPhone, $note,
                $isDisabled, $userGroupId ?: null, $isEmp, $insuranceId, $bankName, $bankAccount, $managerId ?: null, $relatedTo ?: null,
                $sex, $maritalStatus, $address, $healthCondition,
                $idH, $idHStart, $idHEnd, $idLicense, $idLicenseStart, $idLicenseEnd,
                $idPassport, $idPassportStart, $idPassportEnd, $idHealth, $idHealthStart, $idHealthEnd,
                $createdBy
            ]);
            $empId = $connect_pdo->lastInsertId();
        }
        
        // Create/Update contract renewal record
        if ($empId > 0 && ($sectionId || $jobtitleId || $salary)) {
            $stmt = $connect_pdo->prepare("INSERT INTO tblremewal 
                (UserID, BranchID, SectionID, GroupID, GradeID, jobtitleID, TypeID, shiftID, fingerID, 
                Salary, Currency, new_s_date, new_e_date, state, CreatedBy, CreatedDate)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, CURDATE())");
            $stmt->execute([
                $empId, $branchId, $sectionId ?: null, $groupId ?: null, $gradeId ?: null, 
                $jobtitleId ?: null, $typeId ?: null, $shiftId ?: null, $fingerId ?: null,
                $salary, $currency, $contractStart, $contractEnd, $createdBy
            ]);
            $renewalId = $connect_pdo->lastInsertId();
            
            // Update user's lastversion to point to this renewal
        ...
        $connect_pdo->commit();
        $data = ['emp_id' => $empId];
        $msg = $id > 0 ? 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø¨ÙŠØ§Ù†Ø§Øª Ø§Ù„Ù…ÙˆØ¸Ù Ø¨Ù†Ø¬Ø§Ø­' : 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ù…ÙˆØ¸Ù Ø¨Ù†Ø¬Ø§Ø­';
...
case 'UserCertifcate-add':
    ...
    if (empty($userId)) { $result = false; $msg = 'Ø§Ù„Ù…ÙˆØ¸Ù Ù…Ø·Ù„ÙˆØ¨'; break; }
    if (empty($name)) { $result = false; $msg = 'Ø§Ø³Ù… Ø§Ù„Ø´Ù‡Ø§Ø¯Ø© Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            ...
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø§Ù„Ø´Ù‡Ø§Ø¯Ø© Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            ...
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ø´Ù‡Ø§Ø¯Ø© Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'UserExperince-add':
    ...
    if (empty($userId)) { $result = false; $msg = 'Ø§Ù„Ù…ÙˆØ¸Ù Ù…Ø·Ù„ÙˆØ¨'; break; }
    if (empty($company)) { $result = false; $msg = 'Ø§Ø³Ù… Ø§Ù„Ø´Ø±ÙƒØ© Ù…Ø·Ù„ÙˆØ¨'; break; }
    ...
    try {
        if ($id > 0) {
            ...
            $data = ['id' => $id];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø§Ù„Ø®Ø¨Ø±Ø© Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            ...
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„Ø®Ø¨Ø±Ø© Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'save-setting-account-salary':
    ...
    try {
        ...
        $msg = 'ØªÙ… Ø­ÙØ¸ Ø¥Ø¹Ø¯Ø§Ø¯Ø§Øª Ø§Ù„Ø­Ø³Ø§Ø¨Ø§Øª Ø¨Ù†Ø¬Ø§Ø­';
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
    }
    ...
    try {
        if ($editId > 0) {
            ...
            $stmt = $connect_pdo->prepare("UPDATE tblremewal SET 
                ...
                WHERE Id = ?");
            $stmt->execute([
                $branchId, $sectionId ?: null, $groupId ?: null, $gradeId ?: null, $jobtitleId ?: null, $typeId ?: null,
                $shiftIds ?: null, $fingerIds ?: null, $salary, $currency, $contractStart, $contractEnd,
                $dayBeforeGo, $reason, $state, $editId
            ]);
            $data = ['id' => $editId, 'edit' => true];
            $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø§Ù„ØªØ±Ù‚ÙŠØ© Ø¨Ù†Ø¬Ø§Ø­';
        } else {
            ...
            $stmt = $connect_pdo->prepare("INSERT INTO tblremewal 
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 2, ?, CURDATE())");
            $stmt->execute([
                $userId, $branchId, $sectionId ?: null, $groupId ?: null, $gradeId ?: null, 
                $jobtitleId ?: null, $typeId ?: null, $shiftIds ?: null, $fingerIds ?: null,
                $salary, $currency, $contractStart, $contractEnd, $dayBeforeGo, $reason, $createdBy
            ]);
            $newId = $connect_pdo->lastInsertId();
            $data = ['id' => $newId];
            $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø§Ù„ØªØ±Ù‚ÙŠØ© Ø¨Ù†Ø¬Ø§Ø­';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'fingerprint-add':
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') { $result = false; $msg = 'طريقة الطلب غير صحيحة'; break; }
    ...
    if (empty($name)) { $result = false; $msg = 'اسم الجهاز مطلوب'; break; }
    if (empty($branchIds)) { $result = false; $msg = 'الفرع مطلوب'; break; }
...
case 'fingerprint-test':
    ...
    if ($deviceId <= 0) { $result = false; $msg = 'معرف الجهاز مطلوب'; break; }
...
case 'fingerprint-info':
    ...
    try {
        ...
        if (!$device) { $result = false; $msg = 'الجهاز غير موجود'; break; }
...
case 'import-emp-atten':
    ...
    try {
        if (!empty($records) && is_array($records)) {
            ...
            $result = false;
            $msg = 'Ù„Ø§ ØªÙˆØ¬Ø¯ Ø¨ÙŠØ§Ù†Ø§Øª Ù„Ù„Ø§Ø³ØªÙŠØ±Ø§Ø¯';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'empvalidate':
    ...
    try {
        ...
        if (!empty($email)) {
            ...
            if ($stmt->fetch()) {
                $errors['email'] = 'Ø§Ù„Ø¨Ø±ÙŠØ¯ Ø§Ù„Ø¥Ù„ÙƒØªØ±ÙˆÙ†ÙŠ Ù…Ø³ØªØ®Ø¯Ù… Ø¨Ø§Ù„ÙØ¹Ù„';
            }
        }
        ...
        if (!empty($phone)) {
            ...
            if ($stmt->fetch()) {
                $errors['phone'] = 'Ø±Ù‚Ù… Ø§Ù„Ù‡Ø§ØªÙ Ù…Ø³ØªØ®Ø¯Ù… Ø¨Ø§Ù„ÙØ¹Ù„';
            }
        }
        ...
        if (!empty($fingerId)) {
            ...
            if ($stmt->fetch()) {
                $errors['finger_id'] = 'Ø±Ù‚Ù… Ø§Ù„Ø¨ØµÙ…Ø© Ù…Ø³ØªØ®Ø¯Ù… Ø¨Ø§Ù„ÙØ¹Ù„';
            }
        }
        
        if (empty($errors)) {
            $data = ['valid' => true];
            $msg = 'Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª ØµØ§Ù„Ø­Ø©';
        } else {
            $result = false;
            $data = ['valid' => false, 'errors' => $errors];
            $msg = 'ØªÙˆØ¬Ø¯ Ø£Ø®Ø·Ø§Ø¡ ÙÙŠ Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª';
        }
    } catch (PDOException $e) {
        $result = false;
        $msg = 'Ø®Ø·Ø£ ÙÙŠ Ù‚Ø§Ø¹Ø¯Ø© Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª: ' . $e->getMessage();
...
case 'ess-contracts-list':
    if (!$user) { echo json_encode(['result'=>false,'data'=>[]]); exit; }
    try {
        ...
        foreach ($rows as $r) {
            $data[] = [
                'id'   => $r['Id'],
                'type' => 'Ø¹Ù‚Ø¯ Ø¹Ù…Ù„ - ' . ($r['job_title'] ?? ''),
                'date' => ($r['new_s_date'] ?? '') . ' Ø¥Ù„Ù‰ ' . ($r['new_e_date'] ?? '')
            ];
        }
...
case 'ess-contract-print':
    if (!$user) { echo json_encode(['result'=>false,'msg'=>'ØºÙŠØ± Ù…ØµØ±Ø­']); exit; }
    $contractId = intval($_GET['id'] ?? 0);
    if ($contractId <= 0) {
        header('Content-Type: text/html; charset=utf-8');
        echo '<div style="text-align:center;padding:40px;font-family:Arial;">Ù…Ø¹Ø±Ù Ø§Ù„Ø¹Ù‚Ø¯ ØºÙŠØ± ØµØ§Ù„Ø­</div>';
        exit;
    }
    try {
        $stmt = $connect_pdo->prepare("
        ...
        if (!$contract) {
            header('Content-Type: text/html; charset=utf-8');
            echo '<div style="text-align:center;padding:40px;font-family:Arial;">Ø§Ù„Ø¹Ù‚Ø¯ ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯ Ø£Ùˆ ØºÙŠØ± Ù…ØµØ±Ø­ Ù„Ùƒ Ø¨Ø¹Ø±Ø¶Ù‡</div>';
            exit;
        }
        
        ob_end_clean();
        header('Content-Type: text/html; charset=utf-8');
        echo '<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><title>Ø¹Ù‚Ø¯ Ø¹Ù…Ù„</title>
        <style>
        ...
        <div class="header">
            <h1>Ø¹Ù‚Ø¯ Ø¹Ù…Ù„</h1>
            <p>Ø±Ù‚Ù… Ø§Ù„Ø¹Ù‚Ø¯: ' . $contractId . '</p>
        </div>
        
        <div class="section-title">Ø¨ÙŠØ§Ù†Ø§Øª Ø§Ù„Ù…ÙˆØ¸Ù</div>
        <table>
            <tr><th>Ø§Ø³Ù… Ø§Ù„Ù…ÙˆØ¸Ù</th><td>' . htmlspecialchars($contract['empname'] ?? '') . '</td></tr>
            <tr><th>Ø±Ù‚Ù… Ø§Ù„Ù‡ÙˆÙŠØ©</th><td>' . htmlspecialchars($contract['national_id'] ?? '-') . '</td></tr>
            <tr><th>Ø±Ù‚Ù… Ø§Ù„Ø¬ÙˆØ§Ù„</th><td>' . htmlspecialchars($contract['phone'] ?? '-') . '</td></tr>
        </table>
        
        <div class="section-title">Ø¨ÙŠØ§Ù†Ø§Øª Ø§Ù„Ø¹Ù‚Ø¯</div>
        <table>
            <tr><th>Ø§Ù„ÙØ±Ø¹</th><td>' . htmlspecialchars($contract['branch_name'] ?? '-') . '</td></tr>
            <tr><th>Ø§Ù„Ù‚Ø³Ù…</th><td>' . htmlspecialchars($contract['section_name'] ?? '-') . '</td></tr>
            <tr><th>Ø§Ù„Ù…Ø³Ù…Ù‰ Ø§Ù„ÙˆØ¸ÙŠÙÙŠ</th><td>' . htmlspecialchars($contract['job_title'] ?? '-') . '</td></tr>
            <tr><th>Ù†ÙˆØ¹ Ø§Ù„ØªÙˆØ¸ÙŠÙ</th><td>' . htmlspecialchars($contract['employment_type'] ?? '-') . '</td></tr>
            <tr><th>ØªØ§Ø±ÙŠØ® Ø¨Ø¯Ø§ÙŠØ© Ø§Ù„Ø¹Ù‚Ø¯</th><td>' . htmlspecialchars($contract['new_s_date'] ?? $contract['CreatedDate'] ?? '-') . '</td></tr>
            <tr><th>ØªØ§Ø±ÙŠØ® Ù†Ù‡Ø§ÙŠØ© Ø§Ù„Ø¹Ù‚Ø¯</th><td>' . htmlspecialchars($contract['new_e_date'] ?? '-') . '</td></tr>
        </table>
        
        <div class="section-title">Ø§Ù„Ø¨ÙŠØ§Ù†Ø§Øª Ø§Ù„Ù…Ø§Ù„ÙŠØ©</div>
        <table>
            <tr><th>Ø§Ù„Ø±Ø§ØªØ¨ Ø§Ù„Ø£Ø³Ø§Ø³ÙŠ</th><td>' . number_format((float)($contract['Salary'] ?? 0), 2) . ' ' . htmlspecialchars($contract['Currency'] ?? 'SAR') . '</td></tr>
        </table>
        
        <div class="footer">
            <p>ØªÙ… Ø¥Ù†Ø´Ø§Ø¡ Ù‡Ø°Ø§ Ø§Ù„Ù…Ø³ØªÙ†Ø¯ Ø¥Ù„ÙƒØªØ±ÙˆÙ†ÙŠØ§Ù‹ Ù…Ù† Ù†Ø¸Ø§Ù… Vision HR</p>
        ...
        header('Content-Type: text/html; charset=utf-8');
        echo '<div style="text-align:center;padding:40px;">Ø®Ø·Ø£: ' . htmlspecialchars($e->getMessage()) . '</div>';
    }
    exit;
...
case 'ess-salary-print':
    ...
    try {
        ...
        if (!$sal) {
            header('Content-Type: text/html; charset=utf-8');
            echo '<div style="text-align:center;padding:40px;font-family:Arial;">Ù„Ø§ ÙŠÙˆØ¬Ø¯ ÙƒØ´Ù Ø±Ø§ØªØ¨ Ù„Ù‡Ø°Ø§ Ø§Ù„Ø´Ù‡Ø±</div>';
            exit;
        }
        header('Content-Type: text/html; charset=utf-8');
        echo '<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><title>ÙƒØ´Ù Ø±Ø§ØªØ¨</title>
        ...
        <h2 style="text-align:center">ÙƒØ´Ù Ø±Ø§ØªØ¨ - ' . htmlspecialchars($sal['empname']) . '</h2>
        <p style="text-align:center">' . $mn . '/' . $yr . '</p>
        <table>
        <tr><th>Ø§Ù„Ø±Ø§ØªØ¨ Ø§Ù„Ø£Ø³Ø§Ø³ÙŠ</th><td>' . number_format((float)$sal['Salary'],2) . '</td></tr>
        <tr><th>Ø§Ù„Ù…ÙƒØ§ÙØ¢Øª</th><td>' . number_format((float)($sal['incentive']??0),2) . '</td></tr>
        <tr><th>Ø§Ù„ØªØ¹ÙˆÙŠØ¶Ø§Øª</th><td>' . number_format((float)($sal['benefit']??0),2) . '</td></tr>
        <tr><th>Ø§Ù„Ø®ØµÙˆÙ…Ø§Øª</th><td>' . number_format((float)($sal['deductions']??0),2) . '</td></tr>
        <tr><th>Ø§Ù„Ø³Ù„Ù</th><td>' . number_format((float)($sal['advances']??0),2) . '</td></tr>
        <tr><th>ØµØ§ÙÙŠ Ø§Ù„Ø±Ø§ØªØ¨</th><td><strong>' . number_format((float)($sal['net_salary']??$sal['end_salary']??0),2) . '</strong></td></tr>
        </table>
        <script>window.onload=function(){window.print();}</script></body></html>';
    } catch (Exception $e) {
        echo '<div>Ø®Ø·Ø£: ' . $e->getMessage() . '</div>';
    }
    exit;
...
case 'ess-request-certificate':
    if (!$user) { echo json_encode(['result'=>false,'msg'=>'ØºÙŠØ± Ù…ØµØ±Ø­']); exit; }
    $type = $_POST['type'] ?? '';
    echo json_encode(['result'=>true,'msg'=>'ØªÙ… Ø¥Ø±Ø³Ø§Ù„ Ø·Ù„Ø¨Ùƒ Ø¨Ù†Ø¬Ø§Ø­ØŒ Ø³ÙŠØªÙ… Ø§Ù„ØªÙˆØ§ØµÙ„ Ù…Ø¹Ùƒ Ù‚Ø±ÙŠØ¨Ø§Ù‹'], JSON_UNESCAPED_UNICODE);
    exit;
...
case 'ess-report-excel':
    if (!$user) { echo 'ØºÙŠØ± Ù…ØµØ±Ø­'; exit; }
    $from = $_GET['from'] ?? date('Y-01-01');
    $to   = $_GET['to']   ?? date('Y-m-d');
    $type = $_GET['type'] ?? 'attendance';
    header('Content-Type: text/html; charset=utf-8');
    echo '<!DOCTYPE html><html dir="rtl"><head><meta charset="utf-8"><title>ØªÙ‚Ø±ÙŠØ±</title>
    ...
    echo '<h2>ØªÙ‚Ø±ÙŠØ± ' . htmlspecialchars($type) . ' Ù…Ù† ' . $from . ' Ø¥Ù„Ù‰ ' . $to . '</h2>';
    try {
        if ($type === 'attendance') {
            ...
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            echo '<table><tr><th>Ø§Ù„ØªØ§Ø±ÙŠØ®</th><th>ÙˆÙ‚Øª Ø§Ù„Ø­Ø¶ÙˆØ±</th><th>ÙˆÙ‚Øª Ø§Ù„Ø§Ù†ØµØ±Ø§Ù</th></tr>';
            foreach ($rows as $r) { echo '<tr><td>'.$r['Date'].'</td><td>'.($r['check_in']??'-').'</td><td>'.($r['check_out']??'-').'</td></tr>'; }
            echo '</table>';
        } elseif ($type === 'leaves') {
            ...
            $stmt->execute([$user,$from,$to]);
            $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
            echo '<table><tr><th>Ù…Ù†</th><th>Ø¥Ù„Ù‰</th><th>Ø£ÙŠØ§Ù…</th><th>Ø§Ù„Ù†ÙˆØ¹</th><th>Ø§Ù„Ø­Ø§Ù„Ø©</th></tr>';
            foreach ($rows as $r) { $st = $r['status']===null?'Ù‚ÙŠØ¯ Ø§Ù„Ù…Ø±Ø§Ø¬Ø¹Ø©':($r['status']==1?'Ù…Ù‚Ø¨ÙˆÙ„':'Ù…Ø±ÙÙˆØ¶'); echo '<tr><td>'.$r['leave_start_date'].'</td><td>'.$r['leave_end_date'].'</td><td>'.$r['day_leave'].'</td><td>'.$r['leavetype'].'</td><td>'.$st.'</td></tr>'; }
            echo '</table>';
        } else {
            echo '<p>Ù„Ø§ ØªÙˆØ¬Ø¯ Ø¨ÙŠØ§Ù†Ø§Øª Ù„Ù„Ø¹Ø±Ø¶</p>';
        }
    } catch (Exception $e) { echo '<p>Ø®Ø·Ø£: '.$e->getMessage().'</p>'; }
...
case 'ess-attendance':
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && $user) {
        ...
        if ($lat !== null && $lng !== null) {
            $officeLat = (float) ($attSettings['office_lat'] ?? 0);
            $officeLng = (float) ($attSettings['office_lng'] ?? 0);
            $maxRadius = (int) ($attSettings['max_gps_radius_meters'] ?? 0);

            if ($officeLat && $officeLng && $maxRadius > 0) {
                $distance = 6371000 * acos(
                    cos(deg2rad($officeLat)) * cos(deg2rad($lat)) *
                    cos(deg2rad($lng) - deg2rad($officeLng)) +
                    sin(deg2rad($officeLat)) * sin(deg2rad($lat))
                );

                if ($distance > $maxRadius) {
                    $result = false;
                    $msg = 'أنت خارج نطاق موقع العمل المسموح. المسافة الحالية ' . round($distance) . ' متر، والحد الأقصى ' . $maxRadius . ' متر';
                    break;
                }
            }
        }
...
case 'ess-leaves-list':
    if (!$user) { $result = false; $msg = 'ØºÙŠØ± Ù…ØµØ±Ø­'; break; }
    $draw   = (int)($_POST['draw'] ?? 1);
    $start  = (int)($_POST['start'] ?? 0);
    $length = (int)($_POST['length'] ?? 10);
...
case 'ess-leave-submit':
    if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !$user) { $result = false; $msg = 'ØºÙŠØ± Ù…ØµØ±Ø­'; break; }
    ...
    if ($leaveUnit === 'hour') {
        ...
        if (!$leavetype || !$startDate || !$startTime || !$endTime) {
            $result = false;
            $msg = 'ÙŠØ±Ø¬Ù‰ ØªØ¹Ø¨Ø¦Ø© Ø¬Ù…ÙŠØ¹ Ø§Ù„Ø­Ù‚ÙˆÙ„ Ø§Ù„Ù…Ø·Ù„ÙˆØ¨Ø© Ù„Ø¥Ø¬Ø§Ø²Ø© Ø§Ù„Ø³Ø§Ø¹Ø©';
            break;
        }
        ...
        if ($startDate !== $endDate) {
            $result = false;
            $msg = 'Ù„Ø¥Ø¬Ø§Ø²Ø© Ø§Ù„Ø³Ø§Ø¹Ø©ØŒ ÙŠØ¬Ø¨ Ø£Ù† ÙŠÙƒÙˆÙ† ØªØ§Ø±ÙŠØ® Ø§Ù„Ø¨Ø¯Ø§ÙŠØ© ÙˆØ§Ù„Ù†Ù‡Ø§ÙŠØ© Ù‡Ùˆ Ù†ÙØ³Ù‡.';
            break;
        }
        ...
        if ($dayLeave <= 0) {
            $result = false;
            $msg = 'Ø¹Ø¯Ø¯ Ø§Ù„Ø³Ø§Ø¹Ø§Øª ØºÙŠØ± ØµØ§Ù„Ø­. ÙŠØ±Ø¬Ù‰ Ø§Ù„ØªØ­Ù‚Ù‚ Ù…Ù† Ø£ÙˆÙ‚Ø§Øª Ø§Ù„Ø¨Ø¯Ø§ÙŠØ© ÙˆØ§Ù„Ù†Ù‡Ø§ÙŠØ©.';
            break;
        }
        ...
        if (!$leavetype || !$startDate || !$endDate) {
            $result = false;
            $msg = 'ÙŠØ±Ø¬Ù‰ ØªØ¹Ø¨Ø¦Ø© Ø¬Ù…ÙŠØ¹ Ø§Ù„Ø­Ù‚ÙˆÙ„ Ø§Ù„Ù…Ø·Ù„ÙˆØ¨Ø© Ù„Ø¥Ø¬Ø§Ø²Ø© Ø§Ù„ÙŠÙˆÙ… Ø§Ù„ÙƒØ§Ù…Ù„';
            break;
        }
    ...
    $Audit->log(AuditLog::CREATE, AuditLog::ENTITY_LEAVE, $newId,
        'ØªÙ‚Ø¯ÙŠÙ… Ø·Ù„Ø¨ Ø¥Ø¬Ø§Ø²Ø© Ø¬Ø¯ÙŠØ¯',
        null, ['leavetype' => $leavetype, 'start_date' => $startDate, 'end_date' => $endDate, 'start_time' => $startTime, 'end_time' => $endTime, 'duration' => $dayLeave, 'unit' => $leaveUnit]
    );
...
case 'ess-advances-list':
    if (!$user) { $result = false; $msg = 'غير مصرح'; break; }
    $draw   = (int)($_POST['draw'] ?? 1);
    $start  = (int)($_POST['start'] ?? 0);
    $length = (int)($_POST['length'] ?? 10);
    ...
    foreach ($rows as $r) {
        $statusBadge = '';
        if ($r['Status'] == 1) $statusBadge = '<span class="badge badge-success">معتمد</span>';
        elseif ($r['Status'] == 2) $statusBadge = '<span class="badge badge-danger">مرفوض</span>';
...
case 'ess-advance-submit':
    if ($_SERVER['REQUEST_METHOD'] !== 'POST' || !$user) { $result = false; $msg = 'غير مصرح'; break; }
    ...
    $Audit->log(AuditLog::CREATE, AuditLog::ENTITY_ADVANCE, $newId,
        'تقديم طلب سلفة جديد',
        null, ['amount' => $amount, 'type' => $type, 'due_date' => $dueDate]
    );
...
case 'ess-order-submit':
    ...
    $Audit->log(AuditLog::CREATE, AuditLog::ENTITY_ORDER, $newId,
...
case 'ess-attendance-history':
    ...
    foreach ($rows as $r) {
        ...
        if ($m === 'gps') $badgeClass = 'info';
        elseif ($m === 'qr') $badgeClass = 'primary';
        elseif ($m === 'manual') $badgeClass = 'warning text-dark';
...
case 'process-attendance-request':
    ...
    $adminNotes = trim($_POST['notes'] ?? 'ØªÙ…Øª Ø§Ù„Ù…Ø¹Ø§Ù„Ø¬Ø© Ø¨ÙˆØ§Ø³Ø·Ø© Ø§Ù„Ù…Ø³Ø¤ÙˆÙ„');
    ...
    if ($requestId <= 0 || !in_array($decision, ['approve', 'reject'], true) || $adminId <= 0) {
        echo json_encode(['result' => false, 'msg' => 'Ø¨ÙŠØ§Ù†Ø§Øª ØºÙŠØ± ØµØ§Ù„Ø­Ø©'], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    try {
        ...
        if (!$request || ($request['status'] ?? '') !== 'pending') {
            throw new Exception('Ù„Ù… ÙŠØªÙ… Ø§Ù„Ø¹Ø«ÙˆØ± Ø¹Ù„Ù‰ Ø§Ù„Ø·Ù„Ø¨ Ø£Ùˆ ØªÙ…Øª Ù…Ø¹Ø§Ù„Ø¬ØªÙ‡ Ø¨Ø§Ù„ÙØ¹Ù„.');
        }
        ...
        if (!empty($request['emp_id'])) {
            try {
                ...
                $notifService->notify(
                    (int)$request['emp_id'],
                    $decision === 'approve' ? 'ØªÙ… Ø§Ø¹ØªÙ…Ø§Ø¯ Ø·Ù„Ø¨ ØªØ¹Ø¯ÙŠÙ„ Ø§Ù„Ø­Ø¶ÙˆØ±' : 'ØªÙ… Ø±ÙØ¶ Ø·Ù„Ø¨ ØªØ¹Ø¯ÙŠÙ„ Ø§Ù„Ø­Ø¶ÙˆØ±',
                    $decision === 'approve'
        ...
        echo json_encode(['result' => true, 'msg' => 'ØªÙ…Øª Ù…Ø¹Ø§Ù„Ø¬Ø© Ø§Ù„Ø·Ù„Ø¨ Ø¨Ù†Ø¬Ø§Ø­'], JSON_UNESCAPED_UNICODE);
    } catch (Exception $e) {
        if ($connect_pdo->inTransaction()) {
            $connect_pdo->rollBack();
        }
        echo json_encode(['result' => false, 'msg' => 'Ø­Ø¯Ø« Ø®Ø·Ø£: ' . $e->getMessage()], JSON_UNESCAPED_UNICODE);
    }
    exit;
...
case 'process-attendance-request-legacy':
    ...
    if ($requestId <= 0 || !in_array($action, ['approve', 'reject']) || $adminId <= 0) {
        echo json_encode(['result' => false, 'msg' => 'Ø¨ÙŠØ§Ù†Ø§Øª ØºÙŠØ± ØµØ§Ù„Ø­Ø©']);
        exit;
    }
    ...
    try {
        ...
        $connect_pdo->commit();
        echo json_encode(['result' => true, 'msg' => 'ØªÙ…Øª Ù…Ø¹Ø§Ù„Ø¬Ø© Ø§Ù„Ø·Ù„Ø¨ Ø¨Ù†Ø¬Ø§Ø­']);

    } catch (Exception $e) {
        $connect_pdo->rollBack();
        echo json_encode(['result' => false, 'msg' => 'Ø­Ø¯Ø« Ø®Ø·Ø£: ' . $e->getMessage()]);
    }
    exit;
// ============================================================
...
// ============================================================
case 'audit-log-list':
    if (!$User->userIsEmployer()) { $result = false; $msg = 'ØºÙŠØ± Ù…ØµØ±Ø­'; break; }
    ...
    if (!empty($_POST['entity_filter']))  $filters['entity_type'] = $_POST['entity_filter'];
    if (!empty($_POST['user_filter']))    $filters['user_id']     = (int)$_POST['user_filter'];
    if (!empty($_POST['date_from']))      $filters['date_from']   = $_POST['date_from'];
    if (!empty($_POST['date_to']))        $filters['date_to']     = $_POST['date_to'];
    if (!empty($_POST['search']['value'])) $filters['search']     = $_POST['search']['value'];
    ...
    $entityTranslations = [
        ...
        'shift' => 'ÙØªØ±Ø©',
        'section' => 'Ù‚Ø³Ù…',
        'jobtitle' => 'Ù…Ø³Ù…Ù‰ ÙˆØ¸ÙŠÙÙŠ',
        'jobgrade' => 'Ø¯Ø±Ø¬Ø© ÙˆØ¸ÙŠÙÙŠØ©',
        'group' => 'Ù…Ø¬Ù…ÙˆØ¹Ø©',
        'insurance' => 'ØªØ£Ù…ÙŠÙ†',
        'holiday' => 'Ø¥Ø¬Ø§Ø²Ø© Ø±Ø³Ù…ÙŠØ©',
        'fingerprint' => 'Ø¨ØµÙ…Ø©',
        'report' => 'ØªÙ‚Ø±ÙŠØ±',
        'mail' => 'Ø¨Ø±ÙŠØ¯',
        'notification' => 'Ø¥Ø´Ø¹Ø§Ø±',
        'promotion_requests' => 'Ø·Ù„Ø¨Ø§Øª Ø§Ù„ØªØ±Ù‚ÙŠØ©',
        'leave_request' => 'Ø·Ù„Ø¨ Ø¥Ø¬Ø§Ø²Ø©',
        'advance_request' => 'Ø·Ù„Ø¨ Ø³Ù„ÙØ©'
    ];

    $formatted = [];
    foreach ($logResult['data'] as $row) {
        $actionBadges = [
            'login'=>'info', 'logout'=>'secondary', 'create'=>'success', 'update'=>'primary',
            'delete'=>'danger', 'approve'=>'success', 'reject'=>'danger', 'attendance'=>'warning',
            'import'=>'info', 'export'=>'info', 'permission'=>'dark'
        ];
...
case 'audit-log-detail':
    ...
    if (!$logId) { $result = false; $msg = 'Ù…Ø¹Ø±Ù ØºÙŠØ± ØµØ§Ù„Ø­'; break; }

    $stmt = $connect_pdo->prepare("SELECT * FROM audit_log WHERE id = :id LIMIT 1");
    $stmt->execute([':id' => $logId]);
    $data = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$data) { $result = false; $msg = 'Ø§Ù„Ø³Ø¬Ù„ ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯'; }
    break;
...
case 'attendance-method-stats':
    if (!$User->userIsEmployer()) { $result = false; $msg = 'غير مصرح'; break; }
...
case 'revoke-qr-token':
    ...
    $token = $_POST['token'] ?? '';
    if (empty($token)) { $result = false; $msg = 'الرمز مطلوب'; break; }
...
case 'fingerprint-test-connection':
    ...
    if (empty($deviceIp)) { $result = false; $msg = 'عنوان IP للجهاز مطلوب'; break; }
...
case 'get-leave-policy':
    ...
    if (!$data) { $result = false; $msg = 'Ø§Ù„Ø³ÙŠØ§Ø³Ø© ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯Ø©'; }
    break;
...
case 'save-leave-policy':
    ...
    if ($savedId) {
        $data = ['id' => $savedId];
        $msg = 'ØªÙ… Ø­ÙØ¸ Ø§Ù„Ø³ÙŠØ§Ø³Ø© Ø¨Ù†Ø¬Ø§Ø­';
        $Audit->log(AuditLog::UPDATE, 'leave_policies', $savedId, $id ? 'ØªØ¹Ø¯ÙŠÙ„ Ø³ÙŠØ§Ø³Ø© Ø¥Ø¬Ø§Ø²Ø§Øª' : 'Ø¥Ø¶Ø§ÙØ© Ø³ÙŠØ§Ø³Ø© Ø¥Ø¬Ø§Ø²Ø§Øª', null, $_POST);
    } else {
        $result = false;
        $msg = 'ÙØ´Ù„ Ø­ÙØ¸ Ø§Ù„Ø³ÙŠØ§Ø³Ø©';
    }
    break;
...
case 'toggle-leave-policy':
    ...
    $msg = $active ? 'ØªÙ… ØªÙØ¹ÙŠÙ„ Ø§Ù„Ø³ÙŠØ§Ø³Ø©' : 'ØªÙ… ØªØ¹Ø·ÙŠÙ„ Ø§Ù„Ø³ÙŠØ§Ø³Ø©';
    break;
...
case 'process-year-end':
    if (!$User->userIsAdmin()) { $result = false; $msg = 'ØºÙŠØ± Ù…ØµØ±Ø­'; break; }
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') { $result = false; $msg = 'Ø·Ø±ÙŠÙ‚Ø© ØºÙŠØ± ØµØ­ÙŠØ­Ø©'; break; }
    ...
    $data = $results;
    $msg = 'ØªÙ…Øª Ù…Ø¹Ø§Ù„Ø¬Ø© Ù†Ù‡Ø§ÙŠØ© Ø§Ù„Ø³Ù†Ø©';
...
case 'get-employee-leave-balance':
    ...
    if ($empId != $user && !$User->userIsEmployer()) { $result = false; $msg = 'ØºÙŠØ± Ù…ØµØ±Ø­'; break; }
    
    require_once __DIR__ . '/../classes/LeavePolicyManager.php';
    $manager = new LeavePolicyManager($connect_pdo);
    $data = $manager->getBalanceSummary($empId);
    if (!$data) { $result = false; $msg = 'Ù„Ø§ ÙŠÙˆØ¬Ø¯ Ø±ØµÙŠØ¯'; }
    break;
...
case 'get-violation':
    ...
    if (!$data) { $result = false; $msg = 'Ø§Ù„Ù…Ø®Ø§Ù„ÙØ© ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯Ø©'; }
    break;
...
case 'record-violation':
    ...
    if ($res['success']) {
        $data = $res;
        $msg = $res['message'];
        $Audit->log(AuditLog::CREATE, 'employee_violations', $res['violation_id'], 'ØªØ³Ø¬ÙŠÙ„ Ù…Ø®Ø§Ù„ÙØ©', null, $_POST);
    } else {
        $result = false;
        $msg = $res['message'];
    }
...
case 'update-violation-status':
    ...
    $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø­Ø§Ù„Ø© Ø§Ù„Ù…Ø®Ø§Ù„ÙØ©';
...
case 'save-violation-type':
    ...
    $data = ['id' => $savedId];
    $msg = 'ØªÙ… Ø­ÙØ¸ Ù†ÙˆØ¹ Ø§Ù„Ù…Ø®Ø§Ù„ÙØ©';
    break;
...
case 'create-promotion-request':
    ...
    if ($res['success']) {
        $data = $res;
        $msg = $res['message'];
        $Audit->log(AuditLog::CREATE, 'promotion_requests', $res['request_id'], 'Ø¥Ù†Ø´Ø§Ø¡ Ø·Ù„Ø¨ ØªØ±Ù‚ÙŠØ©', null, $_POST);
    } else {
        $result = false;
        $msg = $res['message'];
    }
...
case 'approve-promotion':
    ...
    if ($res['success']) {
        $data = $res;
        $msg = $res['message'];
        $Audit->log(AuditLog::UPDATE, 'promotion_requests', $id, 'Ø§Ù„Ù…ÙˆØ§ÙÙ‚Ø© Ø¹Ù„Ù‰ ØªØ±Ù‚ÙŠØ©', null, $_POST);
    } else {
        $result = false;
        $msg = $res['message'];
        $data = $res;
...
case 'save-promotion-policy':
    ...
    $savedId = $manager->savePolicy($_POST, $id);
    $data = ['id' => $savedId];
    $msg = 'ØªÙ… Ø­ÙØ¸ Ø³ÙŠØ§Ø³Ø© Ø§Ù„ØªØ±Ù‚ÙŠØ§Øª';
    break;
...
case 'get-employee-presence':
    ...
    if (!$data) { $result = false; $msg = 'Ø§Ù„Ù…ÙˆØ¸Ù ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯'; }
    break;
...
case 'get-org-node':
    ...
    if (!$data) {
        $result = false;
        $msg = 'Ø§Ù„Ø¹Ù†ØµØ± ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯';
    }
    break;
...
case 'move-org-node':
    ...
    if (!$nodeId) {
        $result = false;
        $msg = 'Ù…Ø¹Ø±Ù Ø§Ù„Ø¹Ù‚Ø¯Ø© Ù…Ø·Ù„ÙˆØ¨';
        break;
    }
    ...
    if ($nodeId === $parentId) {
        $result = false;
        $msg = 'Ù„Ø§ ÙŠÙ…ÙƒÙ† Ù†Ù‚Ù„ Ø§Ù„Ø¹Ù‚Ø¯Ø© Ø¥Ù„Ù‰ Ù†ÙØ³Ù‡Ø§';
        break;
    }
    
    $res = $manager->updateNode($nodeId, ['parent_id' => $parentId]);
    if ($res) {
        $msg = 'ØªÙ… Ù†Ù‚Ù„ Ø§Ù„Ø¹Ù‚Ø¯Ø© Ø¨Ù†Ø¬Ø§Ø­';
    } else {
        $result = false;
        $msg = 'ÙØ´Ù„ Ù†Ù‚Ù„ Ø§Ù„Ø¹Ù‚Ø¯Ø©';
    }
    break;
...
case 'process-approval':
    ...
    if (!$instanceId || !in_array($action, ['approved', 'rejected'])) {
        $result = false;
        $msg = 'Ø¨ÙŠØ§Ù†Ø§Øª ØºÙŠØ± ØµØ­ÙŠØ­Ø©';
        break;
    }
...
case 'save-workflow-config': // This action is specifically for toggling is_active
    ...
    if ($id) {
        ...
        $result = true;
        $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø­Ø§Ù„Ø© Ø³ÙŠØ± Ø§Ù„Ø¹Ù…Ù„.';
    } else {
        $result = false;
        $msg = 'Ù…Ø¹Ø±Ù Ø³ÙŠØ± Ø§Ù„Ø¹Ù…Ù„ ØºÙŠØ± ØµØ§Ù„Ø­.';
    }
    break;
...
case 'add-workflow':
    ...
    if (empty($name_ar) || empty($entity_type)) {
        $result = false;
        $msg = 'Ø§Ù„Ø±Ø¬Ø§Ø¡ ØªÙˆÙÙŠØ± Ø§Ø³Ù… Ø³ÙŠØ± Ø§Ù„Ø¹Ù…Ù„ ÙˆÙ†ÙˆØ¹ Ø§Ù„ÙƒÙŠØ§Ù†.';
        break;
    }
  // --- NEW CHECK FOR DUPLICATE entity_type ---
    ...
    if ($stmtCheck->fetchColumn() > 0) {
        $result = false;
        $msg = 'ÙŠÙˆØ¬Ø¯ Ø¨Ø§Ù„ÙØ¹Ù„ Ø³ÙŠØ± Ø¹Ù…Ù„ Ù„Ù‡Ø°Ø§ Ø§Ù„Ù†ÙˆØ¹ Ù…Ù† Ø§Ù„ÙƒÙŠØ§Ù†Ø§Øª. ÙŠØ±Ø¬Ù‰ ØªØ¹Ø¯ÙŠÙ„ Ø§Ù„Ø³ÙŠØ± Ø§Ù„Ø¹Ù…Ù„ Ø§Ù„Ù…ÙˆØ¬ÙˆØ¯ Ø¨Ø¯Ù„Ø§Ù‹ Ù…Ù† Ø¥Ø¶Ø§ÙØ© ÙˆØ§Ø­Ø¯ Ø¬Ø¯ÙŠØ¯.';
    ...
    try {
        ...
        foreach ($steps as $step) {
            ...
            if (empty($step_name_ar) || empty($approver_type) || $step_order <= 0) {
                throw new Exception('Ø¨ÙŠØ§Ù†Ø§Øª Ø®Ø·ÙˆØ© Ø³ÙŠØ± Ø§Ù„Ø¹Ù…Ù„ ØºÙŠØ± Ù…ÙƒØªÙ…Ù„Ø© Ø£Ùˆ ØºÙŠØ± ØµØ§Ù„Ø­Ø©.');
            }

            // Validate specific user/role requirements
            if ($approver_type === 'specific_user' && $approver_id === null) {
                throw new Exception('Ø§Ù„Ø±Ø¬Ø§Ø¡ Ø§Ø®ØªÙŠØ§Ø± Ù…Ø³ØªØ®Ø¯Ù… Ù…Ø­Ø¯Ø¯ Ù„Ø®Ø·ÙˆØ© Ø§Ù„Ù…ÙˆØ§ÙÙ‚.');
            }
            if ($approver_type === 'role' && $approver_role === null) {
                throw new Exception('Ø§Ù„Ø±Ø¬Ø§Ø¡ Ø§Ø®ØªÙŠØ§Ø± Ø¯ÙˆØ± Ù…Ø­Ø¯Ø¯ Ù„Ø®Ø·ÙˆØ© Ø§Ù„Ù…ÙˆØ§ÙÙ‚.');
            }
        ...
        $connect_pdo->commit();
        $result = true;
        $msg = 'ØªÙ… Ø¥Ø¶Ø§ÙØ© Ø³ÙŠØ± Ø§Ù„Ø¹Ù…Ù„ Ø¨Ù†Ø¬Ø§Ø­.';

    } catch (Exception $e) {
        $connect_pdo->rollBack();
        $result = false;
        $msg = 'ÙØ´Ù„ Ø¥Ø¶Ø§ÙØ© Ø³ÙŠØ± Ø§Ù„Ø¹Ù…Ù„: ' . $e->getMessage();
    }
    break;

case 'get-workflow-details':
    if (!$User->userIsAdmin() && !$User->userIsEmployer()) { $result = false; $msg = 'ØºÙŠØ± Ù…ØµØ±Ø­'; break; }
    if ($_SERVER['REQUEST_METHOD'] !== 'GET') { $result = false; $msg = 'Ø·Ø±ÙŠÙ‚Ø© ØºÙŠØ± ØµØ­ÙŠØ­Ø©'; break; }
    ...
    try {
        ...
        if (!$workflow_config) {
            $result = false;
            $msg = 'Ù„Ù… ÙŠØªÙ… Ø§Ù„Ø¹Ø«ÙˆØ± Ø¹Ù„Ù‰ Ø³ÙŠØ± Ø§Ù„Ø¹Ù…Ù„.';
            break;
        }
        ...
        $result = false;
        $msg = 'ÙØ´Ù„ Ø¬Ù„Ø¨ ØªÙØ§ØµÙŠÙ„ Ø³ÙŠØ± Ø§Ù„Ø¹Ù…Ù„: ' . $e->getMessage();
    }
    break;
...
case 'update-workflow':
    ...
    if (!$workflow_id || empty($name_ar) || empty($entity_type)) {
        $result = false;
        $msg = 'Ø§Ù„Ø±Ø¬Ø§Ø¡ ØªÙˆÙÙŠØ± Ù…Ø¹Ø±Ù Ø³ÙŠØ± Ø§Ù„Ø¹Ù…Ù„ØŒ Ø§Ù„Ø§Ø³Ù…ØŒ ÙˆÙ†ÙˆØ¹ Ø§Ù„ÙƒÙŠØ§Ù†.';
        break;
    }
    ...
    try {
        ...
        $connect_pdo->commit();
        $result = true;
        $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ« Ø³ÙŠØ± Ø§Ù„Ø¹Ù…Ù„ Ø¨Ù†Ø¬Ø§Ø­.';

    } catch (Exception $e) {
        $connect_pdo->rollBack();
        $result = false;
        $msg = 'ÙØ´Ù„ ØªØ­Ø¯ÙŠØ« Ø³ÙŠØ± Ø§Ù„Ø¹Ù…Ù„: ' . $e->getMessage();
    }
    break;
...
case 'create-evaluation-period':
    ...
    if (empty($name_ar) || empty($start_date) || empty($end_date)) {
        $result = false;
        $msg = 'ÙŠØ±Ø¬Ù‰ ØªØ¹Ø¨Ø¦Ø© Ø¬Ù…ÙŠØ¹ Ø§Ù„Ø­Ù‚ÙˆÙ„ Ø§Ù„Ù…Ø·Ù„ÙˆØ¨Ø© (Ø§Ù„Ø§Ø³Ù…ØŒ ØªØ§Ø±ÙŠØ® Ø§Ù„Ø¨Ø¯Ø¡ØŒ ØªØ§Ø±ÙŠØ® Ø§Ù„Ø§Ù†ØªÙ‡Ø§Ø¡).';
        break;
    }
    ...
    try {
        ...
        if ($periodId) {
            ...
            $data = ['id' => $periodId];
            $msg = 'ØªÙ… Ø¥Ù†Ø´Ø§Ø¡ ÙØªØ±Ø© Ø§Ù„ØªÙ‚ÙŠÙŠÙ… Ø¨Ù†Ø¬Ø§Ø­.';
        } else {
            $result = false;
            $msg = 'ÙØ´Ù„ ÙÙŠ Ø¥Ù†Ø´Ø§Ø¡ ÙØªØ±Ø© Ø§Ù„ØªÙ‚ÙŠÙŠÙ….';
        }
    } catch (Exception $e) {
        $result = false;
        $msg = 'Ø­Ø¯Ø« Ø®Ø·Ø£: ' . $e->getMessage();
...
case 'save-evaluation-criteria':
    ...
    $data = ['id' => $criteriaId];
    $msg = 'ØªÙ… Ø­ÙØ¸ Ù…Ø¹ÙŠØ§Ø± Ø§Ù„ØªÙ‚ÙŠÙŠÙ…';
    break;
case 'acknowledgeEvaluation':
    ...
    $msg = $result ? 'ØªÙ… Ø¥Ù‚Ø±Ø§Ø± Ø§Ù„ØªÙ‚ÙŠÙŠÙ… Ø¨Ù†Ø¬Ø§Ø­' : 'ÙØ´Ù„ ÙÙŠ Ø¥Ù‚Ø±Ø§Ø± Ø§Ù„ØªÙ‚ÙŠÙŠÙ…';
    break;
case 'create-evaluation':
    if (!$User->userIsEmployer() && !$User->userIsAdmin()) { $result = false; $msg = 'ØºÙŠØ± Ù…ØµØ±Ø­'; break; }
    if ($_SERVER['REQUEST_METHOD'] !== 'POST') { $result = false; $msg = 'Ø·Ø±ÙŠÙ‚Ø© ØºÙŠØ± ØµØ­ÙŠØ­Ø©'; break; }
    ...
    if ($evaluatorId === 0) {
        $result = false;
        $msg = 'Ù…Ø¹Ø±Ù Ø§Ù„Ù…Ù‚ÙŠÙ… ØºÙŠØ± Ù…ØªÙˆÙØ±. ÙŠØ±Ø¬Ù‰ ØªØ³Ø¬ÙŠÙ„ Ø§Ù„Ø¯Ø®ÙˆÙ„.';
        break;
    }
    ...
    try {
        ...
        $msg = $res['message'] ?? ($res['success'] ? 'ØªÙ… Ø¥Ù†Ø´Ø§Ø¡ Ø§Ù„ØªÙ‚ÙŠÙŠÙ… Ø¨Ù†Ø¬Ø§Ø­.' : 'ÙØ´Ù„ ÙÙŠ Ø¥Ù†Ø´Ø§Ø¡ Ø§Ù„ØªÙ‚ÙŠÙŠÙ….');

    } catch (Exception $e) {
        $result = false;
        $msg = 'Ø­Ø¯Ø« Ø®Ø·Ø£ ØºÙŠØ± Ù…ØªÙˆÙ‚Ø¹: ' . $e->getMessage();
...
case 'submit-evaluation':
    ...
    $msg = $result ? 'ØªÙ… ØªÙ‚Ø¯ÙŠÙ… Ø§Ù„ØªÙ‚ÙŠÙŠÙ… Ù„Ù„Ù…Ø±Ø§Ø¬Ø¹Ø©' : 'ÙØ´Ù„ ÙÙŠ ØªÙ‚Ø¯ÙŠÙ… Ø§Ù„ØªÙ‚ÙŠÙŠÙ…';
    break;
...
case 'approve-evaluation':
    ...
    $msg = $result ? 'ØªÙ… Ø§Ø¹ØªÙ…Ø§Ø¯ Ø§Ù„ØªÙ‚ÙŠÙŠÙ…' : 'ÙØ´Ù„ ÙÙŠ Ø§Ø¹ØªÙ…Ø§Ø¯ Ø§Ù„ØªÙ‚ÙŠÙŠÙ…';
    break;
...
case 'create-reward':
    ...
    $data = ['id' => $rewardId];
    $msg = 'ØªÙ… Ø¥Ù†Ø´Ø§Ø¡ Ø§Ù„Ù…ÙƒØ§ÙØ£Ø©';
    break;
...
case 'approve-reward':
    ...
    $msg = $result ? 'ØªÙ… Ø§Ø¹ØªÙ…Ø§Ø¯ Ø§Ù„Ù…ÙƒØ§ÙØ£Ø©' : 'ÙØ´Ù„ ÙÙŠ Ø§Ø¹ØªÙ…Ø§Ø¯ Ø§Ù„Ù…ÙƒØ§ÙØ£Ø©';
    break;
...
case 'deliver-reward':
    ...
    $msg = $result ? 'ØªÙ… ØªØ³Ù„ÙŠÙ… Ø§Ù„Ù…ÙƒØ§ÙØ£Ø©' : 'ÙØ´Ù„ ÙÙŠ ØªØ³Ù„ÙŠÙ… Ø§Ù„Ù…ÙƒØ§ÙØ£Ø©';
    break;
...
case 'save-salary-range':
    ...
    $data = ['id' => $rangeId];
    $msg = 'ØªÙ… Ø­ÙØ¸ Ù†Ø·Ø§Ù‚ Ø§Ù„Ø±Ø§ØªØ¨';
    break;
...
case 'get-notifications':
    ...
    if (!$userId) { $result = false; $msg = 'ØºÙŠØ± Ù…ØµØ±Ø­'; break; }
    require_once __DIR__ . '/../classes/NotificationService.php';
...
case 'mark-all-notifications-read':
    ...
    $msg = 'ØªÙ… ØªØ­Ø¯ÙŠØ¯ Ø¬Ù…ÙŠØ¹ Ø§Ù„Ø¥Ø´Ø¹Ø§Ø±Ø§Øª ÙƒÙ…Ù‚Ø±ÙˆØ¡Ø©';
    break;
...
case 'direct-approve':
    ...
    if (!$type || !$id) { $result = false; $msg = 'البيانات غير صحيحة'; break; }
...
case 'get-order-details':
    $id = intval($_GET['id'] ?? 0);
    if (!$id) { $result = false; $msg = 'Ù…Ø¹Ø±Ù ØºÙŠØ± ØµØ§Ù„Ø­'; break; }
    ...
    if ($order) {
        $data = [
            'Ø§Ù„Ù…ÙˆØ¶ÙˆØ¹' => $order['title'],
            'Ø§Ù„ØªÙØ§ØµÙŠÙ„' => $order['description'],
            'Ù…Ù‚Ø¯Ù… Ø§Ù„Ø·Ù„Ø¨' => $order['FirstName'] . ' ' . $order['LastName'],
            'Ø§Ù„ÙØ±Ø¹' => $order['branch_name'],
            'ØªØ§Ø±ÙŠØ® Ø§Ù„Ø·Ù„Ø¨' => $order['CreatedDate'],
            'Ø§Ù„Ø­Ø§Ù„Ø©' => $order['Status'] === null ? 'Ù…Ø¹Ù„Ù‚' : ($order['Status'] == 1 ? 'Ù…ÙˆØ§ÙÙ‚' : 'Ù…Ø±ÙÙˆØ¶')
        ];
    } else {
        $result = false;
        $msg = 'Ø§Ù„Ø·Ù„Ø¨ ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯';
    }
    break;
...
case 'Benefits-remove-modal':
    ...
    header('Content-Type: text/html; charset=utf-8');
    $id = intval($_GET['id'] ?? 0);
    if ($id <= 0) { echo '<div class="alert alert-danger">Ù…Ø¹Ø±Ù ØºÙŠØ± ØµØ§Ù„Ø­</div>'; exit; }
    ...
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$row) { echo '<div class="alert alert-danger">Ø§Ù„ØªØ¹ÙˆÙŠØ¶ ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯</div>'; exit; }
    echo '<div class="alert alert-warning"><h5>ØªØ£ÙƒÙŠØ¯ Ø§Ù„Ø­Ø°Ù</h5><p>Ù‡Ù„ Ø£Ù†Øª Ù…ØªØ£ÙƒØ¯ Ù…Ù† Ø­Ø°Ù Ø§Ù„ØªØ¹ÙˆÙŠØ¶: <strong>' . htmlspecialchars($row['name'] ?? '') . '</strong>ØŸ</p><p class="text-muted">Ø§Ù„ÙØ±Ø¹: ' . htmlspecialchars($row['branch_name'] ?? '-') . '</p><p class="text-danger">Ù‡Ø°Ø§ Ø§Ù„Ø¥Ø¬Ø±Ø§Ø¡ Ù„Ø§ ÙŠÙ…ÙƒÙ† Ø§Ù„ØªØ±Ø§Ø¬Ø¹ Ø¹Ù†Ù‡.</p></div>';
    echo '<form id="deleteForm"><input type="hidden" name="id" value="' . $id . '"><div class="text-center"><button type="submit" class="btn btn-danger"><i class="fas fa-trash"></i> Ø­Ø°Ù</button> <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="fas fa-times"></i> Ø¥Ù„ØºØ§Ø¡</button></div></form>';
    echo '<script>$("#deleteForm").on("submit",function(e){e.preventDefault();$.ajax({url:"hr-app/index.php?action=Benefits-remove",type:"POST",data:$(this).serialize(),dataType:"json",success:function(r){if(r.result){$("#modal_default").modal("hide");toastr.success(r.msg);if($.fn.DataTable.isDataTable("#data_tb")){$("#data_tb").DataTable().ajax.reload();}}else{toastr.error(r.msg);}},error:function(){toastr.error("Ø­Ø¯Ø« Ø®Ø·Ø£ Ø£Ø«Ù†Ø§Ø¡ Ø§Ù„Ø­Ø°Ù");}});});</script>';
    exit;
...
case 'deductions-remove-modal':
    ...
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$row) { echo '<div class="alert alert-danger">Ø§Ù„Ø®ØµÙ… ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯</div>'; exit; }
    echo '<div class="alert alert-warning"><h5>ØªØ£ÙƒÙŠØ¯ Ø§Ù„Ø­Ø°Ù</h5><p>Ù‡Ù„ Ø£Ù†Øª Ù…ØªØ£ÙƒØ¯ Ù…Ù† Ø­Ø°Ù Ø§Ù„Ø®ØµÙ…: <strong>' . htmlspecialchars($row['name'] ?? '') . '</strong>ØŸ</p><p class="text-muted">Ø§Ù„ÙØ±Ø¹: ' . htmlspecialchars($row['branch_name'] ?? '-') . '</p><p class="text-danger">Ù‡Ø°Ø§ Ø§Ù„Ø¥Ø¬Ø±Ø§Ø¡ Ù„Ø§ ÙŠÙ…ÙƒÙ† Ø§Ù„ØªØ±Ø§Ø¬Ø¹ Ø¹Ù†Ù‡.</p></div>';
    echo '<form id="deleteForm"><input type="hidden" name="id" value="' . $id . '"><div class="text-center"><button type="submit" class="btn btn-danger"><i class="fas fa-trash"></i> Ø­Ø°Ù</button> <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="fas fa-times"></i> Ø¥Ù„ØºØ§Ø¡</button></div></form>';
...
case 'incentive-remove-modal':
    ...
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$row) { echo '<div class="alert alert-danger">Ø§Ù„Ù…ÙƒØ§ÙØ£Ø© ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯Ø©</div>'; exit; }
    echo '<div class="alert alert-warning"><h5>ØªØ£ÙƒÙŠØ¯ Ø§Ù„Ø­Ø°Ù</h5><p>Ù‡Ù„ Ø£Ù†Øª Ù…ØªØ£ÙƒØ¯ Ù…Ù† Ø­Ø°Ù Ø§Ù„Ù…ÙƒØ§ÙØ£Ø©: <strong>' . htmlspecialchars($row['name'] ?? '') . '</strong>ØŸ</p><p class="text-muted">Ø§Ù„ÙØ±Ø¹: ' . htmlspecialchars($row['branch_name'] ?? '-') . '</p><p class="text-danger">Ù‡Ø°Ø§ Ø§Ù„Ø¥Ø¬Ø±Ø§Ø¡ Ù„Ø§ ÙŠÙ…ÙƒÙ† Ø§Ù„ØªØ±Ø§Ø¬Ø¹ Ø¹Ù†Ù‡.</p></div>';
    echo '<form id="deleteForm"><input type="hidden" name="id" value="' . $id . '"><div class="text-center"><button type="submit" class="btn btn-danger"><i class="fas fa-trash"></i> Ø­Ø°Ù</button> <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="fas fa-times"></i> Ø¥Ù„ØºØ§Ø¡</button></div></form>';
...
case 'EmpAdvances-remove-modal':
    ...
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$row) { echo '<div class="alert alert-danger">Ø§Ù„Ø³Ù„ÙØ© ØºÙŠØ± Ù…ÙˆØ¬ÙˆØ¯Ø©</div>'; exit; }
    echo '<div class="alert alert-warning"><h5>ØªØ£ÙƒÙŠØ¯ Ø§Ù„Ø­Ø°Ù</h5><p>Ù‡Ù„ Ø£Ù†Øª Ù…ØªØ£ÙƒØ¯ Ù…Ù† Ø­Ø°Ù Ø³Ù„ÙØ© Ø§Ù„Ù…ÙˆØ¸Ù: <strong>' . htmlspecialchars($row['EmpName'] ?? '') . '</strong>ØŸ</p><p class="text-muted">Ø§Ù„Ù…Ø¨Ù„Øº: ' . htmlspecialchars($row['Amount'] ?? '0') . '</p><p class="text-danger">Ù‡Ø°Ø§ Ø§Ù„Ø¥Ø¬Ø±Ø§Ø¡ Ù„Ø§ ÙŠÙ…ÙƒÙ† Ø§Ù„ØªØ±Ø§Ø¬Ø¹ Ø¹Ù†Ù‡.</p></div>';
    echo '<form id="deleteForm"><input type="hidden" name="id" value="' . $id . '"><div class="text-center"><button type="submit" class="btn btn-danger"><i class="fas fa-trash"></i> Ø­Ø°Ù</button> <button type="button" class="btn btn-secondary" data-dismiss="modal"><i class="fas fa-times"></i> Ø¥Ù„ØºØ§Ø¡</button></div></form>';
