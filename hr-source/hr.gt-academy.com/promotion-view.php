...
.bdanger{ border-right:0.3rem solid red; display:block;}
	...
	<?php endif;?>
			<div class="row">
                ...
                <?php
                if ($User->isAllowedPerm(['تعديل ترقية'], $appid)) {   ?>
								<a href="promotion-add?edit=<?=$row['Id']?>" class="btn btn-default btn-white  btn-sm quick-action-btn">
								...
								حذف</button>
                <?php  } if ($User->isAllowedPerm(['اعتماد ترقية'], $appid)) { ?>
...
function confirm_check (id) {
	if(id !=''){
			$('#modal_title').text('تأكيد عملية التاكيد');
				  ...
				  $('#modal_default .modal-dialog').removeClass('modal-lg');
				$('#modal_default').modal({show:true});
			  $('#modal_default .modal-body').load('./hr-app/promotion-conform?id='+id+'',function(){
			});
		}
	}    
