...
if(!empty($_GET['confirme']) && !empty($_POST['leave_id']) ){
	...
	function removeRow($connect, $id) {
		...
		try {
                ...
                $parma2 = ["id" => (int)$id ];
                $q2 = "delete  from leaveclassification  WHERE id =:id LIMIT 1";
...
<form class="form-horizontal" role="form" action="#" method="post" id="remove_confirmed_fm">
	...
	<input type="hidden" name="leave_id" id="leave_id" class="form-control" value="<?=$_GET['id']?>" >
...
$(document).ready(function(){
    ...
    $("#remove_confirmed_fm").on('submit', function(e){
        ...
        var form_data = $(this).serialize();
            $.ajax({
                url: 'hr-app/index.php?action=leaveClassficate-remove?confirme=1',
                type: 'POST',
                data:form_data,
                dataType:"json",
                beforeSend:function(){
                ...
                success:function(data){
                    ...
                    if(data.reload){
                        window.location.href = 'leaveClassficate-list';
                    }
