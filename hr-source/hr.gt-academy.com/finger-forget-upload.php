...
if(!empty($_GET['confirme']) && !empty($_POST['forget_finger']) ){
	
	 $result = false;
	 $reload = false;
	 $msg ='خطاء..!';
	 $id= (int) $_POST['forget_finger'];
...
<form class="form-horizontal" role="form" action="#" method="post" id="conform_incentive">
	<p class="text-danger"><b>هل ترغب بالفعل برفع طلب التحصير</b>
	<input type="hidden" name="forget_finger" id="forget_finger" class="form-control" value="<?=$_GET['id']?>" >
...
$(document).ready(function(){
    ...
    $("#conform_incentive").on('submit', function(e){
        ...
        var form_data = $(this).serialize();
            $.ajax({
                url: 'hr-app/index.php?action=finger-forget-upload?confirme=1',
                type: 'POST',
                data:form_data,
                dataType:"json",
                beforeSend:function(){
                ...
                success:function(data){
                    ...
                    if(data.reload){
                        window.location.href = 'finger-forget-view?id='+$("#forget_finger").val()+'; ';
                    }
