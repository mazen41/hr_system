    ...
    <div class="row" id="filter-area" style="display:none_">
       ...
       <form class="form-horizontal" role="form" action="#" method="post" id="filter-fm">
               ...
               <div class="form-group">
                   ...
                   <label for="incentive_name" class="col-form-label">اسم الاجازة</label>
                ...
                <label for="filter_status" class="col-form-label">حالة الاجازة</label>
                  <select class=" selectpicker show-tick" data-width="100%"  title="أي" id="filter_status" name="filter_status">
                    ...
                    <option value="2">موقفه</option>
                                 ...
                                 <div class="col-md-3">
              <div class="form-group">
                <label for="filter_bay" class="col-form-label">الدفع</label>
                  <select class=" selectpicker show-tick" data-width="100%"  title="أي" id="filter_bay" name="filter_bay">
                    <option value="1">مدفوعه كليا</option>
                    <option value="2">مدفوعه جزئيا</option>
                    <option value="3">غير مدفوعه</option>
                  </select>
              </div>
                  </div>
			...
			<div class="col-md-12">
					...
					<div class="table-responsive">
					   <table id="entries_tb" class="table dataTable table-bordered table-hover nowrap dtr-inline collapsed   display " width="100%" style="background: white;">
						<thead>
                        <tr >
<th style="text-align: center; vertical-align: middle;" >اسم الاجازة</th>
<th style="text-align: center; vertical-align: middle;" >الفرع</th>
<th style="text-align: center; vertical-align: middle;" >حالة الاجازة</th>
<th style="text-align: center; vertical-align: middle;" >نوع الاجازة</th>
...
entriesData('no');
function entriesData(is_date_search,name,branchs=[],date_range='',filter_status='',filter_bay=''){
	//var account = 0;
	var groupColumn = 0;
  ...
  var dataTable = $('#entries_tb').DataTable({
   ...
   "ajax" : {
	url:"hr-app/index.php?action=report-leaveclassficate",
	type:"POST",
	data:{
     ...
     buttons: [
         ...
         drawCallback: function (settings) {
           ...
           // $('#selected_client').html(settings.json.results_note.selected_client);
            $('#selected_client').html('<h5>إجازة '+$('#select2-client-container').text()+'</h5>');
             }else{
...
function apply_filters(){
  ...
  var name = $('#incentive_name').val();
    ...
    var filter_bay = $('#filter_bay').val();

//   $('#entries_tb').DataTable().destroy();
  entriesData('yes',name,branchs,date_range,filter_status,filter_bay);
