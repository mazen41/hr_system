...
if(!empty($_GET['confirme']) && !empty($_POST['jobgrade_id']) ){
	
	 $result = false;
	 $reload = false;
	 $msg ='خطاء..!';
	 $id= (int) $_POST['jobgrade_id'];
	...
	function removeRow($connect, $id) {
		...
		try {
                ...
                $parma2 = ["id" => (int)$id ];
                $q2 = "delete  from  tbljobgrade  WHERE Id  =:id LIMIT 1";
		...
		return false;
				...
				{
					$msg = 'هذه الدرجة مرتبطه بمستخدمين ولايمكن حذفها';
				}
...
<form class="form-horizontal" role="form" action="#" method="post" id="remove_confirmed_fm">
	<p class="text-danger"><b>هل ترغب بالفعل بحذف الدرجة الوظيفية</b>
	<input type="hidden" name="jobgrade_id" id="jobgrade_id" class="form-control" value="<?=$_GET['id']?>" >
...
$(document).ready(function(){
    ...
    $("#remove_confirmed_fm").on('submit', function(e){
        ...
        var form_data = $(this).serialize();
            $.ajax({
                url: 'hr-app/index.php?action=jobgrade-remove?confirme=1',
                type: 'POST',
                data:form_data,
                dataType:"json",
                beforeSend:function(){
                ...
                success:function(data){
                    ...
                    if(data.reload){
                        window.location.href = 'jobgrade-list';
                    }
