  <?php
if(!isset($_GET['id'])){
	echo'<script> location.replace("report-insurance"); </script>';
	die(); 
} 
$screen = 'الفترات';
$page_title = 'تفاصيل  شركات شركات';
$report_name = 'تفاصيل  شركات التامين';
...
$query = " SELECT a.Id  ,b.branch_name,a.Name,a.type,a.state,a.CreatedDate,a.NameOfRepresentative,
 u.FirstName , u.LastName,a.Phone,a.Email,a.Address,a.Note
FROM  tbinsurance AS a
...
";
    ...
    if ($st->rowCount() > 0) {
        ...
        WHERE FIND_IN_SET(?, u.user_insurance)";
...
/* 
	...
	<?php endif;?>
			...
			<div class="card-body p-2">
              ...
              <li class="nav-item">
                <a class="nav-link active" id="main-info-tab" data-toggle="pill" href="#main-info" role="tab" aria-controls="main-info" aria-selected="true"><strong>عن شركة التامين</strong></a>
              </li>            
			</ul>
			...
			<!-- start -->
                               ...
                               <div class="section">
            <div class="section-title">معلومات شركة التامين</div>
            <table>
                <tbody><tr>
                ...
                <tr>
                    <td style="font-weight: bold;">اسم ممثل الشركة</td>
                    <td><?= !empty($row['NameOfRepresentative'])?$row['NameOfRepresentative']:'غير محدد' ?></td>
                    <td style="font-weight: bold;">نوع التامين</td>
                    <td><?= ($row['type']==1) ? 'تأمين على الحياة' : (($row['type']==2) ? 'تأمين صحي' : (($row['type']==3) ? 'تأمين على السيارات' : (($row['type']==4) ? 'تأمين ضد الحوادث' : 'تأمين الممتلكات'))) ?></td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">رقم الاتصال التسلسلي</td>
                    <td><?= !empty($row['Phone'])?$row['Phone']:'غير محدد' ?></td>
                    <td style="font-weight: bold;">البريد الالكتروني</td>
                    <td><?= !empty($row['Email'])?$row['Email']:'غير محدد' ?></td>
                </tr>
                    <tr>
                    <td style="font-weight: bold;">عنوان الشركة</td>
                    <td><?= !empty($row['Address'])?$row['Address']:'غير محدد' ?></td>
                    <td style="font-weight: bold;">حالة الشركة</td>
                    <td><?= ($row['state']==1)?'نشط':'غير نشط' ?></td>
                </tr>
                <tr>
                    ...
                    <td colspan="3"><?= !empty($row['Note'])?$row['Note']:'غير محدد' ?></td>
        ...
        <div class="section">
            <div class="section-title">المضافين في هذا الجهاز</div>
            <table>
                <thead>
                    <tr>
                        <th style="width: 5%;">#</th>
...
function printData() {
    ...
    body {
        ...
        printWindow.document.write(`
            <html>
                <head>
                    <title>تقرير شركات التامين</title>
                    ${styles}
                </head>
                <body>
                    ${printContents}
