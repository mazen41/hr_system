...
$query = " SELECT a.Id ,a.UserID ,b.branch_name,a.name,a.Status,a.LastUpdateDate,a.incentive_type,a.monthly,a.DueDate,
...
foreach($result as $row)
{
        ...
        'date' => ($row['incentive_type']==1)?'شهرية':'تاريخ محدد',
        'type' => ($row['incentive_type'] == 1)
    ? 'نهاية كل شهر'
    : (
        ($row['monthly'] == 1)
        ...
        'link' => '<a href="report-one-incentive?id='.$row['Id'].'" class="btn btn-info btn-sm show-details" data-id="'.$row['Id'].'" title="تفاصيل"><i class="fa fa-eye"></i></button>',
    ];
