...
?>
<!-- Make sure these scripts are loaded in footer.php or before this script block -->
<!-- If not, you might need to manually include them here, but typically they'd be in footer.php -->
...
function apply_filters()
{
    ...
    var states = $('#filter_status').val(); // Corrected variable name from `state` to `states`
    $('#data_tb').DataTable().destroy();
    clients_data('yes',date_range,branchs,states); // افحص مخرجات الداله
}
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='',branchs=[], states=[])
 {
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=contractRenewal-list",
    type:"POST",
    data:{
    ...
    aoColumns: [
            ...
            {
                data: 'employee_name', // Changed to match backend key
                render: function(data, type) {
                    return data; // Directly return the name
                }
            },  
            {
                data: 'branch_name', // Changed to match backend key
                className: 'col_states',
                render: function(data, type) {
                    return data; // Directly return the branch name
                     
                }
            },
            {
                data: 'creator_info', // Changed to match backend key
                render: function(data, type) {
                    return '<small>'+data+'</small>'; // Backend now provides combined info
                     
                }
            },
            {
                data: 'renewal_status', // Changed to match backend key
                className: 'col_states',
                render: function(data, type) {
                    ...
                    if (data == 1) {
                        ...
                        statusText = 'غير محدد'; // Default for unknown status
                    }
                    return statusText;
                }
            },
            ...
            { 
               data: 'id', // This should be the ID for actions
               "bSortable": false,
               render: function(data, type, row) { // `row` param contains all data for the current row
                    ...
                    <?php if ($User->isAllowedPerm(['عرض عقد'], $appid)) { ?>
                        options += '<a href="contractRenewal-view?id=' + data + '" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a>';
                    <?php } ?>

                    <?php if ($User->isAllowedPerm(['تعديل عقد'], $appid)) { ?>
                        options += '<a href="contractRenewal-add?edit=' + data + '" type="button" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a>';
                    <?php } ?>
