...
//$fix_foot = false;
$appid  = 'HR';
$page_perm=['اضافة بصمة','عرض بصمة','تعديل بصمة','حذف بصمة'];
// $screen = 'الاجهزة ';
...
.table.dataTable  td, .table.dataTable  th {
    ...
    <div class="content-header page-nav" >
      <div class="container-fluid ">
        <div class="row ">
          <div class="col-7">
            <span class="page-title">اجهزة البصمة</span>
          </div><!-- /.col -->
              ...
              <div class="form-group">
                <label for="filter_status" class="col-form-label">الحالة</label>
                  <select class=" selectpicker show-tick" data-width="100%"  title="أي" id="filter_status" name="filter_status">
                    ...
                    <option value="3">جاري الصيانة</option>
                  </select>
              </div>
                  </div>
...
$('#add-client-bt').on('click', function(event){
   window.open("fingerprint-add","_self");
});
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='',branchs=[], states=[])
 {
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=fingerprint-list",
    type:"POST",
    data:{
    ...
    columns: [
        ...
        { 
            ...
            render: function(data, type, row) {
                ...
                <?php if ($User->isAllowedPerm(['عرض بصمة'], $appid)) { ?>
                    options += '<a href="fingerprint-view?id=' + row.id + '" class="dropdown-item">' +
                        '<i class="fa fa-eye"></i> عرض</a>';
                <?php } ?>
                <?php if ($User->isAllowedPerm(['تعديل بصمة'], $appid)) { ?>
                    options += '<a href="fingerprint-add?id=' + row.id + '" class="dropdown-item">' +
