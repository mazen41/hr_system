...
if(!empty($_GET['confirme']) && !empty($_POST['section_id']) ){
	...
	function removeRow($connect, $id) {
		...
		try {
                ...
                $parma2 = ["id" => (int)$id ];
                $q2 = "delete  from tblsection  WHERE Id  =:id LIMIT 1";
		...
		return false;
				...
				{
					$msg = 'هذه القسم عباره عن اب ولديه ابناء';
				}
			}
				else
				{
					$msg = 'هذه القسم مرتبطه بمستخدمين ولايمكن حذفها';
				}
...
<form class="form-horizontal" role="form" action="#" method="post" id="remove_confirmed_fm">
	<p class="text-danger"><b>هل ترغب بالفعل بحذف القسم</b>
	<input type="hidden" name="section_id" id="section_id" class="form-control" value="<?=$_GET['id']?>" >
...
$(document).ready(function(){
    ...
    $("#remove_confirmed_fm").on('submit', function(e){
        ...
        var form_data = $(this).serialize();
            $.ajax({
                url: 'hr-app/index.php?action=section-remove?confirme=1',
                type: 'POST',
                data:form_data,
                dataType:"json",
                beforeSend:function(){
