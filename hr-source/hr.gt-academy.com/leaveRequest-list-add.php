...
$('#add-leaverequest-bt').on('click', function(event){
   window.open("leaveRequest-add-add","_self");
});
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='', states=[])
 {
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=leaveRequest-list-add",
    type:"POST",
    data:{
    ...
    aoColumns: [
            ...
            },
            {
			  ...
			  render: function(data, type) {
				 if( data.draft___==0)
                   return '<div class="btn-group"> <button type="button" class="btn btn-default " data-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-left" role="menu"><a href="leaveRequest-view-add?id='+data.id+'" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a><a type="button"  href="leaveRequest-add-add?id='+data.id+'" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a><button type="button" class="dropdown-item  remove_client" value="'+data.id+'"><i class="fa fa-trash-alt"></i> حذف</button></div></div>';    
                    else
...
function confirm_remove (id) {
		if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  $('#modal_default .modal-body').load('./hr-app/leaveRequest-remove-add?id='+id+'',function(){
			});
		}
	}
