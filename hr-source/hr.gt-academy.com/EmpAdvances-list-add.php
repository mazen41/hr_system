...
$('#add-advances-bt').on('click', function(event){
   window.open("EmpAdvances-add-add","_self");
});
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 ...
 { 
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=EmpAdvances-list-add",
    type:"POST",
    data:{
            ...
            },
            {
			  ...
			  render: function(data, type) {
				 if(data.status==0 && data.draft___==0)
                   return '<div class="btn-group"> <button type="button" class="btn btn-default " data-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-left" role="menu"><a href="EmpAdvances-view-add?id='+data.id+'" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a><a type="button"  href="EmpAdvances-add-add?id='+data.id+'" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a><button type="button" class="dropdown-item  remove_client" value="'+data.id+'"><i class="fa fa-trash-alt"></i> حذف</button></div></div>';    
                    else
...
function confirm_remove (id) {
		if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  $('#modal_default .modal-body').load('./hr-app/EmpAdvances-remove-add?id='+id+'',function(){
			});
		}
	}
