...
if(!empty($_GET['confirme']) && !empty($_POST['EmpAdvances_id']) ){
	
	 $result = false;
	 $reload = false;
	 $msg ='خطاء..!';
	 $id= (int) $_POST['EmpAdvances_id'];

     function GetlastID($connect, $id) {
        $sql = "SELECT Status, DueDate,Draft FROM tblempadvances WHERE Id = :id LIMIT 1";
	...
	function removeRow($connect, $id) {
		...
		try {
                ...
                $parma2 = ["id" => (int)$id ];
                $q2 = "delete  from   tblempadvances  WHERE Id  =:id ";
    ...
    $check=GetlastID($connect_pdo,$id);		
				...
				{
					$msg = 'تاريخ استحقاق السلفه اكبر من تاريخ اليوم';
				}
                else if($check==2)
				{
					$msg = 'هذا السلفة  تم اعتمادها ولايمكن حذفها';
				}
                else if($check==4)
				{
					$msg = 'السلفة هذه تم رفعها وليس لديك الصلاحية لحذفها';
				}
...
<form class="form-horizontal" role="form" action="#" method="post" id="remove_confirmed_fm">
	<p class="text-danger"><b>هل ترغب بالفعل بحذف السلفة</b>
	<input type="hidden" name="EmpAdvances_id" id="EmpAdvances_id" class="form-control" value="<?=$_GET['id']?>" >
...
$(document).ready(function(){
    ...
    $("#remove_confirmed_fm").on('submit', function(e){
        ...
        var form_data = $(this).serialize();
            $.ajax({
                url: 'hr-app/index.php?action=EmpAdvances-remove-add?confirme=1',
                type: 'POST',
                data:form_data,
                dataType:"json",
                beforeSend:function(){
