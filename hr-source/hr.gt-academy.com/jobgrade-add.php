...
if(isset($_GET['id'])){
	...
	if($st->rowCount() > 0){
		$row = $st->fetch();
		$client_id = $row['Id'];
		$form_title = 'تعديل درجة  ' .$row['Name'].'';
		...
		$sql = "SELECT 	GradeID FROM tblremewal WHERE FIND_IN_SET(:id, GradeID) LIMIT 1";
    ...
    <div class="row">
			<div class="col-md-12">
                ...
                <div class="card-header"    style="background: aliceblue; cursor: pointer">
					<h4 class="card-title">تفاضيل الدرجه</h4>
					...
					<div class="card-body p-3">
					  <div class="row">
                          ...
                          <div class="form-group col-md-6">
							<label class="col-form-label required"  for="jobgradename">اسم الدرجة الوظيفية</label>
							<input type="text"  value="<?=!empty($row['Name']) ? $row['Name'] : ''?>" class="form-control"  data-toggle="tooltip"  id="jobgradename" name="jobgradename"  autocomplete="off" required>
						 </div>
                         <div class="form-group col-md-6">
                          ...
                          <label class="col-form-label required" for="branchs_list">هذه الدرجة تم ربطها بموظفين لايمكن تغير الفرع</label>
...
$('#AddShift').on('submit', function(e){  
	...
	if($(this).valid()){
	$.ajax({
        url:"hr-app/index.php?action=jobgrade-add",
        method:"POST", 
		data:form_data,
		dataType:"json", 
		beforeSend:function(){
		...
		success:function(data){
			if(data.result){
				...
				if(data.id > 0){  
                   window.location.href = 'jobgrade-view?id='+data.id+'';
				}
