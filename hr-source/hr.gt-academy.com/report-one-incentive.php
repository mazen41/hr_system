<?php
if(!isset($_GET['id'])){
	echo'<script> location.replace("report-incentive"); </script>';
	die(); 
} 
$screen = 'المكافئات';
$page_title = 'تفاصيل  المكافئات';
$report_name = 'تفاصيل المكافئة';
 ...
 if (isset($_GET['id'])) {
    ...
    $query = "SELECT a.Id, a.BranchID, a.UserID, a.name, a.Amount,a.AmountType, a.Currency, a.Reason, a.for_what, a.extionsion,a.Status ,
                     a.DueDate, a.CreatedDate, a.LastUpdateDate, a.CreatedBy, u.FirstName, u.LastName, d.branch_name,a.incentive_type,a.monthly,a.DueDate
...
/* 
	...
	<?php endif;?>
			...
			<!-- start -->
                               ...
                               <div class="section">
            <div class="section-title">معلومات المكافأة</div>
            <table>
                <tbody><tr>
                    <td style="width: 25%; font-weight: bold;">اسم المكافأة</td>
                    <td style="width: 25%;"><?= !empty($row['name'])?$row['name']:'غير محدد' ?></td>
                    <td style="width: 25%; font-weight: bold;">نوع المكافأة</td>
                    <td style="width: 25%;"><?= !empty($row['incentive_type'])?(($row['incentive_type']==1)?'شهرية':'محدده'):'غير محدد' ?></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><?= !empty($row['DueDate'])?$row['DueDate']:(($row['incentive_type']==1)?'نهاية كل شهر':'') ?></td>
...
<td style="font-weight: bold;">حالة المكافئة</td>
