...
//$fix_foot = false;
$appid  = 'HR';
$page_perm=['إضافة مكافئه','عرض مكافئه','تعديل مكافئه','حذف مكافئه'];
...
.table.dataTable  td, .table.dataTable  th {
    ...
    <div class="content-header page-nav" >
      <div class="container-fluid ">
        <div class="row ">
          <div class="col-7">
            <span class="page-title">المكافئات</span>
          </div><!-- /.col -->
...
$('#add-client-bt').on('click', function(event){
   window.open("incentive-add","_self");
});
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='',branchs=[], states=[])
 {
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=incentive-list",
    type:"POST",
    data:{
    ...
    aoColumns: [
          ...
          var options='<div class="btn-group"> <button type="button" class="btn btn-default " data-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-left" role="menu">';
                <?php if ($User->isAllowedPerm(['عرض مكافئه'], $appid)) { ?>
                    options += '<a href="incentive-view?id=' + data + '" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['تعديل مكافئه'], $appid) ) { ?>
                    options += '<a href="incentive-add?id=' + data + '" type="button" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['حذف مكافئه'], $appid)) { ?>
...
function confirm_remove (id) {
		if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  $('#modal_default .modal-body').load('hr-app/index.php?action=incentive-remove-modal&id='+id,function(){
