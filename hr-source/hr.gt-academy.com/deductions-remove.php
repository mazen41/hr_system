...
if(!empty($_GET['confirme']) && !empty($_POST['deduction_id']) ){
     ...
     function GetlastID($connect, $id) {
        $sql = "SELECT Status, DueDate FROM  tbldeductions WHERE Id = :id LIMIT 1";
...
$flage_3=false;
    ...
    if($today_date < $check['DueDate'] ) $flage=true; else $flage_2=true;
    if(empty($check['Status'])) $flage=true; else $flage_3=true;
	...
	function removeRow($connect, $id) {
		...
		try {
                ...
                $parma2 = ["id" => (int)$id ];
                $q2 = "delete  from  tbldeductions  WHERE Id  =:id ";
		...
		return false;
				...
				{
					$msg = 'تاريخ استحقاق التعويض اقل من تاريخ اليوم';
				}
                else if($flage_3)
				{
					$msg = 'هذه التعويض تم اعتمادها ولايمكن حذفها';
				}
...
<form class="form-horizontal" role="form" action="#" method="post" id="remove_confirmed_fm">
	<p class="text-danger"><b>هل ترغب بالفعل بحذف الخصم</b>
	<input type="hidden" name="deduction_id" id="deduction_id" class="form-control" value="<?=$_GET['id']?>" >
...
$(document).ready(function(){
    ...
    $("#remove_confirmed_fm").on('submit', function(e){
        ...
        var form_data = $(this).serialize();
            $.ajax({
                url: 'hr-app/index.php?action=deductions-remove?confirme=1',
                type: 'POST',
                data:form_data,
                dataType:"json",
                beforeSend:function(){
