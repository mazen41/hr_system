...
if(isset($_GET['id'])){
	...
	$query = "SELECT Id,BranchID,UserID,name,Amount,Currency,Reason,for_what,extionsion,DueDate,incentive_type,AmountType,monthly,Status
 	FROM  tblincentives 
	WHERE  Id  = :id ";
    ...
    <div class="row">
			<div class="col-md-12">
                ...
                <div class="card-header"    style="background: aliceblue; cursor: pointer">
					<h4 class="card-title">تفاضيل المكافئة</h4>
           ...
           <div class="form-group col-md-4">
                                <label class="col-form-label required" for="name">اسم المكافئة</label>
                             ...
                             <div class="form-group col-md-4">
                              ...
                              <div class="form-group col-md-4">
                               <label class="col-form-label required" for="incentive_type">نوع المكافئة</label>
                               <select class=" selectpicker "   data-container="body" data-size="5" data-width="100%" title="أدخل النوع" id="incentive_type" name="incentive_type"  required>
                               <option value="1" <?= (!empty($row['incentive_type']) && $row['incentive_type'] == '1') ? 'selected' : '' ?>>شهرية تتكرر كل شهر </option>
                               <option value="2" <?= (!empty($row['incentive_type']) && $row['incentive_type'] == '2') ? 'selected' : '' ?>>شهر محدد</option>
                                   </select>
                             </div>
                        ...
                        <div class="form-group col-md-4">
                            ...
                            <div class="form-group col-md-4">
                             ...
                             <div class="form-group col-md-4" id="Div_Due_date" style="display:none">
                              <label class="col-form-label required" for="Due_date">حدد اخر يوم من الشهر اللي تضاف فيه هذه المكافئة<br>تصرف نهاية الشهر </label>
                       ...
                       <div class="row">
                        <div class="form-group col-md-4">
                           ...
                           <option value="1" >لموظف</option>
                            ...
                            <!-- <option value="5" >لمسمى وظيفي لم تتم ضبطة لان هاني قد اضفها من قبل</option> -->
                               </select>
                         </div>
...
$('#AddIncentive').on('submit', function(e) {  
  ...
  // var form_data = $(form).serialize() + '&type=' + type + '&id=' + param_id + '&extinsion__=' + extinsion + '&employer__=' + employer;

	if($(this).valid()){
	$.ajax({
        url:"hr-app/index.php?action=incentive-add",
        method:"POST", 
		data:form_data,
		dataType:"json", 
		beforeSend:function(){
...
$('#employer').change(function() {
    ...
    if (selectedValue && selectedValue.length > 0) {
        $.ajax({
            url: 'hr-app/index.php?action=incentive-info_show',
            type: 'POST',
