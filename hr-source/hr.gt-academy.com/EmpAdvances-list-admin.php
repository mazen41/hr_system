...
.advance-action-menu{
    ...
    <div class="content-header page-nav" >
      <div class="container-fluid ">
        <div class="row ">
          <div class="col-7">
            <span class="page-title">جميع السلف</span>
          </div><!-- /.col -->
...
function clients_data(is_date_search, date_range='', states=[])
 {
  ...
  advancesAdminTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=EmpAdvances-list-admin",
    type:"POST",
    data:{
    ...
    aoColumns: [
            ...
            },
            {
			  ...
			  "bSortable": false,
			  render: function(data, type) {
                var html = '<div class="btn-group"> <button type="button" class="btn btn-default" data-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-left advance-action-menu" role="menu">';
                html += '<a href="EmpAdvances-view-admin?id='+data.id+'" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a>';
                if((data.status === null || data.status === '' || Number(data.status) === 0) && Number(data.draft) === 1){
                    html += '<button type="button" class="dropdown-item approve-advance" value="'+data.id+'"><i class="fa fa-check text-success"></i> اعتماد</button>';
                    html += '<button type="button" class="dropdown-item reject-advance" value="'+data.id+'"><i class="fa fa-times text-danger"></i> رفض</button>';
                }
                html += '<button type="button" class="dropdown-item remove_client" value="'+data.id+'"><i class="fa fa-trash-alt"></i> حذف</button></div></div>';
                return html; 
				 }
...
function reloadAdvancesAdminTable() {
...
function confirm_remove (id) {
		if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  $('#modal_default .modal-body').load('./hr-app/EmpAdvances-remove-admin?id='+id+'',function(){
			});
		}
	}
