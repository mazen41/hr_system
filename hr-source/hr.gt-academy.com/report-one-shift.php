<?php
if(!isset($_GET['id'])){
	echo'<script> location.replace("report-shift"); </script>';
	die(); 
} 
$screen = 'الفترات';
$page_title = 'تفاصيل  الفتره';
$report_name = 'تفاصيل الفترع';
 ...
 if (isset($_GET['id'])) {
    ...
    $query = "SELECT a.ShiftID,b.branch_name,a.ShiftName,a.ShiftStartTime,a.ShiftEndTime,a.LastUpdatetim,a.TotalworkHour,a.ShiftState,a.CreatedDate,
 u.FirstName , u.LastName,c.enable_half_day,c.half_day_minutes,c.absent_minutes,c.allowed_early_leave,c.enable_early_half_day,c.early_half_day_minutes,
c.shift_id,c.allowed_late_minutes,c.early_absent_minutes,c.missing_checkout_action,c.late_checkout_policy,a.NumFootprint
FROM tbshift AS a 
LEFT JOIN branches AS b ON a.BranchID = b.branch_id
LEFT JOIN tblusers AS u ON a.CreatedBy = u.UserID
LEFT JOIN shift_setting AS c ON a.ShiftID = c.shift_id
where a.ShiftID=:id";
    ...
    if ($st->rowCount() > 0) {
        ...
        WHERE FIND_IN_SET(?, t.shiftID)";
            
            
                
                
                $stmt_emp = $connect_pdo->prepare($query_emp);
                $stmt_emp->execute([$get_id]);
...
/* 
	...
	<?php endif;?>
			...
			<!-- start -->
                               ...
                               <div class="section">
            <div class="section-title">معلومات الفتره</div>
            <table>
                <tbody><tr>
                    <td style="width: 25%; font-weight: bold;">اسم الفترة</td>
                    <td style="width: 25%;"><?= !empty($row['ShiftName'])?$row['ShiftName']:'غير محدد' ?></td>
                    <td style="width: 25%; font-weight: bold;">نوع البصمة</td>
                    <td style="width: 25%;"><?= !empty($row['NumFootprint'])?(($row['NumFootprint']==1)?'بصمة حضور':'بصمة حضور وإنصراف'):'غير محدد' ?></td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">الفرع</td>
                    <td><?= !empty($row['branch_name'])?$row['branch_name']:'غير محدد' ?></td>
                    <td style="font-weight: bold;">انشئات بواسطة</td>
                    <td><?= !empty($row['FirstName'])?$row['FirstName'].' '.$row['LastName']:'غير محدد' ?></td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">وقت بداية الداوم</td>
                    <td><?= !empty($row['ShiftStartTime'])?$row['ShiftStartTime']:'غير محدد' ?></td>
                    <td style="font-weight: bold;">وقت انتهاء الدوام</td>
                    <td><?= !empty($row['ShiftEndTime'])?$row['ShiftEndTime']:'غير محدد' ?></td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">دقائق التاخير المسموح به</td>
                    <td><?= !empty($row['allowed_late_minutes'])?$row['allowed_late_minutes']:'غير محدد' ?></td>
                    <td style="font-weight: bold;">دقائق الانصراف المسموح بها</td>
                    <td><?= !empty($row['allowed_early_leave'])?$row['allowed_early_leave']:'غير محدد' ?></td>
                </tr>
                <tr>
                    <td style="font-weight: bold;">اعدادت مابعد التاخير المسموح بة </td>
                    <td><?= !empty($row['enable_half_day'])?(($row['enable_half_day']==1)?'نعم، حساب ساعات العمل الفعلية فقط':($row['enable_half_day']==2)?'لا، أي تأخير أكثر من المسموح = غياب':'نعم، أريد تحديد وقت نصف دوام وغياب' ):'غير محدد' ?></td>
                    <td style="font-weight: bold;">اعدادات ما بعد الانصراف المسموح به </td>
                    <td><?= !empty($row['enable_early_half_day'])?(($row['enable_early_half_day']==1)?'نعم، حساب ساعات العمل الفعلية فقط':($row['enable_early_half_day']==2)?'لا، أي تأخير أكثر من المسموح = غياب':'نعم، أريد تحديد وقت نصف دوام وغياب' ):'غير محدد' ?></td>
                </tr>

                <tr>
                    <td style="font-weight: bold;">دقائق بداية اعتبار نصف دوام بالنسبة للحضور المتاخر</td>
                    <td><?= !empty($row['half_day_minutes'])?$row['half_day_minutes']:'غير محدد' ?></td>
                    <td style="font-weight: bold;">دقائق بداية اعتبار غياب بالنسبة للحضور المتاخر</td>
                    <td><?= !empty($row['absent_minutes'])?$row['absent_minutes']:'غير محدد' ?></td>
                </tr>

                <tr>
                    <td style="font-weight: bold;">دقائق بداية اعتبار نصف دوام بالنسبة للانصراف المبكر</td>
                    <td><?= !empty($row['early_half_day_minutes'])?$row['early_half_day_minutes']:'غير محدد' ?></td>
                    <td style="font-weight: bold;">دقائق بداية اعتبار غياب بالنسبة للانصراف المبكر</td>
                    <td><?= !empty($row['early_absent_minutes'])?$row['early_absent_minutes']:'غير محدد' ?></td>
                </tr>

                <tr>
                    <td style="font-weight: bold;">في حال عدم تسجيل الانصراف</td>
                    <td><?= !empty($row['missing_checkout_action'])?(($row['missing_checkout_action']==1)?'اعتماد نهاية الدوام تلقائيًا':($row['missing_checkout_action']==2)?'يُحسب نصف دوام':($row['missing_checkout_action']==3)?'يُحسب غياب كامل':'فقط تنبيه بدون خصم'):'غير محدد' ?></td>
                    <td style="font-weight: bold;">في حال تسجيل انصراف بعد نهاية الدوام</td>
                    <td><?= !empty($row['late_checkout_policy'])?(($row['late_checkout_policy']==1)?'لا يُحسب شيء':($row['late_checkout_policy']==2)?'يُحسب كعمل إضافي':'فقط إشعار / تنبيه'):'غير محدد' ?></td>
                </tr>

                <tr>
                    <td style="font-weight: bold;">حالة الفترة</td>
                    <td><?= !empty($row['ShiftState'])?'موقف':'نشط' ?></td>
                    <td style="font-weight: bold;">تاريخ الانشاء</td>
        ...
        <div class="section">
            <div class="section-title">المضافين في الفترة</div>
            <table>
                <thead>
                    <tr>
                        <th style="width: 5%;">#</th>
