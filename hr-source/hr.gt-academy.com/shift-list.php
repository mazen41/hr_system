<?php
$appid  = 'HR';
$page_perm=['اضافة فترة','عرض فترة','تعديل فترة','حذف فترة'];
...
@media (max-width: 768px) {
    ...
    <div class="content-header page-nav" >
      <div class="container-fluid ">
        <div class="row ">
          <div class="col-7">
            <span class="page-title">الفترات</span>
          </div><!-- /.col -->
          <div class="col-5 text-left">	
          <?php if($User->isAllowedPerm(['اضافة فترة'],$appid)){ ?>
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='',branchs=[], states=[])
 {
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=shift-list",
    type:"POST",
    data:{
    ...
    "columns": [
            ...
            { data: 'branch' },
            { data: 'date_range' },
            { data: 'creator', render: function(data){ return data || '-'; } },
            { data: 'start_time' },
            { data: 'end_time' },
            { data: 'total_hours' },
            { data: 'state_html', orderable: false, searchable: false },
            {
                data: 'id',
                orderable: false,
                searchable: false,
                render: function(id){
                    ...
                    <?php if ($User->isAllowedPerm(['عرض فترة'], $appid)) { ?>
                        options += '<a href="shift-view?id=' + id + '" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a>';
                    <?php } ?>

                    <?php if ($User->isAllowedPerm(['تعديل فترة'], $appid)) { ?>
                        options += '<a href="shift-add?id=' + id + '" type="button" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a>';
                    <?php } ?>

                    <?php if ($User->isAllowedPerm(['حذف فترة'], $appid)) { ?>
                        options += '<button type="button" class="dropdown-item remove_client" value="' + id + '"><i class="fa fa-trash-alt"></i> حذف</button>';
                    <?php } ?>
