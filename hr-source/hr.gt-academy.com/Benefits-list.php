<?php
$appid  = 'HR';
$page_perm=['إضافة تعويض','عرض تعويض','تعديل تعويض','حذف تعويض'];
...
.table.dataTable  td, .table.dataTable  th {
    ...
    <div class="content-header page-nav" >
      <div class="container-fluid ">
        <div class="row ">
          <div class="col-7">
            <span class="page-title">التعويضات</span>
          </div><!-- /.col -->
          <div class="col-5 text-left">	
						
      <?php if($User->isAllowedPerm(['إضافة تعويض'],$appid)){ ?>
...
$('#add-client-bt').on('click', function(event){
   window.open("Benefits-add","_self");
});
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='',branchs=[], states=[])
 {
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=Benefits-list",
    type:"POST",
    data:{
    ...
    aoColumns: [
            ...
            },
            {
                data: 'createdby',
                render: function(data, type, row) {
					return '<small>'+(data || '')+'<br>'+(row.createddate || '')+'</small>';
                }
            },
            {
                data: 'status',
                className: 'col_states',
                render: function(data, type, row) {
					return data == 1 ? '<span class="badge badge-success">معتمد</span>' : '<span class="badge badge-warning">غير معتمد</span>';
                }
            },
          ...
          var options='<div class="btn-group"> <button type="button" class="btn btn-default " data-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-left" role="menu">';
                <?php if ($User->isAllowedPerm(['عرض تعويض'], $appid)) { ?>
                    options += '<a href="Benefits-view?id=' + data + '" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['تعديل تعويض'], $appid)) { ?>
                    options += '<a href="Benefits-add?id=' + data + '" type="button" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['حذف تعويض'], $appid)) { ?>
...
function confirm_remove (id) {
		if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  $('#modal_default .modal-body').load('hr-app/index.php?action=Benefits-remove-modal&id='+id,function(){
