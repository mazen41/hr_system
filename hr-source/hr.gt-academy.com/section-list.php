<?php
$appid  = 'HR';
$page_perm=['اضافة قسم','عرض قسم','تعديل قسم','حذف قسم'];
// $screen = 'الاقسام ';
// $page_title = 'إدارة الاقسام';
...
.nested {
    ...
    <div class="content-header page-nav" >
      <div class="container-fluid ">
        <div class="row ">
          <div class="col-7">
            <span class="page-title">الادارات</span>
          </div><!-- /.col -->
          <div class="col-5 text-left">	
          <?php if($User->isAllowedPerm(['اضافة قسم'],$appid)){ ?>
...
echo '<a href="section-add?id=' . $node['Id'] . '" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a>';
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='',branchs=[])
 {
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=section-list",
    type:"POST",
    data:{
    ...
    columns: [
        ...
        { data: 'createddate', className: 'col_states' },
        { data: 'name', className: 'col_states' },
        { 
            data: null,
            orderable: false,
            render: function(data, type, row) {
                ...
                <?php if ($User->isAllowedPerm(['عرض قسم'], $appid)) { ?>
                    options += '<a href="section-view?id=' + row.id + '" class="dropdown-item">' +
                        '<i class="fa fa-eye"></i> عرض</a>';
                <?php } ?>
                <?php if ($User->isAllowedPerm(['تعديل قسم'], $appid)) { ?>
                    options += '<a href="section-add?id=' + row.id + '" class="dropdown-item">' +
                        '<i class="fa fa-edit"></i> تعديل</a>';
                <?php } ?>
                <?php if ($User->isAllowedPerm(['حذف قسم'], $appid)) { ?>
...
function confirm_remove (id) {
		if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  $('#modal_default .modal-body').load('./hr-app/section-remove?id='+id+'',function(){
