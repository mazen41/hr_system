...
$appid  = 'HR';
$page_perm=['عرض خصم','تعديل خصم','حذف خصم','اعتماد خصم','إضافة خصم'];
// $screen = 'الخصومات';
...
$page_title = 'إدارة الخصومات';
 ...
 if (isset($_GET['id'])) {
    ...
    $query = "SELECT a.Id, a.BranchID, a.UserID, a.name, a.Amount, a.Currency, a.Reason, a.for_what, a.extionsion,a.Status ,
                     a.DueDate, a.CreatedDate, a.LastUpdateDate, a.CreatedBy, u.FirstName, u.LastName, d.branch_name
    ...
    if ($st->rowCount() > 0) {
        ...
        if ($row['for_what'] == 1 || $row['for_what'] == 5) {
				...
				<div class="col-md-12">
					<div class="card">
						<div class="card-header p-2 d-print-none with-nav-btn">
                            ...
                            <?php  if ($User->isAllowedPerm(['تعديل خصم'], $appid)) {  if($flage){  ?>
								<a href="deductions-add?id=<?=$row['Id']?>" class="btn btn-default btn-white  btn-sm quick-action-btn">
                                ...
                                <a href="deductions-add?copy=1&id=<?=$row['Id']?>" class="btn btn-default btn-white  btn-sm quick-action-btn">
								<span class="fa fa-copy" aria-hidden="true"></span>
								نسخ</a>
                                <?php  } if ($User->isAllowedPerm(['اعتماد خصم'], $appid)) { ?>
...
function confirm_remove (id) {
	if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  $('#modal_default .modal-body').load('./hr-app/deductions-remove?id='+id+'',function(){
...
function confirm_check (id) {
	if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
				  ...
				  $('#modal_default .modal-dialog').removeClass('modal-lg');
				$('#modal_default').modal({show:true});
			  $('#modal_default .modal-body').load('./hr-app/deductions-conform?id='+id+'',function(){
			});
		}
	}    
