...
if(!empty($_GET['confirme']) && !empty($_POST['benefits_id']) ){
	
	 $result = false;
	 $reload = false;
	 $msg ='خطاء..!';
	 $id= (int) $_POST['benefits_id'];

     function GetlastID($connect, $id) {
        $sql = "SELECT Status, DueDate,beneft_type FROM tblbenefit WHERE Id = :id LIMIT 1";
...
$flage_3=false;
	...
	function removeRow($connect, $id) {
		...
		try {
                ...
                $parma2 = ["id" => (int)$id ];
                $q2 = "delete  from  tblbenefit  WHERE Id  =:id ";
...
<form class="form-horizontal" role="form" action="#" method="post" id="remove_confirmed_fm">
	<p class="text-danger"><b>هل ترغب بالفعل بحذف التعويض</b>
	<input type="hidden" name="benefits_id" id="benefits_id" class="form-control" value="<?=$_GET['id']?>" >
...
$(document).ready(function(){
    ...
    $("#remove_confirmed_fm").on('submit', function(e){
        ...
        var form_data = $(this).serialize();
            $.ajax({
                url: 'hr-app/index.php?action=Benefits-remove?confirme=1',
                type: 'POST',
                data:form_data,
                dataType:"json",
                beforeSend:function(){
