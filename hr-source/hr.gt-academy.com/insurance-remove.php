...
if(!empty($_GET['confirme']) && !empty($_POST['insureance_id']) ){
	
	 $result = false;
	 $reload = false;
	 $msg ='خطاء..!';
	 $id= (int) $_POST['insureance_id'];
	...
	function removeRow($connect, $id) {
		...
		try {
                ...
                $parma2 = ["id" => (int)$id ];
                $q2 = "delete  from   tbinsurance  WHERE Id   =:id LIMIT 1";
		...
		return false;
				...
				{
					$msg = 'شركة التامين مرتبطة باجهزة اخرى  مرتبطه بمستخدمين ولايمكن حذفها';
				}
...
<form class="form-horizontal" role="form" action="#" method="post" id="remove_confirmed_fm">
	<p class="text-danger"><b>هل ترغب بالفعل بحذف العميل</b>
	<input type="hidden" name="insureance_id" id="insureance_id" class="form-control" value="<?=$_GET['id']?>" >
...
$(document).ready(function(){
    ...
    $("#remove_confirmed_fm").on('submit', function(e){
        ...
        var form_data = $(this).serialize();
            $.ajax({
                url: 'hr-app/index.php?action=insurance-remove?confirme=1',
                type: 'POST',
                data:form_data,
                dataType:"json",
                beforeSend:function(){
                ...
                success:function(data){
                    ...
                    if(data.reload){
                        window.location.href = 'insurance-list';
                    }
