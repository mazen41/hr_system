...
$parma = array( ':id'  => $id, );
		...
		u.end_date_health,u.path_health,t.new_s_date as start_date ,t.new_e_date as end_date,
        r.DueDate,r.Status,r.type
        FROM tblusers u
        
		 
        left join tblremewal t ON t.Id = u.lastversion
        left join tblresignation r ON r.UserID = u.UserID and r.Status is not null
    ...
    if($stm->rowCount() > 0)
    {
        ...
        if(!empty($employee['DueDate']) && $today_day >= $employee['DueDate'])
        {
             $stopped_status = $employee['type']==1?'مستقيل':'مفصول';
            $stopped_color = 'danger';
        }
        if($today_day >$employee['end_date'] )
        {
            $stopped_status = 'انتهاء العقد';
            $stopped_color = 'danger';
        }
    }

    $structure="SELECT t.id,t.UserID ,t.SectionID,t.GroupID,t.GradeID,t.jobtitleID,t.TypeID,t.shiftID,
    t.fingerID,t.Salary,t.Currency,t.state,t.come_name,t.new_s_date,t.new_e_date ,t.conform_date,
...
$user_shift = !empty($struct['shiftID']) ? array_unique(explode ( ',', $struct['shiftID'])) : [];
...
@media (max-width: 768px) {
    ...
    <div class="card-header p-2 d-print-none with-nav-btn">
        <div class=" btn-group dropdown-btn-group" style="direction:rtl">
            <?php   if ($User->isAllowedPerm(['تعديل موظف'], $appid)){ ?> 
            <a href="employer-add?id=<?=$employee['UserID']?>" class="btn btn-default btn-white  btn-sm quick-action-btn">
                    ...
                    <div class="card">
                        <div class="card-body p-2">
                            <ul class="nav nav-tabs d-print-none" id="employee-tabs" role="tablist">
                                ...
                                <!-- <li class="nav-item">
                                    <a class="nav-link" id="contract-tab" data-toggle="pill" href="#contract" role="tab" aria-controls="contract" aria-selected="false">
                                        <strong>اصدار عقد العمل</strong>
                                    </a>
                                </li> -->
                            </ul>
                                ...
                                <table class="table table-hover table-striped mb-0">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <span class="d-block"><?= !empty($stru['Salary'])?($stru['Salary'].' '.$stru['Currency']) :'' ?></span>
                                                <small class="text-muted">تاريخ الاعتماد:  <?= !empty($stru['conform_date'])?$stru['conform_date'] :'' ?></small> 
                                            </td>
                                            <td>
...
//         document.getElementById('promotionDetailsCard').classList.remove('d-none');
//         window.scrollTo({
//             top: document.getElementById('promotionDetailsCard').offsetTop - 20,
//             behavior: 'smooth'
//         });
//     });
// });
...
//     document.getElementById('promotionDetailsCard').classList.add('d-none');
// });
</script>