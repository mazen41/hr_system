...
class UploadController
{
    ...
    public static function leaveAttachment(array $params): void
    {
        ...
        // Update leave record
        $connect_pdo->prepare("UPDATE tblleaverequest SET path = :file, LastUpdateDate = NOW() WHERE Id = :id")
            ->execute([':file' => $result['filename'], ':id' => $id]);

        $auditLog->log($apiUser['id'], 'upload', 'tblleaverequest', $id, null, ['path' => $result['filename']]);
    ...
    public static function orderAttachment(array $params): void
    {
        ...
        $stm = $connect_pdo->prepare("SELECT Id, UserID FROM emp_order WHERE Id = :id LIMIT 1");
        $stm->execute([':id' => $id]);
        ...
        $connect_pdo->prepare("UPDATE emp_order SET attachment = :file, LastUpdateDate = NOW() WHERE Id = :id")
            ->execute([':file' => $result['filename'], ':id' => $id]);

        $auditLog->log($apiUser['id'], 'upload', 'emp_order', $id, null, ['attachment' => $result['filename']]);
    ...
    private static function handleUpload(string $subfolder, ?array $allowedExt = null): array
    {
        ...
        $baseUploadDir = defined('UPLOADS_DIR')
            ? rtrim(UPLOADS_DIR, '/\\')
            : rtrim(dirname(__DIR__, 3) . '/uploads', '/\\');
