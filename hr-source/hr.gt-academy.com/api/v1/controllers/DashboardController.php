...
class DashboardController
{
    ...
    public static function employee(): void
    {
        ...
        $stm = $connect_pdo->prepare(
            "SELECT Type, Time FROM attendancet
             WHERE EmpID = :uid AND Date = :date ORDER BY ID ASC"
        );
        ...
        $stm2 = $connect_pdo->prepare(
            "SELECT lc.Name, lc.Amount as max_days,
                    COALESCE(SUM(CASE WHEN l.status = 1 THEN l.day_leave ELSE 0 END), 0) as used,
                    COALESCE(SUM(CASE WHEN l.status IS NULL THEN l.day_leave ELSE 0 END), 0) as pending
             FROM leaveclassification lc
             LEFT JOIN tblleaverequest l ON l.leavetype = lc.Id AND l.UserID = :uid AND YEAR(l.leave_start_date) = :year
             WHERE (lc.BranchID = :branch OR lc.BranchID IS NULL) AND (lc.state IS NULL OR lc.state = 1)
             GROUP BY lc.Id, lc.Name, lc.Amount"
        );
        ...
        $stm3 = $connect_pdo->prepare(
            "SELECT 
                (SELECT COUNT(*) FROM tblleaverequest WHERE UserID = :uid1 AND status IS NULL) as pending_leaves,
                (SELECT COUNT(*) FROM tblempadvances WHERE UserID = :uid2 AND Status IS NULL) as pending_advances,
                (SELECT COUNT(*) FROM emp_order WHERE UserID = :uid3 AND Status IS NULL) as pending_orders"
        );
    ...
    public static function manager(): void
    {
        ...
        $stm3 = $connect_pdo->prepare(
            "SELECT a.EmpID, MIN(a.Time) as first_in,
                    sh.ShiftStartTime, ss.allowed_late_minutes
             FROM attendancet a
             ...
             GROUP BY a.EmpID, sh.ShiftStartTime, ss.allowed_late_minutes"
        );
        ...
        while ($row = $stm3->fetch(PDO::FETCH_ASSOC)) {
            if ($row['ShiftStartTime'] && $row['first_in']) {
                $tolerance = (int) ($row['allowed_late_minutes'] ?? 0);
        ...
        // 3. Pending approvals
        $stm4 = $connect_pdo->prepare("SELECT COUNT(*) as cnt FROM tblleaverequest WHERE UserID IN ($placeholders) AND status IS NULL");
        ...
        $stm7 = $connect_pdo->prepare("SELECT COUNT(*) as cnt FROM emp_order WHERE UserID IN ($placeholders) AND Status IS NULL");
        $stm7->execute(array_map('intval', $subIds));
        $pendingOrders = (int) $stm7->fetch(PDO::FETCH_ASSOC)['cnt'];

        $stm8 = $connect_pdo->prepare("SELECT COUNT(*) as cnt FROM order_finger_add WHERE UserID IN ($placeholders) AND Status IS NULL");
        ...
        $stm9 = $connect_pdo->prepare(
            "SELECT COUNT(*) as cnt FROM tblleaverequest
             WHERE UserID IN ($placeholders) AND status = 1 AND ? BETWEEN leave_start_date AND leave_end_date"
        );
