...
if(isset($_GET['id'])){
	$get_id = (int)$_GET['id'];
    $query = " SELECT a.Id  ,b.branch_name,a.Name,a.type,a.state,a.LastUpdateDate,a.Email,a.Phone,a.Address,a.NameOfRepresentative,a.Note,
					...
					<div class="card">
						<div class="card-header p-2 d-print-none with-nav-btn">
							<div class=" btn-group dropdown-btn-group" style="direction:rtl">
								...
								قائمة الاعدادات</a>
								<a href="insurance-list" class="btn btn-default btn-white  btn-sm quick-action-btn">
								<span class="fa fa-list" aria-hidden="true"></span>
								قائمة الشركات</a>
								<?php
							if ($User->isAllowedPerm(['اضافة شركة تامين'], $appid)) {   ?>
								<a href="insurance-add" class="btn btn-default btn-white  btn-sm quick-action-btn">
								<span class="fa fa-plus" aria-hidden="true"></span>
								إضافة شركة جديدة</a>
								<?php  }
								if ($User->isAllowedPerm(['تعديل شركة تامين'], $appid)) {   ?>
								<a href="insurance-add?id=<?=$row['Id']?>" class="btn btn-default btn-white  btn-sm quick-action-btn">
								<span class="fa fa-edit" aria-hidden="true"></span>
								تعديل</a>
								<?php  }
								if ($User->isAllowedPerm(['حذف شركة تامين'], $appid)) { ?>
                             ...
                             <?php if(!empty($row['CreatedDate']) ):?>
                               ...
                               <?php endif;?>
                                ...
                                <?php if(!empty($row['type']) ):?>
									<td>نوع شركة التامين</td>
									<td>
                                     <?= ($row['type'] == 1 ? "تأمين على الحياة" :
                                        ($row['type'] == 2 ? "تأمين صحي" :
                                        ($row['type'] == 3 ? "تأمين على السيارات" :
                                        ($row['type'] == 4 ? "تأمين ضد الحوادث" :
                                        ($row['type'] == 5 ? "تأمين الممتلكات" : "غير معروف"))))); ?> 
                                    </td>
								</tr>
                                <?php endif;?>
                                <tr>
                                <?php if(!empty($row['Email']) ):?>
									<td>الايميل الخاص بالشركة</td>
									<td><?= $row['Email']?></td>
								</tr>
                                <?php endif;?>
                                <tr>
                                <?php if(!empty($row['Phone']) ):?>
									<td>الهاتف الخاص بالشركة</td>
									<td><?= $row['Phone']?></td>
								</tr>
                                <?php endif;?>
                                <tr>
                                <?php if(!empty($row['Address']) ):?>
									<td>العنوان الخاص بالشركة</td>
									<td><?= $row['Address']?></td>
								</tr>
                                <?php endif;?>
                                <tr>
                                <?php if(!empty($row['NameOfRepresentative']) ):?>
									<td>اسم الشخص المسئول عن الشركة</td>
									<td><?= $row['NameOfRepresentative']?></td>
								</tr>
                                <?php endif;?>
                                <tr>
                                <?php if(!empty($row['Note']) ):?>
									<td>تفاصيل اخرى عن الشركة</td>
									<td><?= $row['Note']?></td>
								</tr>
                                <?php endif;?>
							</tbody>
...
function confirm_remove (id) {
	if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  $('#modal_default .modal-body').load('./hr-app/insurance-remove?id='+id+'',function(){
