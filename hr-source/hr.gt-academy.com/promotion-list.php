<?php
$appid  = 'HR';
$page_perm=['اضافة ترقية','عرض ترقية','تعديل ترقية','حذف ترقية'];
      ...
      <div class="container-fluid ">
        <div class="row ">
          ...
          <div class="col-5 text-left">	
                      <?php if($User->isAllowedPerm(['اضافة ترقية'],$appid)){ ?>
            <button type="button" class="btn btn-primary"  id="add-contract-bt"><i class="fas fa-plus"></i><span class="d-none d-sm-inline"> ترقية</span></button>		
            <?php
         } 
         ?> 
...
$('#add-contract-bt').on('click', function(event){
  window.open("contractRenewal-emp-list?type=permission", "_self");
});
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='',branchs=[], states=[])
 {
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=promotion-list",
    type:"POST",
    data:{
    ...
    aoColumns: [
        {
            data: 'employee_name', // NEW KEY
            render: function(data, type, row) { // Use 'data' if accessing the field directly, or 'row' to access the full object
                return data; 
            }
        },  
        {
            data: 'branch_name', // NEW KEY
        ...
        {
            data: 'creator_info', // NEW KEY
            render: function(data, type, row) {
                return '<small>'+data+'</small>'; 
            }
        },
        {
            data: 'renewal_status', // NEW KEY (This maps to 'state' from the DB)
            className: 'col_states',
            render: function(data, type, row) {
                // Map numeric status to readable Arabic string
                let statusText = '';
                if (data == 1) {
                    statusText = 'معتمد';
                } else if (data == 2) {
                    statusText = 'غير معتمد';
                } else if (data == 'pending') { // Check for workflow status if needed, but DB stores status as int
        ...
        { 
            data: 'id', // Use the ID column directly for actions
            "bSortable": false,
            render: function(data, type, row) {
                ...
                <?php if ($User->isAllowedPerm(['عرض ترقية'], $appid)) { ?>
                    options += '<a href="promotion-view?id=' + data + '" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['تعديل ترقية'], $appid)) { ?>
                    options += '<a href="promotion-add?edit=' + data + '" type="button" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['حذف ترقية'], $appid)) { ?>
                    options += '<button type="button" class="dropdown-item remove_client" value="' + data + '"><i class="fa fa-trash-alt"></i> حذف</button>';
                <?php } ?>
...
function confirm_remove (id) {
		if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  $('#modal_default .modal-body').load('./hr-app/promotion-remove?id='+id+'',function(){
