<?php
$appid  = 'HR';
$page_perm=['اضافة اجازة رسمية','عرض اجازة رسمية','تعديل اجازة رسمية','حذف اجازة رسمية'];
// $screen = 'العطلة';
...
.table.dataTable  td, .table.dataTable  th {
    ...
    <div class="content-header page-nav" >
      <div class="container-fluid ">
        <div class="row ">
          <div class="col-7">
            <span class="page-title">العطلات</span>
          </div><!-- /.col -->
          <div class="col-5 text-left">	
          <?php if($User->isAllowedPerm(['اضافة اجازة رسمية'],$appid)){ ?>
...
$('#add-client-bt').on('click', function(event){
   window.open("holidays-add","_self");
});
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='',branchs=[], states=[])
 {
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=holidays-list",
    type:"POST",
    data:{
    ...
    aoColumns: [
          ...
          var options='<div class="btn-group"> <button type="button" class="btn btn-default " data-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-left" role="menu">';
                <?php if ($User->isAllowedPerm(['عرض اجازة رسمية'], $appid)) { ?>
                    options += '<a href="holidays-view?id=' + data.id + '" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['تعديل اجازة رسمية'], $appid)) { ?>
                    options += '<a href="holidays-add?id=' + data.id + '" type="button" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['حذف اجازة رسمية'], $appid)) { ?>
