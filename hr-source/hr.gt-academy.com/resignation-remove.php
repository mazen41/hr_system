...
if(!empty($_GET['confirme']) && !empty($_POST['resignation_id']) ){
    ...
    $check=GetlastID($connect_pdo,$id);		
				...
				{
					$msg = 'تاريخ  الاستقالة اكبر من تاريخ اليوم';
				}
                else if($check==2)
				{
					$msg = 'هذا الاستقالة  تم اعتمادها ولايمكن حذفها';
				}
                else if($check==4)
				{
					$msg = 'الاستقالة هذه تم رفعها وليس لديك الصلاحية لحذفها';
				}
...
<form class="form-horizontal" role="form" action="#" method="post" id="remove_confirmed_fm">
	<p class="text-danger"><b>هل ترغب بالفعل بحذف الاستقالة</b>
...
$(document).ready(function(){
    ...
    $("#remove_confirmed_fm").on('submit', function(e){
        ...
        var form_data = $(this).serialize();
            $.ajax({
                url: 'hr-app/index.php?action=resignation-remove?confirme=1',
                type: 'POST',
                data:form_data,
                dataType:"json",
                beforeSend:function(){
                ...
                success:function(data){
                    ...
                    if(data.reload){
                        window.location.href = 'resignation-list';
                    }
