          ...
          function get_user_id($connect, $id)
     {
         ...
         if(!empty($user_id))
         {
             $query_upateversion = " UPDATE   tblusers SET
                resigned_or_dismissed=2 where UserID =$user_id
                ";
...
<form class="form-horizontal" role="form" action="#" method="post" id="conform_incentive">
	<p class="text-danger"><b>هل ترغب بالفعل برفع واعتماد الفصل</b>
...
$(document).ready(function(){
    ...
    $("#conform_incentive").on('submit', function(e){
        ...
        var form_data = $(this).serialize();
            $.ajax({
                url: 'hr-app/index.php?action=dismissal-upload?confirme=1',
                type: 'POST',
                data:form_data,
                dataType:"json",
                beforeSend:function(){
                ...
                success:function(data){
                    ...
                    if(data.reload){
                        window.location.href = 'resignation-view-add?id='+$("#resignation_id").val()+'; ';
                    }
