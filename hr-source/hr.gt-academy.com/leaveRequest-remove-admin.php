...
if(!empty($_GET['confirme']) && !empty($_POST['leaveRequest_id']) ){
     ...
     function GetlastID($connect, $id) {
        $sql = "SELECT status FROM  tblleaverequest WHERE Id = :id LIMIT 1";
    ...
    $check=GetlastID($connect_pdo,$id);		
				...
				{
					$msg = 'هذا الاجازة  تم اعتمادها ولايمكن حذفها';
				}
...
$(document).ready(function(){
    ...
    $("#remove_confirmed_fm").on('submit', function(e){
        ...
        var form_data = $(this).serialize();
            $.ajax({
                url: 'hr-app/index.php?action=leaveRequest-remove-admin?confirme=1',
                type: 'POST',
                data:form_data,
                dataType:"json",
                beforeSend:function(){
                ...
                success:function(data){
                    ...
                    if(data.reload){
                        window.location.href = 'leaveRequest-list-admin';
                    }
