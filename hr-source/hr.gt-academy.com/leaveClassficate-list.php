<?php
$appid  = 'HR';
$page_perm=['اضافة اجازة عامة','عرض اجازة عامة','تعديل اجازة عامة','حذف اجازة عامة'];
// $screen = 'انواع الاجازة';
// $page_title = 'إدارة انواع الاجازة';
...
.table.dataTable  td, .table.dataTable  th {
    ...
    <div class="content-header page-nav" >
      <div class="container-fluid ">
        <div class="row ">
          <div class="col-7">
            <span class="page-title">انواع الاجازات</span>
          </div><!-- /.col -->
            ...
            <div class="col-md-3">
              <div class="form-group">
                <label for="filter_status" class="col-form-label">الحالة</label>
                  ...
                  <div class="col-md-3">
              <div class="form-group">
                <label for="filter_type" class="col-form-label">نوع الاجازة</label>
                  <select class=" selectpicker show-tick" data-width="100%"  title="أي" id="filter_type" name="filter_type">
                    <option value="1">مدفوعة كليا</option>
...
function apply_filters()
{
    ...
    var type = get_filter('filter_type');
    $('#data_tb').DataTable().destroy();
    clients_data('yes',date_range,branchs,state,type); // افحص مخرجات الداله
}
...
$('#add-client-bt').on('click', function(event){
   window.open("leaveClassficate-add","_self");
});
...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='',branchs=[], states=[],type=[])
 {
	//var account = 0;
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=leaveClassficate-list",
    type:"POST",
    data:{
    ...
    aoColumns: [
          ...
          var options='<div class="btn-group"> <button type="button" class="btn btn-default " data-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-left" role="menu">';
                <?php if ($User->isAllowedPerm(['عرض اجازة عامة'], $appid)) { ?>
                    options += '<a href="leaveClassficate-view?id=' + data.id + '" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['تعديل اجازة عامة'], $appid)) { ?>
                    options += '<a href="leaveClassficate-add?id=' + data.id + '" type="button" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a>';
                <?php } ?>
