...
$appid  = 'HR';
$page_perm=['عرض مجموعه'];
// $screen = 'المجموعات الوظيفية';
// $page_title = 'إدارة المجموعات الوظيفيه';
...
if(isset($_GET['id'])){
	$get_id = (int)$_GET['id'];
    $query = " SELECT a.Id  ,b.branch_name,a.Name,a.LeaderGroup,a.LastUpdateDate, a.Description,
 u.FirstName , u.LastName,c.FirstName as empyler_f_leader,c.LastName as empyler_l_leader
FROM   tblgroup AS a
LEFT JOIN branches AS b ON a.BranchID = b.branch_id
LEFT JOIN tblusers AS u ON a.CreatedBy = u.UserID
LEFT JOIN tblusers AS c ON a.LeaderGroup = c.UserID
	WHERE a.Id = :id 
	LIMIT 1 ";
...
.bdanger{ border-right:0.3rem solid red; display:block;}
	...
	<?php endif;?>
					...
					<div class="card">
						<div class="card-header p-2 d-print-none with-nav-btn">
							<div class=" btn-group dropdown-btn-group" style="direction:rtl">
								...
								قائمة الاعدادات</a>
								<a href="groub-list" class="btn btn-default btn-white  btn-sm quick-action-btn">
								<span class="fa fa-list" aria-hidden="true"></span>
								قائمة المجموعات</a>
								<?php
							if ($User->isAllowedPerm(['اضافة مجموعه'], $appid)) {   ?>
								<a href="groub-add" class="btn btn-default btn-white  btn-sm quick-action-btn">
								<span class="fa fa-plus" aria-hidden="true"></span>
								إضافة مجموعة جديدة</a>
								<?php  } 
								if ($User->isAllowedPerm(['تعديل مجموعه'], $appid)) {   ?>
								<a href="groub-add?id=<?=$row['Id']?>" class="btn btn-default btn-white  btn-sm quick-action-btn">
								<span class="fa fa-edit" aria-hidden="true"></span>
								تعديل</a>
								<?php  } 
								if ($User->isAllowedPerm(['حذف مجموعه'], $appid)) { ?>
			...
			<div class="card-body p-2">
              ...
              <li class="nav-item">
                <a class="nav-link active" id="main-info-tab" data-toggle="pill" href="#main-info" role="tab" aria-controls="main-info" aria-selected="true"><strong>عن المجموعة</strong></a>
              </li>            
			</ul>
