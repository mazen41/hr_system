					...
					<div class="card">
						<div class="card-header p-2 d-print-none with-nav-btn">
							<div class=" btn-group dropdown-btn-group" style="direction:rtl">
                                ...
                                if($row['Draft']==null ){
                                ?>
								<a href="EmpAdvances-add-add?id=<?=$row['Id']?>" class="btn btn-default btn-white  btn-sm quick-action-btn">
                ...
                <div class="row">
                    <div class="col-md-12">
                         ...
                         <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table">
                          ...
                          <button type="button" value="<?=(!empty($row['path']) ? $row['path'].'.1' :'')?>" class="btn btn-xs btn-default" id="download_file"><i class="fa fa-download"></i> تنزيل المرفق</button>
                                </a></div></td>
								</tr>
                                <?php endif;?> 
...
function confirm_check (id) {
	if(id !=''){
			$('#modal_title').text('تأكيد عملية التقديم');
				  ...
				  $('#modal_default .modal-dialog').removeClass('modal-lg');
				$('#modal_default').modal({show:true});
			  $('#modal_default .modal-body').load('./hr-app/leaveRequest-upload-add?id='+id+'',function(){
			});
		}
	}    
