<?php
if(!isset($_GET['id'])){
	echo'<script> location.replace("report-benefits"); </script>';
	die(); 
} 
$screen = 'التعويض';
$page_title = 'تفاصيل  التعويض';
$report_name = 'تفاصيل التعويض';
 ...
 if (isset($_GET['id'])) {
    ...
    $query = "SELECT a.Id, a.BranchID, a.UserID, a.name, a.Amount,a.AmountType, a.Currency, a.Reason, a.for_what, a.extionsion,a.Status ,
                     a.DueDate, a.CreatedDate, a.LastUpdateDate, a.CreatedBy, u.FirstName, u.LastName, d.branch_name,a.beneft_type,a.monthly,a.DueDate
...
/* 
	...
	<?php endif;?>
			...
			<div class="card-body p-2">
              ...
              <li class="nav-item">
                <a class="nav-link active" id="main-info-tab" data-toggle="pill" href="#main-info" role="tab" aria-controls="main-info" aria-selected="true"><strong>عن التعويض</strong></a>
              </li>            
			</ul>
			...
			<!-- start -->
                               ...
                               <div class="section">
            <div class="section-title">معلومات التعويض</div>
            <table>
                <tbody><tr>
                    <td style="width: 25%; font-weight: bold;">اسم التعويض</td>
                    <td style="width: 25%;"><?= !empty($row['name'])?$row['name']:'غير محدد' ?></td>
                    <td style="width: 25%; font-weight: bold;">نوع التعويض</td>
                    <td style="width: 25%;"><?= !empty($row['beneft_type'])?(($row['beneft_type']==1)?'شهرية':'محدده'):'غير محدد' ?></td>
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><?= !empty($row['Amount']) ?($row['Amount'].' '.(($row['AmountType']=='avg')?'%':$row['Currency'])):'غير محدد' ?></td>
                </tr>
                <tr>
