<?php
$appid  = 'HR';
$page_perm=['إضافة خصم','عرض خصم','تعديل خصم','حذف خصم'];
// $screen = 'الخصومات ';
...
.table.dataTable  td, .table.dataTable  th {
    ...
    <div class="content-header page-nav" >
      <div class="container-fluid ">
        <div class="row ">
          <div class="col-7">
            <span class="page-title">الخصومات</span>
          </div><!-- /.col -->
          <div class="col-5 text-left">	
					
      <?php if($User->isAllowedPerm(['إضافة خصم'],$appid)){ ?>
...
$('#add-client-bt').on('click', function(event){
   window.open("deductions-add","_self");
});
...
clients_data('no');
 ...
 function clients_data(is_date_search, date_range='',branchs=[], states=[])
 {
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"hr-app/index.php?action=deductions-list",
    type:"POST",
    data:{
    ...
    aoColumns: [
          ...
          var options='<div class="btn-group"> <button type="button" class="btn btn-default " data-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-left" role="menu">';
                <?php if ($User->isAllowedPerm(['عرض خصم'], $appid)) { ?>
                    options += '<a href="deductions-view?id=' + data + '" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['تعديل خصم'], $appid)) { ?>
                    options += '<a href="deductions-add?id=' + data + '" type="button" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a>';
                <?php } ?>

                <?php if ($User->isAllowedPerm(['حذف خصم'], $appid)) { ?>
...
function confirm_remove (id) {
		if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  $('#modal_default .modal-body').load('hr-app/index.php?action=deductions-remove-modal&id='+id,function(){
