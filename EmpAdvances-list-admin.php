...
// هذه الداله تستقبل متغيرات وتقوم بعرض المحتوى على datatable
 function clients_data(is_date_search, date_range='', states=[])
 {
  ...
  var dataTable = $('#data_tb').DataTable({
   ...
   "ajax" : {
    url:"./hr-app/EmpAdvances-list-admin",
    type:"POST",
    data:{
    ...
    aoColumns: [
            ...
            },
            {
			  ...
			  "bSortable": false,
			  render: function(data, type) {
                return '<div class="btn-group"> <button type="button" class="btn btn-default " data-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-left" role="menu"><a href="EmpAdvances-view-admin?id='+data.id+'" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a><button type="button" class="dropdown-item  remove_client" value="'+data.id+'"><i class="fa fa-trash-alt"></i> حذف</button></div></div>'; 
				 }
