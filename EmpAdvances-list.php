            ...
            },
            {
			  ...
			  render: function(data, type) {
				 if(data.status==0 && data.draft___==0)
                   return '<div class="btn-group"> <button type="button" class="btn btn-default " data-toggle="dropdown"><i class="fas fa-ellipsis-h"></i></button><div class="dropdown-menu dropdown-menu-left" role="menu"><a href="EmpAdvances-view?id='+data.id+'" type="button" class="dropdown-item view-pos about_account"><i class="fa fa-eye"></i> عرض</a><a type="button"  href="EmpAdvances-add?id='+data.id+'" class="dropdown-item"><i class="fa fa-edit"></i> تعديل</a><button type="button" class="dropdown-item  remove_client" value="'+data.id+'"><i class="fa fa-trash-alt"></i> حذف</button></div></div>';    
                    else
