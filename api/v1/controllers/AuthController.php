...
class AuthController
{
    ...
    public static function login(): void
    {
        ...
        $stm = $connect_pdo->prepare(
            "SELECT UserID, UserEmail, UserPassword, FirstName, SecondName, LastName,
        ...
        $validPassword = Security::verifyPassword($password, $user['UserPassword'] ?? '');
        ...
        if (Security::needsRehash($user['UserPassword'])) {
            $newHash = Security::hashPassword($password);
            $stm2 = $connect_pdo->prepare("UPDATE tblusers SET UserPassword = :hash WHERE UserID = :id");
    ...
    public static function forgotPassword(): void
    {
        ...
        // Find user
        $stm = $connect_pdo->prepare(
            "SELECT UserID, UserEmail, FirstName FROM tblusers WHERE UserEmail = :email AND IsDisabled IS NULL LIMIT 1"
        );
        $stm->execute([':email' => $email]);
        $user = $stm->fetch(PDO::FETCH_ASSOC);
        ...
        Response::success([
            ...
            'note'        => 'في بيئة الإنتاج سيتم إرسال الرابط عبر البريد الإلكتروني',
        ], 'تم إنشاء رمز إعادة تعيين كلمة المرور');
    }
    ...
    public static function resetPassword(): void
    {
        ...
        $connect_pdo->prepare(
            "UPDATE tblusers SET UserPassword = :hash WHERE UserID = :uid"
    ...
    public static function changePassword(): void
    {
        ...
        $stm = $connect_pdo->prepare("SELECT UserPassword FROM tblusers WHERE UserID = :uid LIMIT 1");
        $stm->execute([':uid' => $apiUser['id']]);
        $user = $stm->fetch(PDO::FETCH_ASSOC);

        if (!$user || !Security::verifyPassword($body['current_password'], $user['UserPassword'])) {
            Response::error('كلمة المرور الحالية غير صحيحة', 422);
        }

        // Update password
        $newHash = Security::hashPassword($body['new_password']);
        $connect_pdo->prepare("UPDATE tblusers SET UserPassword = :hash WHERE UserID = :uid")
