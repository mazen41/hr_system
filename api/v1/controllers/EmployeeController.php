...
class EmployeeController
{
    ...
    public static function salarySlips(): void
    {
        ...
        $stm = $connect_pdo->prepare(
            "SELECT sd.Id, sd.registration_id, sd.basic_salary, sd.benefits,
                    sd.deductions, sd.incentives, sd.advances, sd.net_salary,
                    sr.month, sr.year, sr.BranchID, sr.created_at as issued_date
             FROM salary_details sd
             JOIN salary_registration sr ON sr.Id = sd.registration_id
             WHERE sd.UserID = :uid
        ...
        $stm2 = $connect_pdo->prepare(
            "SELECT COUNT(*) as total FROM salary_details WHERE UserID = :uid"
        );
        ...
        $formatted = array_map(function ($slip) {
            return [
                ...
                'benefits'     => (float) $slip['benefits'],
                'deductions'   => (float) $slip['deductions'],
                'incentives'   => (float) $slip['incentives'],
    ...
    public static function salarySlipById(array $params): void
    {
        ...
        $stm = $connect_pdo->prepare(
            "SELECT sd.Id, sd.registration_id, sd.UserID, sd.basic_salary, sd.benefits,
                    sd.deductions, sd.incentives, sd.advances, sd.net_salary,
                    sd.housing, sd.transport, sd.food, sd.phone_allowance,
                    sd.other_benefits, sd.gosi_deduction, sd.absence_deduction,
                    sd.late_deduction, sd.loan_deduction, sd.other_deductions,
                    sr.month, sr.year, sr.BranchID, sr.created_at as issued_date,
                    u.FirstName, u.SecondName, u.LastName, u.UserEmail,
                    jt.Name as JobTitleName, b.branch_name
             FROM salary_details sd
             JOIN salary_registration sr ON sr.Id = sd.registration_id
             JOIN tblusers u ON u.UserID = sd.UserID
             LEFT JOIN tblremewal r ON r.Id = u.lastversion
             LEFT JOIN tbljobtitle jt ON jt.Id = r.jobtitleID
             LEFT JOIN branches b ON b.branch_id = sr.BranchID
             WHERE sd.Id = :id AND sd.UserID = :uid
             LIMIT 1"
        );
        ...
        Response::success([
            ...
            'basic_salary'   => (float) $slip['basic_salary'],
            'earnings'       => [
                'housing'         => (float) ($slip['housing'] ?? 0),
                'transport'       => (float) ($slip['transport'] ?? 0),
                'food'            => (float) ($slip['food'] ?? 0),
                'phone_allowance' => (float) ($slip['phone_allowance'] ?? 0),
                'other_benefits'  => (float) ($slip['other_benefits'] ?? 0),
                'benefits'        => (float) $slip['benefits'],
                'incentives'      => (float) $slip['incentives'],
            ],
            'deductions'     => [
                'gosi'       => (float) ($slip['gosi_deduction'] ?? 0),
                'absence'    => (float) ($slip['absence_deduction'] ?? 0),
                'late'       => (float) ($slip['late_deduction'] ?? 0),
                'loan'       => (float) ($slip['loan_deduction'] ?? 0),
                'other'      => (float) ($slip['other_deductions'] ?? 0),
                'advances'   => (float) $slip['advances'],
                'total'      => (float) $slip['deductions'],
            ],
            'net_salary'     => (float) $slip['net_salary'],
            'issued_date'    => $slip['issued_date'],
        ]);
