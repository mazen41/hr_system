...
class DocumentController
{
    ...
    public static function salarySlip(array $params): void
    {
        ...
        $stm = $connect_pdo->prepare(
            "SELECT sd.*, sr.month, sr.year, sr.BranchID, sr.created_at as issued_date
             FROM salary_details sd
             JOIN salary_registration sr ON sr.Id = sd.registration_id
             WHERE sd.UserID = :uid AND sr.month = :month AND sr.year = :year
             LIMIT 1"
        );
        $stm->execute([
            ':uid'   => $apiUser['id'],
        ...
        Response::success([
            ...
            'earnings' => [
                ...
                'benefits'     => (float) ($slip['benefits'] ?? 0),
                'incentives'   => (float) ($slip['incentives'] ?? 0),
                'details'      => array_map(function ($b) {
                    return ['name' => $b['name'], 'amount' => (float) $b['Amount']];
                }, $benefits),
            ],
            'deductions_detail' => [
                'total'   => (float) ($slip['deductions'] ?? 0),
            ...
            'summary' => [
                'total_earnings'   => (float) ($slip['basic_salary'] ?? 0) + (float) ($slip['benefits'] ?? 0) + (float) ($slip['incentives'] ?? 0),
                'total_deductions' => (float) ($slip['deductions'] ?? 0) + (float) ($slip['advances'] ?? 0),
