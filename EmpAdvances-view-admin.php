...
$page_title = 'إدارة السلف';
 ...
 if (isset($_GET['id'])) {
    ...
    $query = "SELECT a.Id , a.UserID, a.Amount, a.currency,a.DueDate,a.Status,a.Draft,a.type,a.description,
...
function confirm_check (id) {
	if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
				  ...
				  $('#modal_default .modal-dialog').removeClass('modal-lg');
				$('#modal_default').modal({show:true});
			  $('#modal_default .modal-body').load('./hr-app/EmpAdvances-conform-admin?id='+id+'',function(){
			});
		}
	}    
