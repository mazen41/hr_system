...
$screen = 'طلبات الاجازات';
$page_title = 'تفاصيل طلبات الاجازات';
$report_name = 'تفاصيل طلبات الاجازات';
...
f.FirstName as f_name, f.LastName as l_name,b.branch_name,a.description,w.Name
      ...
      <div class="container-fluid "> 
        <div class="row ">
         <div class="col-md-9 col-sm-7 col-xs-12">
            <span class="page-title"><?= $justname['f_name'].' '.$justname['l_name'] ?> </span>
...
/* 
	...
	<?php endif;?>
        ...
        <div class="section">
            <div class="section-title">تقرير اجازات الموظف</div>
            <table>
                <thead>
                    <tr>
                        <th >#</th>
                ...
                <?php
$i = 1;
foreach ($row as $index => $ins) { 
    $rowId = "row-" . $index;
    ?>
    <tr onclick="toggleReason('<?= $rowId ?>')" style="cursor:pointer;">
        <td><?= $i++ ?></td> 
        <td><?= $ins["branch_name"] ?></td>
        <td><?= $ins["Name"]  ?></td>
        <td><?= $ins["leave_start_date"] ?></td>
        <td><?= $ins["leave_end_date"] ?></td>
        <td><?= $ins["day_leave"] ?></td>
        <td><?= !empty($ins["status"]) ? 'معتمد' : 'غير معتمد' ?></td>
        <td><?= !empty($ins["Draft"]) ? 'تم الرفع' : 'مسودة' ?></td>
        <td>
            <?php if (!empty($ins['path'])){  ?>
            <a href="<?= $ins['path'] ?>" download>
                <i class="fa fa-download"></i>
            </a>
        <?php } ?>
        </td>
    </tr>
    <tr id="<?= $rowId ?>" style="display:none; background:#f9f9f9;">
        <td colspan="7">
            <strong>تفاضيل اكثر :</strong> 
             <a href="report-one-specifac-leaveRequest?id=<?= $ins['Id'] ?>">
