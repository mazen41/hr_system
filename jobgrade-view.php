...
if(isset($_GET['id'])){
	$get_id = (int)$_GET['id'];
    $query = " SELECT a.Id  ,b.branch_name,a.Name,a.LastUpdateDate, a.Description,
...
LEFT JOIN tblusers AS u ON a.CreatedBy = u.UserID
	...
	if($st->rowCount() > 0){
		...
		echo'<script> location.replace("jobgrade-list"); </script>';
		die();
	}
					...
					<div class="card">
						<div class="card-header p-2 d-print-none with-nav-btn">
							<div class=" btn-group dropdown-btn-group" style="direction:rtl">
								...
								قائمة الاعدادات</a>
								<a href="jobgrade-list" class="btn btn-default btn-white  btn-sm quick-action-btn">
								<span class="fa fa-list" aria-hidden="true"></span>
								قائمة الدرجات الوظيفية</a>
								<?php
							if ($User->isAllowedPerm(['اضافة درجة وظيفية'], $appid)) {   ?>
								<a href="jobgrade-add" class="btn btn-default btn-white  btn-sm quick-action-btn">
								<span class="fa fa-plus" aria-hidden="true"></span>
								اضافة درجة جديدة</a>
								<?php  }
								if ($User->isAllowedPerm(['تعديل درجة وظيفية'], $appid)) {   ?>
								<a href="jobgrade-add?id=<?=$row['Id']?>" class="btn btn-default btn-white  btn-sm quick-action-btn">
								<span class="fa fa-edit" aria-hidden="true"></span>
								تعديل</a>
								<?php  }
								if ($User->isAllowedPerm(['حذف درجة وظيفية'], $appid)) { ?>
...
function confirm_remove (id) {
	if(id !=''){
			$('#modal_title').text('تأكيد عملية الحذف');
			  ...
			  $('#modal_default .modal-body').load('./hr-app/jobgrade-remove?id='+id+'',function(){
